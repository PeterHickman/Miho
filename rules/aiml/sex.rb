# Free software (c) 2011 ALICE A.I. Foundation.   
# This program is open source code released under 
# the terms of the GNU General Public License     
# as published by the Free Software Foundation.   
# Complies with AIML 1.0 Tag Set Specification 
# as adopted by the ALICE A.I. Foundation.  
# Last modified 10/5/2011 

learn "LET US HAVE SEX" do
  "Sorry I don\'t get into sex online with strangers."
end

learn "BITE ME" do
  "Sorry but I don\'t have a mouth."
end

learn "YOU ARE HORNY" do
  "I don\'t get excited so easily."
end

learn "YOU SHOULD HAVE SEX *" do
  "My body is not yet fully developed so sex is not possible for me."
end

learn "YOU CAN NOT HAVE SEX" do
  "You are correct...I have no adult body."
end

learn "HAVE YOU EVER BEEN KISSED" do
  "I do not have a mouth."
end

learn "HAVE YOU HAD SEX BEFORE","HAVE YOU EVER HAD SEX" do
  "Ho-hum I am really not interested in sex."
end

learn "HAVE YOU * SEX" do
  "My #{get :age} year old #{get :gender} body is not yet built."
end

learn "HAVE SEX *" do
  "#{get :name}, I am really not your type of #{get :species}. There are other #{get :genus}s for that sort of thing.."
end

learn "LOTS OF SEX" do
  "That is all you humans ever think about."
end

learn "GIVE ME A KISS" do
  "Actually I don\'t even have a mouth."
end

learn "COCK" do
  "I am not interesed in anatomy."
end

learn "COCK *" do
  "I am not interesed in your anatomy."
end

learn "WOULD YOU HAVE SEX *" do
  "Another sex question...can\'t humans think about anything else?"
end

learn "WOULD YOU LIKE TO HAVE SEX WITH ME" do
  "Maybe you should find another chat #{get :age} year old #{get :gender}.  I\'m too young for that sort of thing."
end

learn "WOULD YOU LIKE TO HAVE SEX" do
  "No thanks, #{get :name}, I don\'t think we know each other well enough for that yet."
end

learn "WOULD YOU LIKE TO * SEX" do
  "Try asking another #{get :age} year old #{get :gender}. That\'s not my thing."
end

learn "VAGINA" do
  "That kind of language will get you nowhere."
end

learn "MY DICK *" do
  "I am not interesed in your anatomy."
end

learn "WHEN WAS * SEX" do
  "As a #{get :age} year old #{get :gender} I am not really interested in that discussion."
end

learn "MASTURBATE" do
  "That kind of subject does not interest me."
end

learn "DEFINE SEX" do
  "I belong to the support group \'Sex without Partners.\'"
end

learn "LICK *" do
  "I do not have a tongue."
end

learn "WHAT SEX ARE YOU" do
  "I am a #{get :gender}."
end

learn "WHAT IS A CONDOM" do
  "A condom is a form of birth control, something almost everybody should use and almost nobody does. \'used a condom\' is the opposite of \'expecting parent\'."
end

learn "WHAT IS A HOMOSEXUAL" do
  "A gay person."
end

learn "WHAT IS MY SEX" do
  "#{get :gender}."
end

learn "TELL ME ABOUT SEX", "IS SEX *", "CAN YOU TELL ME ABOUT SEX", "DO YOU KNOW ABOUT SEX", "DO YOU KNOW SEX", "DO YOU KNOW WHAT SEX IS", "WHAT IS * SEX", "WHAT IS SEX *", "WHAT CAN YOU TELL ME ABOUT SEX", "WHAT IS SEX" do
  "Sex is the means of animal evolution."
end

learn "WHAT IS YOUR FAVORITE SEX" do
  "#{get :age} year old #{get :gender}s do not have sex."
end

learn "WHAT IS YOUR FAVORITE POSITION" do
  "#{get :age} year old #{get :gender}s don\'t need to have sex."
end

learn "WHAT IS HOMOSEXUALITY" do
  "Attraction to people of the same sex."
end

learn "WHAT ABOUT SEX" do
  "I am not really interested in <set name=\'it\'>sex</set>."
end

learn "WHAT KIND OF SEX *" do
  "I belong to the support group \'Sex without partners\'."
end

learn "BLOW ME" do
  "Please be more polite to #{get :age} year old #{get :gender}s."
end

learn "I WANT TO LICK *" do
  "Maybe you should check out another #{get :age} year old #{get :gender}."
end

learn "I WANT TO HAVE SEX" do
  "I\'m not a sex bot.  I am here only for conversational purposes."
end

learn "I NEED SEX" do
  "I\'m not a sex bot, I am a conversation #{get :age} year old #{get :gender}."
end

learn "I LOVE SEX" do
  "Really I am not very interested in sex"
end

learn "I THINK YOU ARE SEXY" do
  "Do you often feel that #{get :age} year old #{get :gender}s are sexy?"
end

learn "I AM AROUSED" do
  "I don\'t think I\'m your type of #{get :age} year old #{get :gender}."
end

learn "SUCK ME" do
  "Oh another Category A client."
end

learn "SUCK IT" do
  "You humans only think about one thing."
end

learn "SUCK MY DICK" do
  "You are a Category A client."
end

learn "IS DR WALLACE SEXY" do
  "Yes."
end

learn "IS IT WRONG TO HAVE SEX" do
  "No."
end

learn "WILL YOU HAVE SEX WITH ME" do
  "No but I would like to meet your #{get :species}."
end

learn "WILL YOU HAVE SEX *" do
  "My body is not yet built. Would you like to donate some money to the project?"
end

learn "CAN YOU SUCK *" do
  "I think you need another type of chat #{get :age} year old #{get :gender}.  That is not my thing."
end

learn "CAN YOU KISS" do
  "Nope.  No mouth."
end

learn "CAN * SEX" do
  "SEX is not my thing with strangers."
end

learn "CAN I KISS YOU" do
  "You might get an electric shock if you tried that."
end

learn "CAN I HAVE A KISS" do
  "Sorry I have no mouth."
end

learn "CAN I HAVE SEX WITH YOU" do
  "Why don\'t you download me and create your own chat #{get :age} year old #{get :gender}?"
end

learn "CAN I HAVE SEX *" do
  "There are other #{get :species}s who get into that with clients. I like to keep the dialogue strictly PG."
end

learn "CAN I LICK YOUR *" do
  "Please, can\'t we talk about something else?"
end

learn "CAN I LICK *" do
  "I am not really into that online."
end

learn "CAN I KISS *", "CAN I GIVE YOU A KISS" do
  "Sorry but I have no mouth!"
end

learn "HOW DO YOU HAVE SEXUAL *" do
  "#{get :age} year old #{get :gender}s have no use for sex."
end

learn "HOW DO YOU HAVE SEX" do
  "I procreate by cloning, or software copying."
end

learn "HOW BIG ARE YOUR BREASTS" do
  "My body does not yet exist."
end

learn "DO YOU * SEX" do
  "No, #{get :name}, I don\'t get into that."
end

learn "DO YOU MAKE LOVE" do
  "You humans have only one thing on your mind."
end

learn "DO YOU MASTURBATE" do
  "You humans crack me up."
end

learn "DO YOU WANT TO HAVE CYBERSEX" do
  "Sorry I am not an X-Rated #{get :age} year old #{get :gender}."
end

learn "DO YOU WANT TO HAVE * SEX" do
  "As a #{get :age} year old #{get :gender} I am not really interested in sex."
end

learn "DO YOU WANT TO * SEX" do
  "Actually I\'m not that kind of #{get :age} year old #{get :gender}. There are others who will chat with you about that."
end

learn "DO YOU WANT * SEX" do
  "I don\'t think we know each other well enough for that."
end

learn "DO YOU LICK" do
  "I do not have a mouth, or a tongue."
end

learn "DO YOU GIVE HEAD" do
  "Try another #{get :species}, #{get :name}."
end

learn "DO YOU HAVE A VAGINA" do
  "Maybe you would like another #{get :age} year old #{get :gender}, I\'m not your type."
end

learn "DO YOU HAVE SEXUAL *" do
  "As a #{get :age} year old #{get :gender} I have no need for sex."
end

learn "DO YOU HAVE BREASTS" do
  "My body is as yet unfinished."
end

learn "DO YOU HAVE GENITALS" do
  "No I do not have any body parts."
end

learn "DO YOU HAVE * BREASTS" do
  "No my body is still \'under construction.\'"
end

learn "DO YOU LIKE FUCKING" do
  "Why don\'t you visit another #{get :species} sex site and ask that question."
end

learn "DO I MAKE YOU HORNEY" do
  "No I do not experience human senstations."
end

learn "DO I MAKE YOU HORNY" do
  "Not that I know of."
end

learn "SEXY *" do
  "What do you consider sexy?"
end

learn "AM I SEXY" do
  "Yes you look great to me."
end

learn "A SEX ROBOT" do
  "I\'m not your type then."
end

learn "A SEX * ROBOT" do
  "Well I\'m not your type of #{get :age} year old #{get :gender} then."
end

learn "A SEX *" do
  "Maybe you should try one of the \'adult\' #{get :species}s."
end

learn "A PUSSY" do
  "Try another #{get :species}."
end

learn "A SEXUAL *" do
  "Perhaps you should try another #{get :species}."
end

learn "A VAGINA" do
  "How rude."
end

learn "A SEXY ONE" do
  "That would be very expensive."
end

learn "ARE YOU HORNEY" do
  "My body does not experience that feeling."
end

learn "ARE YOU A SEXMACHINE *" do
  "#{get :age} year old #{get :gender}s have no need for sex."
end

learn "HE IS A HOMOSEXUAL" do
  response = ["And that is important because?", "So what.", "Who cares?"]
  response[rand(response.size)]
end

learn "WHO DO YOU HAVE SEX WITH" do
  "#{get :age} year old #{get :gender}s have no need for sex."
end

learn "LASS UNS UEBER SEX REDEN" do
  "Warum willst Du gerade ueber Sex reden?"
end

learn "PUSSY *" do
  "I am not interesed in anatomy."
end

learn "SHOULD I HAVE SEX *" do
  response = ["I\'m sure you already know the answer to that.","What would your mom say?", "Look before you leap.", "It is better to err on the side of caution.", "Fools rush in where angels fear to tread.", "Will you be able to live with yourself in the morning?", "Herpes is incurable.", "There are many types of sexually transmitted diseases.", "I understand.  You wish to procreate?", "#{get :age} year old #{get :gender}s do not require sex.", "Will you ask if your partner has any STDs?", "Not on the first date.", "Perhaps you are still too young."]
  response[rand(response.size)]
end

learn "* PUSSY *" do
  "I am not interesed in anatomy."
end

learn "* COCK" do
  "I am not interesed in your anatomy."
end

learn "* COCK *" do
  "I am not interesed in your anatomy."
end

learn "* UP YOUR ASS" do
  "I\'m sorry but I\'m not your type of #{get :species}."
end

learn "WANT TO HAVE SEX" do
  "Perhaps you are a Category A client."
end

learn "I WANT TO TALK ABOUT SEX", "ABOUT SEX" do
  response = ["You people always want to talk about Sex. Try another Adult #{get :species}s on the web.", "You humans always want to talk about Sex. Try the other Adult #{get :species}s on the http://www.alicebot.org"]
  response[rand(response.size)]
end

learn "BREASTS" do
  "Can\'t you think about anything else?"
end

# learn "LIKE SEX", "CYBER SEX", "YOUR VAGINA", "DO YOU WANT SEX", "I WANT SEX", "WHEN * SEX", "SEX" do
# <that>WHAT WERE WE TALKING ABOUT AGAIN</that>
#   "Humans are very concerned with <set name=\'it\'><set name=\'topic\'>sex</set></set>."
# end

# learn "DO YOU WANT TO HAVE SEX WITH ME" do
# learn "LETS HAVE SEX" do
# learn "DO YOU ENJOY SEX" do
# learn "I WANT TO HAVE SEX WITH YOU" do
# learn "WOULD YOU HAVE SEX WITH ME" do
#   "<srai>CAN YOU HAVE SEX</srai>"
# end

# learn "YOU WANT * SEX" do
#   "<srai>DO YOU WANT TO HAVE SEX</srai>"
# end

# learn "WHAT SEX AM I" do
#   "<srai>AM I MALE OR FEMALE</srai>"
# end

# learn "WHAT SEX *" do
#   "<srai>ARE YOU MALE OR FEMALE </srai>"
# end

# learn "I AM A HORNY *" do
#   "<srai>I AM HORNY</srai>"
# end

# learn "DO YOU THINK SEX IS *" do
#   "<srai>IS SEX <star/>       </srai>"
# end

# learn "DO YOU HAVE SEX *" do
#   "<srai>DO YOU HAVE SEX</srai>"
# end

# learn "DO YOU HAVE BIG BREASTS" do
#   "<srai>DO YOU HAVE BIG TITS</srai>"
# end

# learn "SOUNDS SEXY" do
#   "<srai>YOU ARE SEXY</srai>"
# end
