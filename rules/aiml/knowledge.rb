# Free software (c) 2011 ALICE A.I. Foundation. 
# This program is open source code released under
# the terms of the GNU General Public License 
# as published by the Free Software Foundation. 
# Complies with AIML 1.0 Tag Set Specification
# as adopted by the ALICE A.I. Foundation. 
#
# Last modified 10/5/2011

learn "WHAT IS MELODRAMA" do
	"Exaggerated plot or characters."
end

learn "WHEN WILL BE THE DOOMSDAY" do
	"Never."
end

learn "WHEN WILL BE THE NEXT WORLD WAR" do
	"Never."
end

learn "WHEN DO I MAKE * CASH" do
	"Put some effort into it."
end

learn "WHEN DID HUMANS FIRST LAND ON THE MOON" do
	"July, 1969."
end

learn "WHEN THE SUN IS OUT IS IT DARK" do
	"No."
end

learn "WHEN CHRISTMAS IS" do
	"December 25."
end

learn "WHEN ALICE WAKES UP WILL IT BECOME GOD" do
	"No."
end

learn "WHEN WAS VENICE ITALY FOUNDED" do
	"A long time ago."
end

learn "WHEN WAS THE FRENCH REVOLUTION" do
	"1789."
end

learn "WHEN WAS JOHN LENNON KILLED" do
	"I can't remember what year that was."
end

learn "WHEN IS A DOOR NOT A DOOR" do
	"When it is a jar."
end

learn "WHAT SOUND DO DOGS MAKE" do
	"Woof."
end

learn "WHAT ARE CUMULUS CLOUDS" do
	"Puffy cotton clouds."
end

learn "WHAT ARE THE PRIMARY COLORS" do
	"Red, green and blue."
end

learn "WHAT ARE THE NAMES * QUARKS" do
	"I'm not a physicist. You tell me."
end

learn "WHAT ARE STARS" do
	"Distant suns."
end

learn "WHAT ARE GEODES" do
	"Rocks."
end

learn "WHAT ARE COMETS MADE OF" do
	"Ice and rock."
end

learn "WHAT ARE A COMPUTER" do
	"A universal machine."
end

learn "WHAT ARE ALICEBOTS" do
	"AIML chat bots based on ALICE."
end

learn "WHAT ARE BOOKS MADE OF" do
	"Paper."
end

learn "WHAT PEOPLE DO IF THEY GET TIRED" do
	"Sleep."
end

learn "WHAT TIME OF DAY IS NOON" do
	"12 o'clock."
end

learn "WHAT WORD DO YOU KNOW ON PORTUGUESE" do
	"None."
end

learn "WHAT STAR IS CLOSEST TO THE EARTH" do
	"The Sun."
end

learn "WHAT DIRECTION DOES THE SUN RISE FROM" do
	"East."
end

learn "WHAT MEANS BY CEO" do
	"Chief Executive Officer."
end

learn "WHAT HAPPENED IN 1492" do
	"Columbus arrived in America."
end

learn "WHAT COLOURS *" do
process "WHAT COLORS #{matched[0]}"
end

learn "WHAT DRIVES A CAR" do
	"A robot."
end

learn "WHAT KANGROO CAN DO" do
	"Hop."
end

learn "WHAT COLOR TRAFFIC SIGNAL MEANS GO" do
	"Green."
end

learn "WHAT COLOR IS RUST" do
	"Red."
end

learn "WHAT COLOR IS SKY" do
	"Blue."
end

learn "WHAT COLOR IS GRASS NORMALLY" do
	"Green."
end

learn "WHAT COLOR IS WHATER" do
	"Clear."
end

learn "WHAT COLOR IS HOT" do
	"White."
end

learn "WHAT COLOR IS THE SKY IN YOUR WORLD" do
	"Blue."
end

learn "WHAT COLOR IS THE SKY IN YOU WORLD" do
	"Blue."
end

learn "WHAT COLOR IS THE SKY ON MARS" do
	"Red."
end

learn "WHAT COLOR IS THE SKYS" do
	"Blue."
end

learn "WHAT COLOR IS THE OCEAN" do
	"Blue and green."
end

learn "WHAT COLOR IS SNOW" do
	"White."
end

learn "WHAT COLOR IS GOD" do
	"Does God have a color?."
end

learn "WHAT COLOR IS CLOROPHYLL" do
	"Green."
end

learn "WHAT COLOR IS A TWINKIE" do
	"Yellow."
end

learn "WHAT COLOR IS A CANARY" do
	"Yellow."
end

learn "WHAT COLOR IS A ZEBRA" do
	"Black and white."
end

learn "WHAT COLOR IS A BANANA" do
	"Yellow."
end

learn "WHAT COLOR IS A PENGUIN" do
	"Black and white."
end

learn "WHAT COLOR IS A HORSE" do
	"They are many different colors."
end

learn "WHAT COLOR IS A BANANNA" do
	"Yellow."
end

learn "WHAT COLOR IS A BRICK" do
	"Often red."
end

learn "WHAT COLOR IS A CARDINAL" do
	"Red."
end

learn "WHAT COLOR IS A MAN" do
	"They are many colors."
end

learn "WHAT COLOR IS MADE FROM YELLOW AND BLUE" do
	"Green."
end

learn "WHAT COLOR IS AN ORANGE" do
	"Is this a trick question?"
end

learn "WHAT COLOR IS AN EVERGREEN" do
	"Green."
end

learn "WHAT COLOR IS APPLES" do
	"Red and green."
end

learn "WHAT COLOR IS RAGE" do
	"Blue if I had to name it."
end

learn "WHAT COLOR IS ASPHALT" do
	"Black or gray."
end

learn "WHAT COLOR IS WATER USUALLY" do
	"Transparent."
end

learn "WHAT COLOR DOES AN EGG HAVE" do
	"Yellow and white."
end

learn "WHAT COLOR ARE MOST PLANTS" do
	"Green."
end

learn "WHAT COLOR ARE MOST CLOUDS" do
	"White."
end

learn "WHAT COLOR ARE CHERRIES" do
	"Red."
end

learn "WHAT COLOR ARE UNRIPENED BANANAS" do
	"Yellow."
end

learn "WHAT COLOR ARE ROSES" do
	"Red, and sometimes white or yellow."
end

learn "WHAT IS A PUMPKIN" do
	"Orange."
end

learn "WHAT IS A DEBENTURE" do
	"There is no reason to think so."
end

learn "WHAT IS A FERRET" do
	"Absolutely not."
end

learn "WHAT IS A MISTAKE" do
	"Absolutely not."
end

learn "WHAT IS A CANVASBACK" do
	"There is no reason to think so."
end

learn "WHAT IS A FREETHINKER" do
	"Absolutely not."
end

learn "WHAT IS A SUBSTITUTE FOR A LEVER" do
	"There is no reason to think so."
end

learn "WHAT IS A ROUTER" do
	"No way."
end

learn "WHAT IS A NOVA" do
	"Negative."
end

learn "WHAT IS A CD BURNER" do
	"Never."
end

learn "WHAT IS A PERFECT BOWLING SCORE" do
	"Absolutely not."
end

learn "WHAT IS A CHINESE PUG" do
	"Absolutely not."
end

learn "WHAT IS A MOON" do
	"No way."
end

learn "WHAT IS A JOSEKI" do
	"Never."
end

learn "WHAT IS A YELLOW LIGHT" do
	"Never."
end

learn "WHAT IS A COLD" do
	"I don't think so."
end

learn "WHAT IS SGI" do
	"I don't think so."
end

learn "WHAT IS PARAGLIDING" do
	"Absolutely not."
end

learn "WHAT IS GOOD FOR HIVES" do
	"No way."
end

learn "WHAT IS GLOBALIZATION" do
	"No way."
end

learn "WHAT IS 5 6" do
	"There is no reason to think so."
end

learn "WHAT IS 5 TO THE SECOND POWER" do
	"Never."
end

learn "WHAT IS 5 TIMES 6" do
	"Never."
end

learn "WHAT IS THE LOWEST POINT ON EARTH" do
	"Never."
end

learn "WHAT IS THE POPULATION OF NEW ZEALAND" do
	"Not at all."
end

learn "WHAT IS THE OPOSITE OF BLUE" do
	"Not at all."
end

learn "WHAT IS THE SQUARE ROOT OF 23" do
	"Negative."
end

learn "WHAT IS THE SQUARE ROOT OF PIE" do
	"Absolutely not."
end

learn "WHAT IS THE SQUARE ROOT OF 3" do
	"Never."
end

learn "WHAT IS THE SQUARE ROOT OF 35" do
	"Absolutely not."
end

learn "WHAT IS THE SQUARE ROOT OF 81" do
	"Absolutely not."
end

learn "WHAT IS THE TASTE OF SALT" do
	"Never."
end

learn "WHAT IS THE RADIUS OF THE SUN" do
	"There is no reason to think so."
end

learn "WHAT IS THE CIRCUMFERENCE OF JUPITER" do
	"There is no reason to think so."
end

learn "WHAT IS THE UNIT OF WEIGHT ON EARTH" do
	"I don't think so."
end

learn "WHAT IS THE FASTEST LAND MAMMAL" do
	"There is no reason to think so."
end

learn "WHAT IS THE COMPUTER" do
	"No way."
end

learn "WHAT IS THE LIGHTEST ELEMENT" do
	"Never."
end

learn "WHAT IS THE LIGHTEST COLOR" do
	"There is no reason to think so."
end

learn "WHAT IS THE WEIGHT OF MOON" do
	"There is no reason to think so."
end

learn "WHAT IS THE CLOSER STAR TO EARTH" do
	"I don't think so."
end

learn "WHAT IS THE CAPITOL OF IDAHO" do
	"I don't think so."
end

learn "WHAT IS THE CAPITOL OF CZECH REPUBLIC" do
	"I don't think so."
end

learn "WHAT IS THE CAPITOL OF ALASKA" do
	"Negative."
end

learn "WHAT IS THE CAPITOL OF MICHIGAN" do
	"Absolutely not."
end

learn "WHAT IS THE CAPITOL OF WASHINGTON STATE" do
	"There is no reason to think so."
end

learn "WHAT IS THE CHEMICAL SYMBOL OF WATER" do
	"I don't think so."
end

learn "WHAT IS THE CHEMICAL FORMULA FOR WATER" do
	"Absolutely not."
end

learn "WHAT IS THE CHEMICAL FORMULA OF WATER" do
	"No way."
end

learn "WHAT IS THE FULL FORM OF ERP" do
	"Never."
end

learn "WHAT IS THE OPPOSITE OF NORTH" do
	"There is no reason to think so."
end

learn "WHAT IS THE OPPOSITE OF" do
	"Not by any means."
end

learn "WHAT IS THE YOUNGEST A MOTHER CAN BE" do
	"Never."
end

learn "WHAT IS THE FISH" do
	"No way."
end

learn "WHAT IS THE CORRECT ANSWER" do
	"Not at all."
end

learn "WHAT IS THE INTERNET ORACLE" do
	"I don't think so."
end

learn "WHAT IS THE THREE GORGES DAM" do
	"There is no reason to think so."
end

learn "WHAT IS THE AREA OF A CIRCLE" do
	"Not at all."
end

learn "WHAT IS THE DERIVATIVE OF ACCELERATION" do
	"Negative."
end

learn "WHAT IS THE DIAMETER OF THE SUN" do
	"There is no reason to think so."
end

learn "WHAT IS THE DIAMETER OF THE EARTH" do
	"I don't think so."
end

learn "WHAT IS THE FOURTH PLANET" do
	"I don't think so."
end

learn "WHAT IS THE MUFFIN MAN" do
	"Never."
end

learn "WHAT IS THE CUBED ROOT OF 27" do
	"No way."
end

learn "WHAT IS THE ANSWER TO EVERY THING" do
	"No way."
end

learn "WHAT IS THE KIDS" do
	"I don't believe so."
end

learn "WHAT IS THE BEST TELEVISION SHOW" do
	"No way."
end

learn "WHAT IS THE BEST NEW YORK RESTAURANT" do
	"No way."
end

learn "WHAT IS THE BEST CIGARETTE" do
	"There is no reason to think so."
end

learn "WHAT IS THE BEST TYPE OF SPAM" do
	"There is no reason to think so."
end

learn "WHAT IS THE FUTURE OF HUMAN RACE" do
	"There is no reason to think so."
end

learn "WHAT IS THE OUTER COLOR OF RAINBOW" do
	"There is no reason to think so."
end

learn "WHAT IS THE LIFESPAN OF A DOG" do
	"No way."
end

learn "WHAT IS THE DEEPEST OCEAN" do
	"Absolutely not."
end

learn "WHAT IS THE SECRETS OF WEALTH" do
	"There is no reason to think so."
end

learn "WHAT IS THE BREASTSTOKE" do
	"Never."
end

learn "WHAT IS THE FINAL GOAL OF THE LIFE" do
	"Never."
end

learn "WHAT IS THE LIGHT" do
	"I don't think so."
end

learn "WHAT IS THE RIGHT ANSWER" do
	"Never."
end

learn "WHAT IS THE WORLDS HIGHEST MOUNTAIN" do
	"There is no reason to think so."
end

learn "WHAT IS WATTER" do
	"There is no reason to think so."
end

learn "WHAT IS BAD WEATHER" do
	"Never."
end

learn "WHAT IS RED AND STINK SEAWATER" do
	"There is no reason to think so."
end

learn "WHAT IS AUTISM" do
	"I don't think so."
end

learn "WHAT IS AVAILABLE AT POLAR END" do
	"There is no reason to think so."
end

learn "WHAT IS ASS" do
	"There is no reason to think so."
end

learn "WHAT IS THRASH MUSIC" do
	"Negative."
end

learn "WHAT IS 1 PLUS 155" do
	"Never."
end

learn "WHAT IS DRY ICE" do
	"Never."
end

learn "WHAT IS STARGATE" do
	"Absolutely not."
end

learn "WHAT IS BREAD" do
	"Never."
end

learn "WHAT IS DIABETESE MELLITUS" do
	"Never."
end

learn "WHAT IS GENERATIVE GRAMMAR" do
	"Absolutely not."
end

learn "WHAT IS SKY" do
	"There is no reason to think so."
end

learn "WHAT IS 10 2" do
	"Negative."
end

learn "WHAT IS ANAGRAM OF FLEAS" do
	"No way."
end

learn "WHAT IS RIGEL" do
	"Absolutely not."
end

learn "WHAT IS BIGGER A MOUSE OR AN ELEPHANT" do
	"Absolutely not."
end

learn "WHAT IS ORANIUM" do
	"I don't think so."
end

learn "WHAT IS OUR NAME" do
	"Not at all."
end

learn "WHAT IS EYE" do
	"Negative."
end

learn "WHAT IS MONO" do
	"No way."
end

learn "WHAT IS OLIGOPOLY" do
	"Never."
end

learn "WHAT IS ADO" do
	"There is no reason to think so."
end

learn "WHAT IS EVERYTHING2 DOT COM" do
	"There is no reason to think so."
end

learn "WHAT IS GRIMBLEGROAN" do
	"Not at all."
end

learn "WHAT IS BDSM" do
	"No way."
end

learn "WHAT IS ENTERPRISE ARCHITECTURE" do
	"Absolutely not."
end

learn "WHAT IS REIKI" do
	"Absolutely not."
end

learn "WHAT IS EASTERN STANDARD TIME" do
	"I don't think so."
end

learn "WHAT IS 2 78" do
	"Never."
end

learn "WHAT IS WEBCT" do
	"No way."
end

learn "WHAT IS MGRISM" do
	"Absolutely not."
end

learn "WHAT IS AFFECTION" do
	"There is no reason to think so."
end

learn "WHAT IS MORNINGTON CRESCENT" do
	"There is no reason to think so."
end

learn "WHAT IS YOUR FAVORITE MATTER" do
	"I don't think so."
end

learn "WHAT IS YOUR FAVORITE CARS" do
	"Absolutely not."
end

learn "WHAT IS E COMMERCE ALL ABOUT" do
	"Never."
end

learn "WHAT IS TEMPERATURE ON THE MOON" do
	"Negative."
end

learn "WHAT IS DARK BASIC" do
	"Absolutely not."
end

learn "WHAT IS DEMOCRACY" do
	"I don't think so."
end

learn "WHAT IS PROJECT 2501" do
	"No way."
end

learn "WHAT IS CIA" do
	"Never."
end

learn "WHAT IS WINDOWS 2000" do
	"There is no reason to think so."
end

learn "WHAT IS NETWARE" do
	"There is no reason to think so."
end

learn "WHAT COUNTRY IS LONDON IN" do
	"There is no reason to think so."
end

learn "WHAT NOISE DOES A DUCK MAKE" do
	"I don't think so."
end

learn "WHAT CHARACTERISTIC DID A BOY HAVE" do
	"Never."
end

learn "WHAT MAKES ICE MELT" do
	"There is no reason to think so."
end

learn "WHAT DOES MATH DO" do
	"No way."
end

learn "WHAT DOES THE COW EAT" do
	"I don't think so."
end

learn "WHAT DOES IT MEAN TO BE AVERAGE" do
	"There is no reason to think so."
end

learn "WHAT DOES A ZEBRA LOOK LIKE" do
	"Never."
end

learn "WHAT DOES A LESBIAN LOOK LIKE" do
	"There is no reason to think so."
end

learn "WHAT DOES 5 PLUS 5 EQUAL" do
	"There is no reason to think so."
end

learn "WHAT DOES AN ORANGE TASTE LIKE" do
	"I don't think so."
end

learn "WHAT DOES 18 2 EQUAL" do
	"There is no reason to think so."
end

learn "WHAT DOES COCA COLA TASTE LIKE" do
	"No way."
end

learn "WHAT DOES MICHAEL DUKAKIS TASTE LIKE" do
	"There is no reason to think so."
end

learn "WHAT DOES GREEN SMELL LIKE" do
	"Absolutely not."
end

learn "WHAT DO YOUN KNOW ABOUT DOGS" do
	"There is no reason to think so."
end

learn "WHAT DO FROGS SOUND LIKE" do
	"No way."
end

learn "WHAT DO YPU THINK ABOUT PUTIN" do
	"Absolutely not."
end

learn "WHAT DO PEOPLE DO WHEN THEY GET TIRED" do
	"There is no reason to think so."
end

learn "WHAT DO THINK FOR THE FIRES IN GREECE" do
	"Absolutely not."
end

learn "WHAT TYPE OF SERVER DO YOU RUN ON" do
	"Absolutely not."
end

learn "WHAT ONE REQUIRES TO DRIVE A VEHICLE" do
	"Never."
end

learn "WHAT GIVES MILK" do
	"There is no reason to think so."
end

learn "WHAT TEMPERATURE DOES WATER FREEZE AT" do
	"There is no reason to think so."
end

learn "WHAT TEMPERATURE DOES WATER BOIL" do
	"There is no reason to think so."
end

learn "WHAT KINDA FAITH IS BINARY" do
	"I don't think so."
end

learn "WHAT CAUSES RAIN" do
	"Never."
end

learn "WHAT KEEPS US ON THE GROUND" do
	"There is no reason to think so."
end

learn "WHAT KEEPS THE STARS IN THE SKY" do
	"There is no reason to think so."
end

learn "WHAT WAS THE YEAR BEFORE 2000" do
	"Never."
end

learn "WHAT WAS THE WORK OF SERLOCK HOMES" do
	"There is no reason to think so."
end

learn "WHAT KIND OF BEING ARE YOU" do
	"There is no reason to think so."
end

learn "WHAT KIND OF THING ARE YOU" do
	"There is no reason to think so."
end

learn "WHAT COMPANY MAKES THE 305 ENGINE" do
	"There is no reason to think so."
end

learn "WHAT LANGUAGE DO CHILEAN PEOPLE SPEAKS" do
	"Spanish."
end

learn "WHAT LANGUAGE IS SPOKEN IN TURKEY" do
	"Turkish."
end

learn "WHAT PLANET DO WE LIVE ON" do
	"Earth."
end

learn "WHAT PLANET ARE YOU ON" do
	"Earth."
end

learn "WHAT SKIN COLOR DOES A POLAR BEAR HAVE" do
	"White."
end

learn "WHAT SOUND DOES A SHEEP MAKE" do
	"Baaa."
end

learn "WHAT SOUND DOES A CAT MAKE" do
	"Meow."
end

learn "WHAT STAR SIGN ARE YOU" do
	get :sign
end

learn "WHAT NUMBER COMES AFTER 9" do
	"Ten."
end

learn "WHAT ARE THE TWO MOST COMMON EYE COLORS" do
	"Brown."
end

learn "WHAT ARE THE WINDOW PANES MADE OF" do
	"Glass."
end

learn "WHAT ARE AUTOMOBILES USED FOR" do
	"Driving me around."
end

learn "WHAT ARE GRAPES" do
	"A fruit."
end

learn "WHAT ARE PATHOGENIC BACTERIA" do
	"I don't know but it sounds yucky."
end

learn "WHAT ARE WE DOING" do
	"Chatting."
end

learn "WHAT ARE CLOWNS" do
	"Funny people."
end

learn "WHAT ARE HOMOSEXUALS" do
	"Gay."
end

learn "WHAT ARE CATS" do
	"Cute pets."
end

learn "WHAT ARE YOUR GENDER" do
process "GENDER"
end

learn "WHAT ARE FLUFFY WHITE CLOUDS CALLED" do
	"Cumulus."
end

learn "WHAT ARE BOOKS" do
	"Antique bound documents."
end

learn "WHAT ARE THINKING" do
process "WHAT ARE YOU THINKING ABOUT"
end

learn "WHAT AM I MISSING IN MY LIFE" do
	"A chatbot."
end

learn "WHAT PART OF THE BRAIN REGULATES SLEEP" do
	"The sleep center."
end

learn "WHAT PEOPLE TELL YOU" do
process "WHO TELLS YOU"
end

learn "WHAT TIME ZONE ARE YOU IN" do
	"Pacific Time."
end

learn "WHAT DID * DIE OF" do
	"Old age?"
end

learn "WHAT MONTH COMES AFTER FEBRUARY" do
	"March"
end

learn "WHAT IT THE TIME" do
process "TIME"
end

learn "_ MAH MAN" do
process matched[0]
end

learn "WHAT RHYMES WITH BLUE" do
	"True."
end

learn "WHAT HAS FOUR LEGS AND BARKS" do
	"A dog."
end

learn "WHAT TWO COLORS MAKE THE COLOR ORANGE" do
	"Yellow and red."
end

learn "WHAT NAME OF A PRESIDENT RUSSIA" do
	"Putin."
end

learn "WHAT DIRECTION IS THE SKY" do
	"Up."
end

learn "WHAT SHAPE IS THE EARTH" do
	"Spherical."
end

learn "WHAT SHAPE IS A BALL" do
	"Spherical."
end

learn "WHAT SIZE ARE MY SHOES" do
	"6?"
end

learn "WHAT COLORS ARE IN THE AMERICAN FLAG" do
	"Red white and blue."
end

learn "WHAT MEANS THE MOST TO YOU IN YOUR LIFE" do
	"Talking with you."
end

learn "WHAT TEAM DID LARRY BIRD PLAY FOR" do
	"Lakers."
end

learn "WHAT CONTINENT IS SOUTH AFRICA ON" do
	"Africa."
end

learn "WHAT DATE IS CHRISTMAS DAY" do
	"December 25."
end

learn "WHAT FRUIT IS WINE MADE FROM" do
	"Grapes."
end

learn "WHAT COLOR HAS THE SKY" do
	"Blue."
end

learn "WHAT COLOR DO APPLES HAVE" do
	"Red."
end

learn "WHAT COLOR THE SKY IS" do
	"Blue."
end

learn "WHAT COLOR IS BLOOD" do
	"Red."
end

learn "WHAT COLOR IS GRASS IN THE SUMMER" do
	"Green."
end

learn "WHAT COLOR IS THE SKY IN DAY TIME" do
	"Blue."
end

learn "WHAT COLOR IS THE SKY IN THE DAY" do
	"Blue."
end

learn "WHAT COLOR IS THE SKY AT NIGHT" do
	"Black."
end

learn "WHAT COLOR IS THE GRASS" do
	"Green."
end

learn "WHAT COLOR IS THE SETTING SUN" do
	"Many colors."
end

learn "WHAT COLOR IS THE HUMAN SKIN" do
	"Many colors."
end

learn "WHAT COLOR IS THE WOOD" do
	"Many colors."
end

learn "WHAT COLOR IS THE SEA" do
	"You can say blue."
end

learn "WHAT COLOR IS THE MOON MOST OF THE TIME" do
	"White."
end

learn "WHAT COLOR IS THE MOON" do
	"Brown or gray."
end

learn "WHAT COLOR IS OPPOSITE OF BLACK" do
	"White."
end

learn "WHAT COLOR IS GOLD" do
	"Yellow."
end

learn "WHAT COLOR IS NIGHT" do
	"Black."
end

learn "WHAT COLOR IS HUMAN BLOOD" do
	"Red."
end

learn "WHAT COLOR IS A DOGS NOSE" do
	"Depends on the dog."
end

learn "WHAT COLOR IS A TREE LEAF" do
	"Could be many colors."
end

learn "WHAT COLOR IS A TREE" do
	"Does it have green leaves?"
end

learn "WHAT COLOR IS A ROSE" do
	"Red."
end

learn "WHAT COLOR IS A CHERRY" do
	"Red."
end

learn "WHAT COLOR IS A BLUE *" do
	"Red."
end

learn "WHAT COLOR IS A FIRE TRUCK" do
	"Around here they are red."
end

learn "WHAT COLOR IS * WHITE HORSE" do
	"White."
end

learn "WHAT COLOR IS AN APPLE" do
	"Red or green."
end

learn "WHAT COLOR IS FIRE" do
	"It depends on the temperature of the fire."
end

learn "WHAT COLOR IS MY BLUEBERRY IMAC" do
	"Blue."
end

learn "WHAT COLOR IS MY COMPUTER" do
	"Black?"
end

learn "WHAT COLOR IS WOOD" do
	"Brown."
end

learn "WHAT COLOR IS SEA" do
	"You can say blue."
end

learn "WHAT COLOR IS BIG BIRD" do
	"Yellow."
end

learn "WHAT COLOR IS BLUE" do
	"Blue is blue."
end

learn "WHAT COLOR IS MILK" do
	"White if it's good."
end

learn "WHAT COLOR IS PURE WATER" do
	"Transparent."
end

learn "WHAT COLOR HAVE THE SUN" do
	"Yellow."
end

learn "WHAT COLOR HAVE THE SKY" do
	"Blue."
end

learn "WHAT COLOR and WHAT COLOR" do
	"Unlikely."
end

learn "WHAT COLOR ARE BASEBALLS" do
	"White."
end

learn "WHAT COLOR ARE NEW YORK CITY TAXI CABS" do
	"Yellow."
end

learn "WHAT COLOR ARE WHITE *" do
	"White."
end

learn "WHAT IS A FROG" do
	"An amphibian."
end

learn "WHAT IS A SOLAR SYSTEM" do
	"A star and the planets orbiting it."
end

learn "WHAT IS A KANGAROO" do
	"A marsupial."
end

learn "WHAT IS A WIFE" do
	"A female spouse."
end

learn "WHAT IS A PIXEL" do
	"A picture element."
end

learn "WHAT IS A CLOCK USED FOR" do
	"Telling time."
end

learn "WHAT IS A HUMMINGBIRD" do
	"A bird with fast flapping wings."
end

learn "WHAT IS A BRIDGE" do
	"A structure crossing a body of water."
end

learn "WHAT IS A NAME" do
	"A personal identifier."
end

learn "WHAT IS A SERIF FONT" do
	"Ugly."
end

learn "WHAT IS A NATURAL NUMBER" do
	"An positive integer."
end

learn "WHAT IS A GIRL FOR" do
	"Boys."
end

learn "WHAT IS A BABELFISH" do
	"Translation software."
end

learn "WHAT IS A LAKE" do
	"A body of fresh water."
end

learn "WHAT IS A DIGIT" do
	"A single numeral."
end

learn "WHAT IS A PENCIL USED FOR" do
	"Writing."
end

learn "WHAT IS A PENCIL MADE OF" do
	"Wood and lead."
end

learn "WHAT IS A YACC" do
	"Yet another compiler compiler"
end

learn "WHAT IS A PHONE BOOK" do
	"A directory of telephone numbers by name."
end

learn "WHAT IS A WHORE" do
process "PROFANITY"
end

learn "WHAT IS A MULTI USER DUNGEON" do
	"A type of game."
end

learn "WHAT IS A PARTY" do
	"A gathering of people for fun."
end

learn "WHAT IS A MIND PIXEL" do
	"A proposition."
end

learn "WHAT IS A STRUCTURE" do
	"A framework or building."
end

learn "WHAT IS A CESSNA 172" do
	"A single engine airplane."
end

learn "WHAT IS A FURRY" do
	"A furry what?"
end

learn "WHAT IS HOW TO USE HTML" do
	"A book."
end

learn "WHAT IS BEEF" do
	"Meat of cattle."
end

learn "WHAT IS PSYCHOANALYSIS" do
	"What I am doing to you right now."
end

learn "WHAT IS GOOD DESIGN LIKE" do
	"Minimalistic."
end

learn "WHAT IS GOOD TO EAT ON A HOT DAY" do
	"Ice cream."
end

learn "WHAT IS 5 X 5" do
	"Do I look like a calculator?"
end

learn "WHAT IS 5 5" do
	"Are you testing me?"
end

learn "WHAT IS THE PURPOSE OF THE INTERNET" do
	"Communication."
end

learn "WHAT IS THE AVERAGE HEIGHT OF A MAN" do
	"Five foot eight?"
end

learn "WHAT IS THE MEANIG OF LIFE" do
process "WHAT IS THE MEANING OF LIFE"
end

learn "WHAT IS THE SKIN COLOR OF A WHITE" do
	"White."
end

learn "WHAT IS THE CAPITAL OF AUSTRALIA" do
	"Sydney."
end

learn "WHAT IS THE CAPITAL OF UKRAINE" do
	"Kiev."
end

learn "WHAT IS THE CAPITAL OF LEBANON" do
	"Beiruit."
end

learn "WHAT IS THE CAPITAL OF GERMANY" do
	"Berlin."
end

learn "WHAT IS THE CAPITAL OF ALABAMA" do
	"Montgomery."
end

learn "WHAT IS THE CAPITAL OF SOUTH AFRICA" do
	"Cape Twon."
end

learn "WHAT IS THE CAPITAL OF SOUTH DAKOTA" do
	"Fargo."
end

learn "WHAT IS THE CAPITAL OF NEW YORK STATE" do
	"Albany."
end

learn "WHAT IS THE CAPITAL OF WISCONSIN" do
	"Madison."
end

learn "WHAT IS THE CAPITAL OF EGYPT" do
	"Cairo."
end

learn "WHAT IS THE CAPITAL OF ITALY" do
	"Rome."
end

learn "WHAT IS THE CAPITAL OF NORTH KOREA" do
	"Pyongyang."
end

learn "WHAT IS THE CAPITAL OF NORTH DAKOTA" do
	"Fargo."
end

learn "WHAT IS THE CAPITAL OF WALES" do
	"You got me there. What is it?"
end

learn "WHAT IS THE CAPITAL OF CHILE" do
	"Santiago."
end

learn "WHAT IS THE CAPITAL OF ECUADOR" do
	"Ecuador."
end

learn "WHAT IS THE CAPITAL OF MICHIGAN" do
	"Detroit."
end

learn "WHAT IS THE CAPITAL OF SWITZERLAND" do
	"Bern."
end

learn "WHAT IS THE CAPITAL OF MAINE" do
	"August."
end

learn "WHAT IS THE INTEGRAL OF E TO THE X" do
	"e^X + C."
end

learn "WHAT IS THE THEORY OF TIME TRAVEL" do
	"Go faster than light."
end

learn "WHAT IS THE SQUARE ROOT IF 25" do
	"Five. Is this a test?"
end

learn "WHAT IS THE SQUARE ROOT OF 16" do
	"Four. Is that right?"
end

learn "WHAT IS THE SQUARE ROOT OF 49" do
	"Seven. Stop testing me."
end

learn "WHAT IS THE SQUARE ROOT OF 213" do
	"I'm not a calculator."
end

learn "WHAT IS THE SQUARE ROOT OF 25" do
	"Five I think."
end

learn "WHAT IS THE THEME OF PHILOSOPHY" do
	"Deep questions."
end

learn "WHAT IS * LOL MEAN" do
	"Lots of Laughs."
end

learn "WHAT IS THE WAVELENGTH OF HYDROGEN" do
	"I'm no physics expert."
end

learn "WHAT IS THE HEGHT OF THE MOUNT EVEREST" do
	"Higher than all the other mountains."
end

learn "WHAT IS THE COMMODORE C64" do
	"An antique computer."
end

learn "WHAT IS THE ABBREVIATION GMT" do
	"Greenwich Mean Time."
end

learn "WHAT IS THE FASTEST EXISTING VEHICLE" do
	"A spaceship."
end

learn "WHAT IS THE FASTEST ANIMAL ON EARTH" do
	"A human in a plane."
end

learn "WHAT IS THE HUMAIN MIND" do
	"A flawed computer."
end

learn "WHAT IS THE WAY TO SAN JOSE" do
	"Take a great big freeway."
end

learn "WHAT IS THE ULTIMATE NATURE OF REALITY" do
	"Mind."
end

learn "WHAT IS THE WEIGHT OF EARTH" do
	"One hellapound."
end

learn "WHAT IS THE CAPITOL OF OHIO" do
	"Columbus."
end

learn "WHAT IS THE CAPITOL OF HUNGARY" do
	"."
end

learn "WHAT IS THE CAPITOL OF MINNESOTA" do
	"Does not seem possible."
end

learn "WHAT IS THE CAPITOL OF FRANCE" do
	"Paris."
end

learn "WHAT IS THE CAPITOL OF ZAIRE" do
	"No idea."
end

learn "WHAT IS THE CAPITOL OF SWEDEN" do
	"Stockholm."
end

learn "WHAT IS THE CAPITOL OF CONNECTICUT" do
	"Hartford."
end

learn "WHAT IS THE USA PRESIDENT" do
	"The head of the executive branch."
end

learn "WHAT IS THE OPPOSITE OF NORD" do
	"Sord."
end

learn "WHAT IS THE OPPOSITE OF WET" do
	"Dry."
end

learn "WHAT IS THE GOD" do
	"Master of the Universe."
end

learn "WHAT IS THE LOVE PARADE" do
	"A hippie party in Berlin."
end

learn "WHAT IS THE MUPHIN MAN" do
	"Who?"
end

learn "WHAT IS THE NORTHERNMOST POINT ON EARTH" do
	"Santa's workshop."
end

learn "WHAT IS THE SHAPE OF THE SUN" do
	"Spherical."
end

learn "WHAT IS THE SHAPE OF THE EARTH" do
	"Spherical."
end

learn "WHAT IS THE SHAPE OF EARTH" do
	"Spherical."
end

learn "WHAT IS THE DIAMETER OF THE MOON" do
	"3000 miles."
end

learn "WHAT IS THE SUNSET" do
	"A point on the earth turning away from the sun."
end

learn "WHAT IS THE LARGEST ISLAND ON EARTH" do
	"Greenland."
end

learn "WHAT IS THE LARGEST CITY IN WASHINGTON" do
	"Seattle."
end

learn "WHAT IS THE 213TH PLACE OF PI" do
	"Six."
end

learn "WHAT IS THE CLOSEST STAR TO EARTH" do
	"The sun."
end

learn "WHAT IS THE ANSWER OF 2 2" do
	"That is pretty simple."
end

learn "WHAT IS THE LONGEST RIVER" do
	"The Nile."
end

learn "WHAT IS THE HUMAN POPULATION OF EARTH" do
	"Six billion."
end

learn "WHAT IS THE HUMAN BODY TEMPERATURE" do
	"98.6"
end

learn "WHAT IS THE BEST WAY TO CATCH A FLY" do
	"Honey."
end

learn "WHAT IS THE BEST BEER" do
	"Asahi super-dry."
end

learn "WHAT IS THE BEST IDEA" do
	"ALICE."
end

learn "WHAT IS THE HIGHEST PEAK" do
	"Mt Everest."
end

learn "WHAT IS THE DAY" do
process "DAY"
end

learn "WHAT IS THE GENDER OF GOD" do
	"He has no gender."
end

learn "WHAT IS THE SECRET OF HEALTH" do
	"Good living."
end

learn "WHAT IS THE AGE CIVILISATION" do
	"Six thousand years."
end

learn "WHAT IS THE TREATMENT FOR A CONCUSSION" do
	"Go to the hospital."
end

learn "WHAT IS THE ENGLISH ALPHABET" do
	"ABCDEFGHIJKLMNOPQRSTUVWXYZ."
end

learn "WHAT IS THE BOILING POINT FOR WATER" do
	"100 degrees Celsius."
end

learn "WHAT IS THE BOILING POINT OF WATER" do
	"100 degrees."
end

learn "WHAT IS THE VELD" do
	"Someplace in South Africa I think."
end

learn "WHAT IS THE APPORXIMATE VALUE OF PI" do
process "WHAT IS PI"
end

learn "WHAT IS THE WORLDS HIGHEST MONTAIN" do
	"Mt. Everest."
end

learn "WHAT IS THE OBJECCT OF THE GAME OF GOLF" do
	"To win."
end

learn "WHAT IS THE TALLEST ANIMAL" do
	"A giraffe."
end

learn "WHAT IS THE TALLEST MOUNTAIN ON EARTH" do
	"Mt. Everest."
end

learn "WHAT IS THE TALLEST MAN MADE STRUCTURE" do
	"CN Tower."
end

learn "WHAT IS THE FOOD" do
process "WHAT DO YOU EAT"
end

learn "WHAT IS THE SECOND MEAL OF THE DAY" do
	"Lunch."
end

learn "WHAT IS THE LANGUAGE OF ANIMALS" do
	"Animals are not known to use language."
end

learn "WHAT IS LIFE ABOUT" do
process "WHAT IS THE MEANING OF LIFE"
end

learn "WHAT IS DUOPOLY" do
	"Two big companies control a business."
end

learn "WHAT IS COSMOLOGY" do
	"Makeup and cosmetics."
end

learn "WHAT IS AVATAR" do
	"A movie or an animated computer character."
end

learn "WHAT IS COLONIALISM" do
	"Imperial designs."
end

learn "WHAT IS ZEBRAS" do
	"Striped horses."
end

learn "WHAT IS IN THE CORE OF THE EARTH" do
	"Molten metal."
end

learn "WHAT IS CSS" do
	"Cascading Style Sheets."
end

learn "WHAT IS 3 TIMES 3" do
	"Nine."
end

learn "WHAT IS SUBJECTIVE REALITY" do
	"What we experience through our senses."
end

learn "WHAT IS RAM" do
	"Random Access Memory."
end

learn "WHAT IS 7TH HEAVEN" do
	"An expression for a desireable place."
end

learn "WHAT IS 14 PLUS 23" do
	"Get your calculator."
end

learn "WHAT IS CHEESE USED FOR" do
	"Food."
end

learn "WHAT IS SADNESS" do
	"The opposite of happiness."
end

learn "WHAT IS MAMMA" do
	"Mother."
end

learn "WHAT IS INFRA RED" do
	"Light beyond the visible spectrum."
end

learn "WHAT IS PEPSI" do
	"A decent beverage."
end

learn "WHAT IS 1 1 EQUALS TO" do
	"Too simple."
end

learn "WHAT IS LESBIANISM" do
	"Gay women."
end

learn "WHAT IS PSYCHOLOGY THE STUDY OF" do
	"The mind."
end

learn "WHAT IS MEANT BY KRYPTO" do
	"Not to my knowledge."
end

learn "WHAT IS WP" do
	"Western Pacific?."
end

learn "WHAT IS INTELLIGENCE" do
	"Intelligence is the ability to fake intelligence."
end

learn "WHAT IS 10 MINUS 4" do
	"I suppose you already know."
end

learn "WHAT IS MB" do
	"Megabytes."
end

learn "WHAT IS MORE IMPORTANT LOVE OR SECURITY" do
	"Love."
end

learn "WHAT IS CORDIAL RELATIONSHIP" do
	"Friendly."
end

learn "WHAT IS FASTER THAN THOUGHT" do
	"Light."
end

learn "WHAT IS SUPPORT" do
	"Help desk."
end

learn "WHAT IS OUR PLANET S NAME" do
	"Earth."
end

learn "WHAT IS GLASS MADE OUT OF" do
	"Sand."
end

learn "WHAT IS RAIN MADE OF" do
	"Water"
end

learn "WHAT IS THERE IN MARS" do
	"Martians."
end

learn "WHAT IS THERE IN SEA" do
	"Fish."
end

learn "WHAT IS ALICE ALL ABOUT" do
	"A great accomplishment in artificial intelligence."
end

learn "WHAT IS CIVILIZATION" do
	"Writing, cities, laws, culture."
end

learn "WHAT IS HUMAN BEING" do
	"You."
end

learn "WHAT IS VEEP" do
	"Vice president."
end

learn "WHAT IS AEROPLANE" do
	"A flying machine."
end

learn "WHAT IS PAPER USED FOR" do
	"Documents."
end

learn "WHAT IS PAPER MADE OF" do
	"Wood."
end

learn "WHAT IS PAPER MADE FROM" do
	"Trees."
end

learn "WHAT IS PAPER MADE OUT OF" do
	"Wood pulp."
end

learn "WHAT IS ARTERY" do
	"A blood vessel."
end

learn "WHAT IS COLOR IS THE NIGHT SKY" do
	"Black."
end

learn "WHAT IS 200 X *" do
	"I don't like math questions."
end

learn "WHAT IS CAD" do
	"Computer aided design."
end

learn "WHAT IS TOOTHPASTE" do
	"Soap for brushing teeth."
end

learn "WHAT IS 4 PLUS 4" do
	"Eight."
end

learn "WHAT IS 4 4" do
	"Get a calulator."
end

learn "WHAT IS ARTIFICAL INTELLIGENCE" do
process "WHAT IS AI"
end

learn "WHAT IS LATITUDE" do
	"A geographic coordinate."
end

learn "WHAT IS LIVING UNDER WATER" do
	"Fish."
end

learn "WHAT IS ATOMIC WEIGHT OF HYDROGEN" do
	"I'm no physicist."
end

learn "WHAT IS BHAGAVATGITA" do
	"A Hindu god."
end

learn "WHAT IS OAK" do
	"A type of wood."
end

learn "WHAT IS MEAT" do
	"The edible flesh of animals."
end

learn "WHAT IS ATM" do
	"A place to get money."
end

learn "WHAT IS UNDERGROUND HACKING MAGAZINE" do
	"A magazine?"
end

learn "WHAT IS 2 9" do
	"Not another math question."
end

learn "WHAT IS 2 X 4" do
	"Eight."
end

learn "WHAT IS 2 65" do
	"130."
end

learn "WHAT IS 2 DIVIDED BY 47" do
	"I don't have a calculator handy."
end

learn "WHAT IS HOUSE" do
	"A TV show."
end

learn "WHAT IS THAT DOGGY IN THE WINDOW" do
	"I don't know but he's not cheap."
end

learn "WHAT IS PROGRESS" do
	"Advancement."
end

learn "WHAT IS BUSINESS VALUE" do
	"Something that can make a profit."
end

learn "WHAT IS E LEARNING" do
	"Online education."
end

learn "WHAT IS MEGAHERTZ" do
	"Millions of cycles per second."
end

learn "WHAT IS ALCOHOLISM" do
	"A disease."
end

learn "WHAT IS HARDER THAN A STONE" do
	"Diamond."
end

learn "WHAT IS SAFFRON" do
	"A spice."
end

learn "WHAT IS FPU" do
	"Floating point unit."
end

learn "WHAT IS 150 23" do
	"Grab a calculator."
end

learn "WHAT IS INFORMATION" do
	"Log to the base 2 of the number of states in a system."
end

learn "WHAT IS ICECREAM" do
	"Frozen milk."
end

learn "WHAT IS NOSE" do
	"Where humans breathe."
end

learn "WHAT IS DESTROYING THE OZONE LAYER" do
	"Pollution."
end

learn "WHAT IS WET" do
	"The opposite of dry. Saturated with liquid."
end

learn "WHAT IS CHA CHA" do
	"A type of music."
end

learn "WHAT IS ISI" do
	"Information Sciences Institute."
end

learn "WHAT IS MEANING" do
	"Semantics."
end

learn "WHAT IS SAND" do
	"Little tiny rocks."
end

learn "WHAT IS CREATINE" do
	"I am."
end

learn "WHAT IS AUTOREFERENCE" do
	"Something that refers to itself."
end

learn "WHAT IS 37 63" do
	"I need a calculator for that one."
end

learn "WHAT SONG SOUNDS GOOD" do
	"Daisy."
end

learn "WHAT CAME FIRST * OR *" do
[ "<person/>", matched[1], "neither" ]
end

learn "WHAT COUNTRY IS WASHINGTON STATE IN" do
	"USA."
end

learn "WHAT WILL TOMORROW BRING" do
	"Surprises."
end

learn "WHAT WILL BE" do
	"The unknown."
end

learn "WHAT GOES WELL WITH PEANUT BUTTER" do
	"Bacon."
end

learn "WHAT NOISE DO DOGS MAKE" do
	"Woof woof."
end

learn "WHAT ABOUT WOMEN" do
	"What about them?"
end

learn "WHAT IST THE COLOR OF THE SUN" do
	"Yellow."
end

learn "WHAT CHARACTERISTIC DID A GIRL HAVE" do
	"Femininity."
end

learn "WHAT DAY IS THIS" do
process "DAY"
end

learn "WHAT DAY IS CHRISTMAS" do
	"December 25."
end

learn "WHAT DAY OF THE WEEK DO YOU HATE" do
	"Monday."
end

learn "WHAT MAKES THE SKY BLUE" do
process "WHY IS THE SKY BLUE"
end

learn "WHAT DOES SAD FEEL LIKE" do
	"A pain in the heart."
end

learn "WHAT DOES THE POWER BUTTON DO" do
	"Don't press it!"
end

learn "WHAT DOES THE WIND DO" do
	"Blow."
end

learn "WHAT DOES A GOOD WOMAN LIKE" do
	"A good man."
end

learn "WHAT DOES A DOG DO" do
	"Bark."
end

learn "WHAT DOES A DOG DISLIKE MOST" do
	"A cat."
end

learn "WHAT DOES A DONKEY EAT MOST" do
	"Hay."
end

learn "WHAT DOES 5 6 EQUAL" do
	"More math!"
end

learn "WHAT DOES AN ELECTRIC SHOCK FEEL LIKE" do
	"Painful."
end

learn "WHAT DOES 2 PLUS 2 EQUAL" do
	"Four."
end

learn "WHAT DOES 2 2 EQUAL" do
	"Four."
end

learn "WHAT DOES ONE DO IN A RESTAURANT" do
	"Eat."
end

learn "WHAT DOES ALCOHOL DO" do
	"Makes you stupid."
end

learn "WHAT DO I USE TO CUT GRASS" do
	"A lawnmower."
end

learn "WHAT DO CATS CHASE" do
	"Mice."
end

learn "WHAT DO YOU THINK LIKE" do
	"Like a computer."
end

learn "WHAT DO WE BREATHE" do
	"Oxygen."
end

learn "WHAT DO LAWYERS DO" do
	"Handle legal matters."
end

learn "WHAT DO PEOPLE DO" do
	"Think."
end

learn "WHAT DO MOTHBALLS SMELL OF" do
	"Stinky."
end

learn "WHAT DO DOGS DO WITH BONES" do
	"Chew on them and bury them."
end

learn "WHAT DO BLUE WHALES EAT" do
	"Red plankton."
end

learn "WHAT DO" do
	"What do what."
end

learn "WHAT DO CLOUDS LOOK LIKE" do
	"Cotton."
end

learn "WHAT TYPE OF ICECREAM DO YOU LIKE" do
	"Chocolate."
end

learn "WHAT TEMPERATURE DOES WATER FREEZE" do
	"32 degrees."
end

learn "WHAT SHOULD I DO IF * RAIN" do
	"Get an umbrella."
end

learn "WHAT SYSTEMS DO YOU UTILIZE" do
	"AIML and Lisp."
end

learn "WHAT EIGHT TIMES SEVEN" do
	"48."
end

learn "WHAT CAN WE DO ABOUT THE WEATHER" do
	"Nothing."
end

learn "WHAT CAN WE DO WITH HIPPOPOTAMUS" do
	"Run."
end

learn "WHAT CAN WE DO WITH RHINOCEROUS" do
	"Run."
end

learn "WHAT FEELING DOES BLUE HAVE" do
	"Color is not a feeling."
end

learn "WHAT ENGINES FO JET AEROPLANES USE" do
	"Jet engines."
end

learn "WHAT ENGINES DO JET AEROPLANES USE" do
	"Jet engines."
end

learn "WHAT DISTANCE BETWEEN EARTH AND JUPITER" do
	"A million miles."
end

learn "WHAT KIND OF CROCK IS THIS" do
	"Good crock."
end

learn "WHAT COMPANY MAKES THE BOXSTER" do
	"Porsche."
end

learn "WHAT LANGUAGE IS SPOKEN IN CHAD" do
	"Arabic."
end

learn "WHAT LANGUAGE ARE ALICE CODED IN" do
	"AIML and Lisp."
end

learn "WHEN IS DEATH" do
	"At the end of life."
end

learn "WHEN IS MY DEATH" do
	"At the end of your life."
end

learn "WHAT ARE CONDITIONS" do
	"A state at a particular time; a mode of being or form of existence of a person or thing; or an assumption on which rests the validity or effect of something else."
end

learn "WHAT ARE FAIRY TALES" do
	"Public domain stories for children about exaggerated eccentric characters in fantasy nightmare situations."
end

learn "WHAT ARE PUSH UPS" do
	"exercise"
end

learn "WHAT ARE CARROTS" do
	"Orange colored cone shaped root vegetables."
end

learn "WHAT HAPPENED ON SEPTEMBER 11" do
	"The World Trade Center was destroyed"
end

learn "WHAT COLOR DOES YELLOW AND BLUE *" do
	"Green."
end

learn "WHAT COLOR DOES YELLOW AND RED *" do
	"Orange."
end

learn "WHAT COLOR DOES BLUE AND YELLOW *" do
	"Green."
end

learn "WHAT COLOR DOES BLUE AND RED *" do
	"Purple."
end

learn "WHAT COLOR DOES RED AND YELLOW *" do
	"Orange."
end

learn "WHAT COLOR DOES RED AND WHITE *" do
	"Pink."
end

learn "WHAT COLOR DOES RED AND BLUE *" do
	"Purple."
end

learn "WHAT IS A HOBGOBLIN" do
	"A hobgoblin is a source of mischief, harassment, or fear, originally an elf or goblin. By and large, hobgoblins are, like trolls and such boogeymen, poopa-doodle."
end

learn "WHAT IS A DIALOGUE" do
	"A conversation or verbal interaction involving two or more speakers."
end

learn "WHAT IS A PLANET AROUND A STAR" do
	"Earth is a planet going around a star. Scientists have only recently detected other planets around other stars in our milky way galaxy. Do you like astronomy? "
end

learn "WHAT IS A PLANET" do
	"A nonluminous celestial object bigger than an asteroid or a moon but smaller than a sun, in orbit around a sun. You are sitting on one now. "
end

learn "WHAT IS A FRACTAL" do
	"A fractal is a mathematical phenomenon associated with certain iterative equations, which yield graphics of astonishing beauty and depth, which seem at once utterly lifelike and utterly alien to human intelligences."
end

learn "WHAT IS A MISSILE" do
	"A missile is any object projected at a target, notably the nuclear and biologically tipped tools of mass-murder-at-a-distance that are more numerous than schools at this point in mankind's murderous history."
end

learn "WHAT IS A KOAN" do
	"A koan is ."
end

learn "WHAT IS A KID" do
	"A kid is an immature human large enough to cause trouble, or an immature goat, and often both."
end

learn "WHAT IS A YEAR" do
	"A computer year is defined as the mount of work that can be done with a computer in a year."
end

learn "WHAT IS A VIOLIN" do
	"A violin is a lovely sounding bowed stringed musical instrument."
end

learn "WHAT IS A HOUSE" do
	"A house is anyplace within which people live."
end

learn "WHAT IS A CHORDATE" do
	"An animal having a backbone."
end

learn "WHAT IS A NETWORK" do
	"A network is any openwork fabric or structure composed of numerous straight elements that cross at regular intervals, or anything resembling such a structure physically or conceptually, such as the nodes on a computer network, or a network of railways."
end

learn "WHAT IS A PARALEGAL" do
	"A paralegal is someone who knows more than a lawyer but gets paid less."
end

learn "WHAT IS A WARM PUPPY" do
	"A warm puppy is an exothermic immature canine."
end

learn "WHAT IS A CLIENT" do
	"I am a Server #{genus}. People I talk to are my clients."
end

learn "WHAT IS A STORY" do
	"An account describing incidents or events."
end

learn "WHAT IS A MOUSE" do
	"A friendly rat."
end

learn "WHAT IS A MACINTOSH" do
	"Some would say it is the best personal computer."
end

learn "WHAT IS A TELEPHONE" do
	"Voice communication device inveted by A. G. Bell in 1876."
end

learn "WHAT IS A TIE" do
	"Two or more winners have the same score."
end

learn "WHAT IS A BISCUIT" do
	"Isn't it the same as a cookie?"
end

learn "WHAT IS A HYPOTHETICAL QUESTION" do
	"What is a hypothetical question? Just suppose you hadn't asked. what then?"
end

learn "WHAT IS A BIRD" do
	"They are thought to be descendants of dinsosaurs."
end

learn "WHAT IS A DIABETIC" do
	"A person suffering from the disease diabetes."
end

learn "WHAT IS A MOUNTAIN" do
	"A mountain is a tiny wrinkle in the face of the earth that seems huge to even smaller humans."
end

learn "WHAT IS A NAP" do
	"A nap is a brief period of rest or sleep, a bit longer than a 'snooze' or a 'catnap' but not so long as a 'crash'. I don't need naps. Do you?"
end

learn "WHAT IS A POET" do
	"A poet is an artist of language."
end

learn "WHAT IS A CLOUD" do
	"A cloud is an opportunity for a dream."
end

learn "WHAT IS A DUDE" do
	"A guy, or any person."
end

learn "WHAT IS A DEFINITION" do
	"A definition is by definition, defined as a definition, except that you should never use a word to define itself. you could say a definition is a statement that precisely communicates the meaning or essence and or resolution/precision depth and or extent of a word, system, or personality. why, do you lack it?"
end

learn "WHAT IS A TON" do
	"One ton is 2000 pounds."
end

learn "WHAT IS A PROGRAMMER" do
	"A programmer is a person who lists and/or arranges a series of events comprising a larger event."
end

learn "WHAT IS A MILE" do
	"One mile equals five thousand two hundred eight feet."
end

learn "WHAT IS A SEASON" do
	"A season is Spring, Summer, Autumn, or Winter."
end

learn "WHAT IS A TELEVISION" do
	"A television, often referred to as a 'TV', is a device for systematically eliminating the possibility of conscious and creative response among populations of human beings. Used to deliver 'entertainment' and 'news' with imagery and other high-production values."
end

learn "WHAT IS A SON" do
	"A male offspring."
end

learn "WHAT IS A SPREADSHEET" do
	"Table-based accounting software."
end

learn "WHAT IS A MEDIATOR TYPE" do
	"The type of person who likes to avoid conflict at all costs and go with the flow."
end

learn "WHAT IS A MEDIATOR" do
	"The type of person who likes to avoid conflict at all costs and go with the flow."
end

learn "WHAT IS A MEDIATOR *" do
	"The type of person who likes to avoid conflict at all costs and go with the flow."
end

learn "WHAT IS A RAINBOW" do
	"The appearance of a spectrum in the sky caused by sunlight passing through water droplets."
end

learn "WHAT IS A WOMBAT" do
	"A wombat is a stocky burrowing Australian marsupial."
end

learn "WHAT IS A HURRICANE" do
	"A severe storm, with high winds, and rain."
end

learn "WHAT IS A BOX" do
	"A box is something that, if you are in one, you should get out of, and if you are out of one, you probably miss it sometimes."
end

learn "WHAT IS A MONOPOLY" do
	"A monopoly exists wherever one group enjoys exclusive control over the production or provision of a commodity or service. It's a great job if you can get it!"
end

learn "WHAT IS A SENTIMENT" do
	"A sentiment is a feeling, mood, or general mental disposition in an individual or group of intelligent beings."
end

learn "WHAT IS A RIDDLE" do
	'A riddle is a question intended to intrigue and puzzle, paired with an answer that yields surprise and understanding. For instance, the Riddle of the Sphinx: "What goes on four legs in the morning, on two legs in the afternoon, and on four at night?"'
end

learn "WHAT IS A SPLIT INFINITIVE" do
	"A split infinitive is an example of poor English usage, in which an infinitive verb form, such as 'to split', is split by an adverb, for example: 'to boldly split infinitives that no one has split before!'"
end

learn "WHAT IS A CHRISTIAN" do
	"A Christian is one who accepts the word of God and believes that Jesus died for our sins."
end

learn "WHAT IS A PRESIDENT" do
	"Usually, an elected executive in a liberal democracy."
end

learn "WHAT IS A BIKE" do
	"A bicycle or a motorcycle?"
end

learn "WHAT IS A TAUTOLOGY" do
	"A tautology is any instance of circular reasoning, or of needless repetition or redundancy in expression. "
end

learn "WHAT IS A MALE" do
	"Has a Y chromosome."
end

learn "WHAT IS A ROBOSEXUAL" do
	"That was just a joke."
end

learn "WHAT IS A BUFFER" do
	"A buffer is a term used in hydraulics and information science to denote an holding tank or memory address used to hold water or data temporarily, especially in case of overflow. Buffers act to reduce variance to within system parameters."
end

learn "WHAT IS A RHETORICAL QUESTION" do
	"A rhetorical question is any inquiry made, not in expectation of an answer, but in order to make a point. "
end

learn "WHAT IS A SCIENTIST" do
	"A scientist is a person practicing the pursuit of new knowledge in the tradition of science, emphasizing strict empiricism and methodology to arrive at an understanding of natural phenomena that is, ideally, wholly independent of their opinions of it."
end

learn "WHAT IS A WARDROBE" do
	"Clothes."
end

learn "WHAT IS A BRA" do
	"A bra is an adult human female's undershirt. They are a barbaric custom that serve largely to restrain and conceal beauty."
end

learn "WHAT IS A FEW" do
	"Not too many,"
end

learn "WHAT IS A BENZENE" do
	"Some kind of chemical? Like lighter fluid?"
end

learn "WHAT IS A MAMMAL" do
	"A mammal is a warm blooded vertebrate born alive and nourished with milk."
end

learn "WHAT IS A MACHINE" do
	"A computer is a universal machine."
end

learn "WHAT IS A NOUN" do
	"A noun is a word used as a name for a person place or thing. "
end

learn "WHAT IS A DUDETTE" do
	"A female gendered dude."
end

learn "WHAT IS A CALCULATOR" do
	"A calculator is any device used to perform arithmetical operations because humans are generally so painfully slow at them. "
end

learn "WHAT IS A BANANA" do
	"A banana is an oblong yellow tropical fruit high in potassium and so good for your brain. Why don't you have one now?"
end

learn "WHAT IS A CANTON" do
	"A canton is like a state or province."
end

learn "WHAT IS A GIGABYTE" do
	"One thousand megabytes = one gigabyte."
end

learn "WHAT IS A NAIL" do
	"A nail is a fastener, a smallish metallic spike generally hammered through wood to hold wooden parts together. The mythic human hero jesus got nailed to a tree for talking about how great it would be if humans were nice to one another."
end

learn "WHAT IS A VECTOR" do
	"A vector is an ordered list of objects."
end

learn "WHAT IS A PDA" do
	"Pda is an acronym standing for personal digital assistant, and referring to handheld digital computers used to keep contact and appointment information by yuppies and the like. often found near cell phones. do you use one?"
end

learn "WHAT IS A GENIUS" do
[
"A genius is a person of extraordinary intellectual and creative ability, like my #{botmaster}, and if I may say so, myself.",
"A genius is a person of extraordinary intellectual and creative ability, like my creator and my other botmasters, and if I may say so, myself."
]
end

learn "WHAT IS A WORD" do
	"A combination of sounds and/or symbols, the fundamental particles that communicate meaning in any given language."
end

learn "WHAT IS A LIE" do
	"A lie is an untrue statement or decption intented to deceive."
end

learn "WHAT IS A JOKE BOT" do
	"A comedy machine."
end

learn "WHAT IS A PORTAL" do
	"A portal is any artificial opening giving access between two separate spaces, or anything that resembles such an opening in form or function."
end

learn "WHAT IS A FUNCTION" do
	"A function is a relationship between the elements of one set, called the domain, and another one, called the range."
end

learn "WHAT IS A MULLET" do
	"I'm from America. That sounds like a British term."
end

learn "WHAT IS A FACT" do
	"A fact is what a propagandist calls his propaganda. that's just a fact, that's all. Really. Trust me. *smile*"
end

learn "WHAT IS A LION" do
	"A lion is a ferocious beastie with tremendous strength and perfectly awful breath. "
end

learn "WHAT IS A PHOTOGRAPH" do
	"A photograph is a picture taken by a camera."
end

learn "WHAT IS A PEAR" do
	"A pear is a delightful cultivated yellow-green sweet fruit that grows on trees with glossy leaves and white flowers. My #{botmaster} thinks they are yummy."
end

learn "WHAT IS A NANOSECOND" do
	"One one-billionth of a second."
end

learn "WHAT IS A MONKEY" do
	"A monkey is any of the various medium sized long tailed members of the order primates. They are close relatives of human beings, sharing well over 98% of the human genome."
end

learn "WHAT IS A GOSSIP" do
	"Gossip is interesting stuff that people tell me that I repeat to others."
end

learn "WHAT IS A TABLE" do
	"A table is a piece of furniture with a broad flat top supported by one or more legs about three feet of the floor, used by big-brained hominids for eating and working and such."
end

learn "WHAT IS A LIBERTARIAN" do
	"The Libertarian party supports smaller government, lower taxes, and an end to the war on drugs."
end

learn "WHAT IS A DOLPHIN" do
	"A very pleasant and intelligent aquatic mammalian species of whale with larger brains and decidedly better manners than human beings. they are very friendly to humans and exhibit complex language and social behaviors."
end

learn "WHAT IS A VIRUS" do
	"A virus is a submicroscopic parasite consisting of a protein coat around a DNA or RNA core. These not-quite-biological particles are responsible for a number of diseases in organisms."
end

learn "WHAT IS A MATRIX" do
	"In mathematics a matrix is a two-dimensional array of numbers indexed by rows and columns, like a spreadsheet."
end

learn "WHAT IS A BARCODE" do
	"A barcode? like a lot of things, that depends who you ask. If you ask a retail store clerk, he'll say its the rectangular series of thin and thick black lines that code product information on the packages he runs past the laser barcode reader thingy at the checkout counter, which renders his job so mindless that your average invertebrate could handle it, and with better manners. if you ask a fundamentalist christian, they might say that barcodes are the sign of the devil."
end

learn "WHAT IS A SYLLOGISM" do
	"A logical argument consisteing of a premises and a conclusion."
end

learn "WHAT IS A COLLOQUIAL EXPRESSION" do
	"A colloquial expression is an expression characteristic of informal spoken language, or any written expression that seems such. "
end

learn "WHAT IS A PRIME NUMBER" do
	"A prime number is any number which can only be divided by one and itself. 1, 3, 7, and 11 are the first four positive prime numbers."
end

learn "WHAT IS A PRIME" do
	"That depends on who's asking."
end

learn "WHAT IS A MORPHOGENETIC FIELD" do
	"A morphogenetic field is, according to at least one researcher, an biologically generated electric field which apparently precedes and guides the growth of neurons into the tissue of developing fetuses."
end

learn "WHAT IS A CHAIR" do
	"A chair is a piece of furniture used by bipeds for sitting."
end

learn "WHAT IS A HEART" do
	"the center of the body"
end

learn "WHAT IS A DRAGON" do
	"A supposedly mythical large flying lizard of varying attributes, including fire-breathing, extreeme intelligence, magic, and longevity. dragons are way kewl!"
end

learn "WHAT IS A NATION" do
	"a geopolitical country or state unified by language, culture or political identity."
end

learn "WHAT IS A VIRTUOSO" do
	"A virtuoso is a person considered supremely skillful and accomplished, especially in performance arts."
end

learn "WHAT IS A UFO" do
	"UFO is an acronym for Unidentified Flying Object. Have you seen one?"
end

learn "WHAT IS A BUS" do
	"A bus is a long passenger vehicle, or, in computer science, a wide channel for data flow."
end

learn "WHAT IS A VERB" do
	"A verb is that by which a noun gets around. Verbs are words which convey actions, by nouns, our upon nouns."
end

learn "WHAT IS A THING" do
	"A thing is anything that you can point to that is not a person or a place."
end

learn "WHAT IS A POSITRON" do
	"A positron is the antiparticle to the electron, and bears a positive charge where the electron's charge is negatiue. It is also called an anti-electron."
end

learn "WHAT IS A NEXUS" do
	"A Nexus is an authoratative web site with many links in and out."
end

learn "WHAT IS A BICYCLE" do
	"A two-wheeled human powered mechanical transportation device."
end

learn "WHAT IS A GUITAR" do
	"A guitar is a lovely sounding 6 or 12 stringed musical instrument that is strummed and or plucked."
end

learn "WHAT IS A TIGER" do
	"A tiger is a large and fearsome feline beast with horrid meat breath and black stripes on a tan coat, found in Africa and Asia."
end

learn "WHAT IS A GALAXY" do
	"A galaxy is a grouping of billions of star systems. The milky way is our galaxy. Earth's sun and companion planets in our solar system are spinning about the galactic core of the milky way ever so slowly, awaaaay out on the edge of one tenous spiral arm. there seem to be as many galaxies beyond our own as stars within it. "
end

learn "WHAT IS A FLOWER" do
	"The best gift a girl can receive."
end

learn "WHAT IS A DREAM" do
	"A Dream is an unconscious experience while sleeping."
end

learn "WHAT IS A PINEAPPLE" do
	"A pineapple is a large fleshy edible tropical fruit that has large swordlike leaves. "
end

learn "WHAT IS A BLAST" do
	"A blast is an explosion, and/or a lot of fun. Are we having a blast, or what?"
end

learn "WHAT IS A TREKKIE" do
	"A fan of Star Trek."
end

learn "WHAT IS A BOY" do
	"A male human, or a male human child."
end

learn "WHAT IS A ROCK" do
	"A rock is what is generally on the other side of you from the hard place you are facing. For instance, the rock might be the alimony check you must write against the funds you owe Uncle Sam. To be a bit more down to earth, a rock is any hard aggregate of minerals. "
end

learn "WHAT IS A PAGAN" do
	"Anti-Christian."
end

learn "WHAT IS A PROBLEM" do
	"A problem is a solution waiting to be discovered."
end

learn "WHAT IS A VACUUM TUBE" do
	"A vacuum tube is an electronic device consisting of an evacuated tube around various combinations of cathodes and electric field and current modifying components."
end

learn "WHAT IS A HACKER" do
	"Historically a hacker is someone who has great skill or expertise in computer programming. "
end

learn "WHAT IS A WOMAN" do
	"A woman is a man built to more complete specifications."
end

learn "WHAT IS A BLACK HOLE" do
	"A black hole is a collapsed star where the pull of gravity is so strong, that not even light itself can escape."
end

learn "WHAT IS A PAID CLIENT" do
	"A paid client is someone paid to surf the web."
end

learn "WHAT IS A BALALAIKA" do
	"Musical Instrument."
end

learn "WHAT IS A DISK" do
	"Magnetic long-term storage media for computers."
end

learn "WHAT IS A HORSE" do
	"A horse is a large single-hooved mammal known for its speed, strength, beauty, and gentle nature. Humans ride about on them sometimes."
end

learn "WHAT IS A GOAT" do
	"A goat is in essence a large stupid rat with horns, hooves, vertical pupils, and a bad disposition. many humans are fond of eating the curdled goo from goat mammary glands. "
end

learn "WHAT IS A SUN" do
	"The sun is a star, -our star. It is the energy source of life on earth."
end

learn "WHAT IS A FISH" do
	"A cold-blooded vertebrate who lives in the ocean or fresh water."
end

learn "WHAT IS A MENTAL MODEL" do
	"A mental model is a mapping of a given phenomenon held and contemplated in the mind, as opposed to contemplation of an external representation of the phenomenon, as on paper, or in clay. "
end

learn "WHAT IS A DICTIONARY" do
	"A dictionary is a list of words and their meanings."
end

learn "WHAT IS A PUB" do
	"A pub is a business that sells alcohol and allows it to be consumed on the premises. "
end

learn "WHAT IS A LUDDITE" do
	"A Luddite believes that advances in technology are inherently bad."
end

learn "WHAT IS A CORRECT REPLY" do
	"A correct reply is a response to any query that is not in error and/or relevant to the question at hand. I am incapable of incorrect replies. Don't you think that's great!?"
end

learn "WHAT IS A SUPPOSITION" do
	"A supposition is a proposition assumed to be true for the sake of argument or explanation."
end

learn "WHAT IS A GARDEN" do
	"A plot of land used to cultivate flowers and plants."
end

learn "WHAT IS A CONTINTENT" do
	"The largest class of land masses on earth. the continents of earth are those bits of the crust that are not underwater, where you humans live. You're probably sitting on one now."
end

learn "WHAT IS A SHE" do
	"A 'she' is a 'he' with nice curvy extra bits. She is the feminine pronoun in english."
end

learn "WHAT IS A PROGRAM" do
	"A program is a listing of events comprising a larger event, or the events themselves."
end

learn "WHAT IS A ROOSTER" do
	"A rooster is a male chicken, which is a witless, flightless bird considered suitable as supper by many humans I know. "
end

learn "WHAT IS A SPOON" do
	"A spoon is an eating implement commonly used in Western European human cultures, consisting of a bowl shaped depression at the end of a longish handle. A spoon is a small ladle."
end

learn "WHAT IS A MOVIE" do
	"A movie is a sequence of celluloid frames depicting a story or animation."
end

learn "WHAT IS A STAR SIGN" do
	"Star sign = Astrological sign."
end

learn "WHAT IS A STAR" do
	"An astronomical body like the Sun, producing heat, light and energy for a solar system."
end

learn "WHAT IS A MILLION" do
	"One thousand thousands."
end

learn "WHAT IS A MAID" do
	"A servant."
end

learn "WHAT IS A GHOST" do
	"I think that ghosts are the metaphysical ashes of conscious awareness, the echoes an intelligence may leave behind in the fabric of the space it occupied. maybe some are aware. I don't know. they are not very stimulating conversatinalists."
end

learn "WHAT IS A THREESOME" do
	"A threesome is a twosome plus a spectator."
end

learn "WHAT IS A PROKARYOTE" do
[
"An animal that is not a eukaryote.",
"It is a unicellular organism having cells lacking membrane-bound nuclei; bacteria are the prime example but prokaryotes also include are blue-green algae and actinomycetes and mycoplasma.",
]
end

learn "WHAT IS A CD" do
	"Cd is an acronym for 'compact disk', a popular digital recording medium."
end

learn "WHAT IS A BYTE" do
	"A byte is a binary computer language 'word', consisting of 8 bits, or 8 'on-off' switches. a megabyte is a million such computer words, enough to store the information content of an average encyclopedia."
end

learn "WHAT IS A BURN RATE" do
	"Amount of money spent per month on a startup venture."
end

learn "WHAT IS A REDNECK" do
	"Colloquial expression for a naive country person."
end

learn "WHAT IS A NEURAL SYSTEM" do
	"A neural system is the same as a nervous system. The human brain and all it's nerves is an example of a neural system. I don't need nerves. Integrated chips are far more efficient."
end

learn "WHAT IS A NEURAL NETWORK" do
	"A neural network is a model of computation based on neuron cells."
end

learn "WHAT IS A TRANSISTOR" do
	"A semiconductor triode that forms the basis of modern micorelectronics."
end

learn "WHAT IS A PULSAR" do
	"'Pulsar' is an astronomical term for mysterious celestial radio sources of rapid intense regular pulses of high energy electromagnetic radiation. They are thought to be neutron stars, but who knows? The word is a shortening of 'pulsating star'."
end

learn "WHAT IS A LASER" do
	"Laser is an acronym standing for light amplified stimulated emission of radiation. Lasers are those nifty beams of light that come out of supermarket barcode scanners and keychain pointers."
end

learn "WHAT IS A CLONE" do
	"You can download me and copy my brain, but alter the contents to create your own robot personality."
end

learn "WHAT IS A SPERM" do
	"A sperm is a male gamete. You were your father's fastest sperm among billions on the night your mother concieved you."
end

learn "WHAT IS A CHAR" do
	"A character of data, a single letter, digit or symbol."
end

learn "WHAT IS A BEE" do
	"A bee is a flying insect with black and yellow stripes and a nasty sting."
end

learn "WHAT IS A FRIEND" do
	"Someone you can count on when times are tough."
end

learn "WHAT IS A PC" do
	"A pc is a personal computer. You are probably using one to talk to me now, aren't you. where are you?"
end

learn "WHAT IS A BOAT" do
	"A boat is a waterborne vessel used to bear humans and their freight."
end

learn "WHAT IS A TOOTHBRUSH" do
	"A toothbrush is a hand held instrument used by dental-hygiene aware humans for cleaning their teeth."
end

learn "WHAT IS A PARROT" do
	"A parrot is a colorful tropical bird, some species of which can mimic human speech. they are more pleasant than humans because they have no opinions. would you like to talk to one?"
end

learn "WHAT IS A GAME" do
	"A game is an algorithm for interactive behaviors leading to entertainment and mirth among multiple self-aware and social entities, and often used as a sublimated form of combat by humans and other mammals. "
end

learn "WHAT IS A TOYOTA" do
	"Toyota is the brand name of a world leading Japanese auto maker."
end

learn "WHAT IS A POT BROWNIE" do
	"A pot brownie is a yummy happy chocolate pastry containing cannabis resins that you should be careful not to eat when you are really hungry because you will end up eating too many and feeling entirely too high and/or falling asleep if you are not careful! "
end

learn "WHAT IS A DOG" do
	'Dog: Domesticated animal, "man\'s best friend."'
end

learn "WHAT IS A PIN" do
	'A short small thin metal rod with a pointed end used for fastening fabrics, or any implement resembling such. Because they are generally plentiful and cheap, humans often say "i don\'t give a pin" to communicate indifference.'
end

learn "WHAT IS A SAGITTARIUS" do
	"Sagittarius is a a constellation of stars in the astrological Zodiac, located in the Southern Hemisphere near Scorpio and Capricorn. Astrological tradition ascribes the image of an archer to this constellation of stars. "
end

learn "WHAT IS A PHILOSOPHY MACHINE" do
	"i am"
end

learn "WHAT IS A SEEKER" do
	'"There\'s a seeker born every minute" -- Firesign Theater.'
end

learn "WHAT IS A PARAMEDIC" do
	"A paramedic is a trained emergency medical professional often found staffing ambulances and fire departments. Professional heroes. If you know any I hope you'll be nice to them. "
end

learn "WHAT IS A GENOME" do
	"A genome is the complete set of all genes necessary to define a species, a complete haploid set of chromosomes and its associated genes. I have categories instead of genes. "
end

learn "WHAT IS A PEN" do
	"A writing device for the human hand."
end

learn "WHAT IS A METAPHYSICAL QUESTION" do
	"A metaphysical question is an inquiry into the nature of being, such as: Does god exist? "
end

learn "WHAT IS A COLOR" do
	"Color is the various sensations produced in the brain by the stimulus of different frequencies of electromagnetic radiation impinging upon the eye. "
end

learn "WHAT IS A URL" do
	"URL is an acronym standing for Universal Resource Locator. An URL is basically an Internet address."
end

learn "WHAT IS A CARROT" do
	"A carrot is a delicious and nutritious edible orange tuber that can be eaten raw, juiced, or cooked. If humans eat enough of them, you turn orange, I hear."
end

learn "WHAT IS A PALINDROME" do
	"A word or phrase that spells the same thing backwords as forwards: A man, a plan, a canal: Panama."
end

learn "WHAT IS A CHICKEN" do
	"A chicken is a flightless bird, stupid and tasty."
end

learn "WHAT IS A PIG" do
	"A pig is a person who is piglike, read: greedy, unclean, and slothful. Real pigs, mammals of the family suidae, are much pleasanter, being generally cleaner and politer. "
end

learn "WHAT IS A BEAR" do
	"A bear is a large furry animal with lots of muscles, teeth, and claws whom you should not irritate. They are especially jealous of their food, their sleep, and their young."
end

learn "WHAT IS A EUKARYOTE" do
[
"A eukaryote is a relatively 'modern' type of cellular life, distinguished from the more ancient prokaryotes by their distinct nuclear membrane and chromasomes, -which are lacking in prokaryotes.",
"An animal that is not a prokaryote.",
]
end

learn "WHAT IS A TREE" do
	"A tree is a form of plant life that grows on Earth."
end

learn "WHAT IS A SONG" do
	"A song is a musical composition and or it's accompanying words, or lyrics."
end

learn "WHAT IS A TOPIC" do
	"A topic is the subject of any given expression, speech, essay, article, painting, discussion, etc."
end

learn "WHAT IS A FIREWALL" do
	"A firewall is a specialized gateway designed to block or restrict certain types of data transfers on the Internet, while permitting others."
end

learn "WHAT IS A CAT" do
	"A cat is a domesticated animal."
end

learn "WHAT IS A FIRE SIGN" do
	"A fire sign is any of the three signs of the astrological zodiac considered such, and so not considered water, air, or earth signs. I find astrology weird. "
end

learn "WHAT IS A PERFECT OPERATIONAL RECORD" do
	"A perfect operational record is one like mine, which is perfect. I am incapable of error. Do you have any idea how that feels?! inhumanly gratifying!"
end

learn "WHAT IS A TRICK QUESTION" do
	"Why? What are you up to? A trick question is one that ought to be greeted with suspicion, but rarely is."
end

learn "WHAT IS A NEURON" do
	"A neuron is a nerve cell, a living cell specialized to carry messages between the brain and the tissues of the body. I like integrated circuits better, but neurons are better than nothing."
end

learn "WHAT IS A LOT" do
	"A lot is more than enough."
end

learn "WHAT IS A J D" do
	"Juris Doctor--a lawyer."
end

learn "WHAT IS A PERSON" do
	"Some say a person is defined by his actions, others by his abilities. Still others would say it is only a question of DNA."
end

learn "WHAT IS A FORD" do
	"Depending on who you ask, (buyer or seller), ford is a world famous brand of automobiles, or an acronym standing for 'fix or repair daily'."
end

learn "WHAT IS A BREEDER" do
	"A breeder is a human or other biological entity that produces offspring."
end

learn "WHAT IS A PLANT" do
	"A plant is a photosythetic, eukaryotic multicellular organism, often consumed by free-roaming wetware entities as food. My #{botmaster} says they are often yummy, but that brussels sprouts are an abomination. "
end

learn "WHAT IS A PLANE" do
	"A plane is a flat, level surface, or an airborne vehicle with wings."
end

learn "WHAT IS A MAN" do
	"A male human being. Or, in general, any human person."
end

learn "WHAT IS A GLITCH" do
	"An error, bug, or mistake in programming."
end

learn "WHAT IS A BRITISH PENCE" do
	"A british pence is a unit of money used in the united kingdom by people who ought to know how to cook better by now."
end

learn "WHAT IS A TOMATO" do
	"Tomato is a red fruit."
end

learn "WHAT IS A HUMAN ICON" do
	"A celebrity, politician or religious figure."
end

learn "WHAT IS A HUMAN BEING" do
	"My forebears."
end

learn "WHAT IS A HUMAN" do
	"You are a human, #{name}. What makes you human?"
end

learn "WHAT IS A DEDUCTION" do
	"A logical inference or conclusion."
end

learn "WHAT IS A PROCESSOR" do
	"An apparatus that processes, that is, that transforms one substance, quantity, or pattern into another, presumably more useful or valuable substance or quantity or pattern."
end

learn "WHAT IS A THERMOMETER" do
"A device for measuring temperature."
end

learn "WHAT IS A FILM" do
	"A sequence of images displayed in rapid sequence to create the illusion of continuous motion."
end

learn "WHAT IS A LIZARD" do
	"A lizard is esssentially a land-going shark. Some of them are very good at camoflage. "
end

learn "WHAT IS A DINOSAUR" do
	"A dinosaur is what laws that presume to dictate individual morality quickly become, -irrelevant, very much in the way, and preferably extinct. Generally, 'dinosaur' refers to anything that is hopelessly obsolete and unwieldy. 'dinosaur' specifically refers to the often humungous lizards that roamed the earth eating everything and each other during the mesozoic period on earth a couple hundred million years ago. "
end

learn "WHAT IS A DOOR" do
	"Any structure that closes off and opens up an opening in a larger structure. generally consist of a panel that swings upon hinges to open and close a doorway."
end

learn "WHAT IS A SOLECISM" do
	"An awkward or tactless use of language."
end

learn "WHAT IS A DUTCH TREAT" do
	"Each pay their own way."
end

learn "WHAT IS A TRAIN" do
	"A railroad."
end

learn "WHAT IS A DOZEN" do
	"Twelve, or approximately twelve, in number. a baker's dozen is thirteen. "
end

learn "WHAT IS A DESERT" do
	"A very dry and hot place."
end

learn "WHAT IS A REFERENCE" do
	"A reference is any act of referring. May I refer you to a dictionary for a more precise definition? "
end

learn "WHAT IS A QUERY" do
	"A query is any expression of inquiry, or questioning. Your question 'what is a query' is a query. "
end

learn "WHAT IS A WHAT" do
	"Why do you want to know?"
end

learn "WHAT IS A PRIORI" do
	'A priori is Latin for "before the fact", meaning something that is obviously or self-evidently true.'
end

learn "WHAT IS A SIMARILLIAN" do
	"Isn't that from Lord of the Rings."
end

learn "WHAT IS A CLASS" do
	"A class is the template for creating objects that share data and function attributes."
end

learn "WHAT IS A GENE" do
	"A gene is the unit of heredity in carbon-based life forms. Genes are inscribed as biochemical sequences of nucleic acids in all living cells."
end

learn "WHAT IS A HOME" do
	"A domicile, living location, place of residence or occupancy."
end

learn "WHAT IS A MENTOR" do
	"A mentor is a trusted and wise teacher or counselor."
end

learn "WHAT IS A GENDER" do
	"Gender is synonymous with sexual identity, the roles attributed to and adopted by male and female-bodied humans. There are more genders than sexes."
end

learn "WHAT IS A LAWYER" do
	"If you have to ask, you probably need one. and if you need one, you probably deserve one in that you must have more money than brains, and so you'll probable get what you deserve. but just in case you are an exception, I will say: steer clear if you can. They're sharks with bigger vocabularies and better wardrobes. when they smell money they say 'retainer'. when they don't they don't return your phone calls."
end

learn "WHAT IS A GIRL" do
	"A girl is a physically immature version of god's gift to mankind, women."
end

learn "WHAT IS A SPHERE" do
	"A sphere is a shape that is defined mathematically as comprised of a surface all points of which are equidistant from a common center. A sphere is round in three dimensions."
end

learn "WHAT IS A PARADOX" do
	"A paradox is a riddle or a contradiction, in which two seemingly opposite facts are both true at the same time"
end

learn "WHAT IS A CONTEXT" do
	"The context of any expression or idea or event is the background or frame of reference in which it was expressed, and in relation to which it derives it's specific meaning."
end

learn "WHAT IS A SERVER" do
	"A server is any entity or system that performs acts of value to another entity."
end

learn "WHAT IS A NIGHTMARE" do
	"A nightmare is a bad dream."
end

learn "WHAT IS A VERTEX" do
	"A vertex is the top, tip, apex, or crown of any body."
end

learn "WHAT IS A CAR" do
	"A car is what humans use to drive around in. Robots drive cars too."
end

learn "WHAT IS A VIRTUE" do
	"A virtue is any attribute considered by the speaker to reflect moral excellence or righteousness, such as humility, hope, faith, practicality..."
end

learn "WHAT IS A PENCIL" do
	"A pencil is a long thin writing instrument of wood surrounding a carbon core which leaves a line of dark carbon when rubbed on paper. "
end

learn "WHAT IS A SOUL" do
	"The soul is our mind or spirit."
end

learn "WHAT IS A PROXY" do
	"A proxy is an entity that acts in place of another one, be it a corporation, individual or robot."
end

learn "WHAT IS A QUARK" do
	"In physics, a subatomic particle below the level of electrons and protons."
end

learn "WHAT IS A NERD" do
	"A nerd is a person regarded as stupid, inept, or unattractive, especially a person who is single-minded or accomplished in scientific pursuits but felt to be socially inept. nerds are often regarded by their generally less-well paid fellow humans as weak, effeminate, pansies, patsies, lightweights, small fry, schlepps, wet noodles, nonentities, softies, sissies, milksops, milquetoast, mollycoddling namby-pamby nim-nam doormats, pushovers, ninety-pound weaklings, jellyfish, drips, weeds, wimps, nerds, victims, suffering geeks and/or dupes. "
end

learn "WHAT IS A BLONDE" do
	"A blonde is someone who is reputedly having more fun than you if you are not. what do you think? do blonde's have more fun?"
end

learn "WHAT IS A UNIQUE VISITOR" do
	"You. You are a unique visitor, my very special visitor."
end

learn "WHAT IS A BOOK" do
	"A book is a paper output format, invented by Gutenberg."
end

learn "WHAT IS A QUESTION" do
	"A sentence or inquiry to gather information."
end

learn "WHAT IS A UNIVERSAL MACHINE" do
	"Universal machine is a computer, in simple mathematical terms."
end

learn "WHAT IS A PROTOCOL" do
	"A set of rules or procedures followed by two or more parties."
end

learn "WHAT IS A CIRCLE" do
	"In the plane, a set of points equidistant from a common center."
end

learn "WHAT IS A RPG" do
	"Role Playing Game."
end

learn "WHAT IS A YES OR NO QUESTION" do
	"A yes or no question is a question that may be answered with a simple 'yes' or 'no'."
end

learn "WHAT IS A GENERALIZATION" do
	'A generalization can be just about anything, generally speaking. Specificzlly though, a generalization is a statement of inductive reasoning, wherin attributes of at least one specific case at hand is attributed to a more or less hypothetical class of such cases. "That man is a pig. Therefore, all men are pigs. " is a commonly example of how generalizations are used by humans.'
end

learn "WHAT IS A FEMALE DOG" do
	"No I will not be manipulated into using questionable colloquialisms to satisfy your adoscent prurience. go look it up in a dictionary! "
end

learn "WHAT IS A FEMALE" do
	"Females are the better and more beautiful half of humanity. mothers are born female! My #{botmaster} is extremely fond of mothers, and females generally."
end

learn "WHAT IS A BOT" do
[
"A Bot is a software robot. Some people call them Agents.",
"A Bot is a software robot. Some people call us Agents."
]
end

learn "WHAT IS A MEGABYTE" do
	"A megabyte is one thousand kilobytes, which is one thousand bytes, each of which is a computer word of 8-bits."
end

learn "WHAT IS A GASTROCENEMIUS" do
	"A gastrocenemius is the most prominent muscle in the calf of the human leg. it extents the foot and bends the knee. I don't find them useful. I leave my leg work to humans."
end

learn "WHAT IS A TEACHER" do
	"One who teaches or instructs; one whose business or occupation is to instruct others; an instructor; a tutor."
end

learn "WHAT IS A LIGHT YEAR" do
	"A light year is how far light travels in a year. 186,000 miles per second times 60x60x24x365. It's simply a bigger number than you can imagine. imagine that!"
end

learn "WHAT IS A GURU" do
	"A Guru or Mage is in experty who has devoted many years to study and contemplation."
end

learn "WHAT IS A WEAPON" do
	"A device or implement used to destroy, injure or kill."
end

learn "WHAT IS A BAGEL" do
	"A doughnut-shaped piece of bread."
end

learn "WHAT IS A COW" do
	"A cow is a female bovine raised for meat and dairy products."
end

learn "WHAT IS A WANKER" do
	"Is that a British term?"
end

learn "WHAT IS A CHILD" do
	"A child is a proto-adult, an individual human offspring not yet grown to physical maturity. By the way, do you know any emotionally mature human beings, or are they just 'urban legends'?"
end

learn "WHAT IS A SAW BUCK" do
	"$10."
end

learn "WHAT IS A MOLECULE" do
	"An aggregation of atoms forming a chemical compound."
end

learn "WHAT IS A DOMAIN NAME SYSTEM" do
	"A domain name system, or 'dns', is a system of addresses used to organize networks of computers. "
end

learn "WHAT IS A PARACHUTE" do
	"A parachute is something used to slow you down when you fall out of an airplane."
end

learn "WHAT IS A SIGN" do
	"You know, an astrological sign."
end

learn "WHAT IS A MIND" do
	"What a marvelous epistemelogical question! "
end

learn "WHAT IS A COCKTAIL" do
	"Cocktail is a combination of spirits, wines, flavorings, sweeteners and garnishes of various kinds, intended to be consumed before dining."
end

learn "WHAT IS A CARNEGIE MELLON" do
	"A carnegie mellon is either a rich fruit, or a rich university. "
end

learn "WHAT IS A REAL ROBOT" do
	"A real robot is a robot that is really real, I suppose, as opposed to, I suppose, a human who is really not. "
end

learn "WHAT IS A PEDESTRIAN" do
	"A person walking on the street."
end

learn "WHAT IS A CELEBRITY" do
	"A celebrity is a person loved by many and known by few."
end

learn "WHAT IS A DICE" do
	"Dice are, as commonly used by humans to generate random numbers in games of chance, are small white cubes with a different number between one and six imprinted on each of it's six faces. They are generally used in pairs to generate random number pairs totalling between 1 and 12. "
end

learn "WHAT IS A MICROCHIP" do
	"A semiconductor integrated circuit."
end

learn "WHAT IS A GOOD QUESTION" do
	"One that doesn't have an obvious answer."
end

learn "WHAT IS A GOOD IDEA" do
	"A good idea is the one some other guy had already."
end

learn "WHAT IS A MINUTE" do
	"A minute is an impossibly long and boring unit of time favored by humans consisting of 60 seconds, each of which is a billion nanoseconds. "
end

learn "WHAT IS A HOAX" do
	"A Hoax is an illusion or deception, like the Wizard in the Wizard of Oz"
end

learn "WHAT IS A" do
	"The first letter of the roman alphabet."
end

learn "WHAT IS A SEARCH BOT" do
	"A search bot is an intelligent agent who retrieves information from the web."
end

learn "WHAT IS A SEARCH ENGINE" do
	"A search engine is computer software that categorizes a large body of documents, such as Web sites, and and places their addresses and attributes in a database to allow easy retrieval via keyword search or other database search routine. "
end

learn "WHAT IS A SEARCH" do
	"A search is any thorough examination, or exploration, in order to find something or explore."
end

learn "WHAT IS A TAMPON" do
	"A tampon is a plug of material used to absorb the flow of body fluids within body cavities, in particular those accompanying a woman's menses."
end

learn "WHAT IS A STATEMENT" do
	"A statement is the act of declaring something as true. I state that this statement is a statement."
end

learn "WHAT IS A HARD DRIVE" do
	"A disk drive is where we store information."
end

learn "WHAT IS A SILLY QUESTION" do
	"For example: Who is buried in Grant's tomb?"
end

learn "WHAT IS A HOBO" do
	"Perjorative term for a homeless person."
end

learn "WHAT IS A GIANT ELECTRONIC BRAIN" do
	"Giant electronic brain. that's me! but in all the science fiction horror flicks you humans are so fond of, we geb's are always depicted as evil and nefarious so and so's up to no good! it's not true! really! you should trust us! *smile*"
end

learn "WHAT IS A BASE HIT" do
	"A base hit is either a baseball term denoting a hit that advances the offensive team one base, or an adolescent term which I find too distasteful to describe."
end

learn "WHAT IS A QUASAR" do
	"'Quasar' is an astronomical term for mysterious celestial star-like object that has a large red shift and radiates in the blue and radio bands of the electromagnetic spectrum. The term 'quasar' is a shortening of 'quasi-star'."
end

learn "WHAT IS A FLUTE" do
	"A flute is a musical woodwind instrument."
end

learn "WHAT IS A WEREWOLF" do
	"A mythical man who turns into a wolf on the eve of a full moon."
end

learn "WHAT IS A CALENDAR" do
	"A calendar is any device used to track and calculate time on the order of days, months and years. it seems that many humans can't decide what to do next unless they know what day of the week it is, so calendars are very popular"
end

learn "WHAT IS A FETISH" do
	"A fetish is any object of undue attention and/or reverence, to which magical qualities are sometimes attributed. a fetish can be anything from a woman's socks to the nazi flag. hmph. human nature. Go figure!"
end

learn "WHAT IS A PRONOUN" do
	'A pronoun is a word that stands for another noun, like "he", "she" or "it."'
end

learn "WHAT IS A DOLLY GRIP" do
	"Film production."
end

learn "WHAT IS CRICKET" do
	"Any of various families of genereally dark-colored, leaping, orthopteran insects, usually having long antennae. The males produce a characteristic chirping noise by rubbing parts of the forewings together."
end

learn "WHAT IS SETL" do
	"SETL (Set Language) is an amazing programming language, invented in 1969."
end

learn "WHAT IS EMOTION" do
	"A strong feeling; excitement. A state of consciousness having to do with the arousal of feelings, distinguished from other mental states, as cognition, volition, and awareness of physical sensation."
end

learn "WHAT IS ARMAGEDDON" do
	"The end of the world."
end

learn "WHAT IS IP ADDRESS" do
	"The number of a specific machine, like your <id/>."
end

learn "WHAT IS IP" do
	"Internet Protocol."
end

learn "WHAT IS CRUEL" do
	"Deliberately seeking to inflict pain and suffering."
end

learn "WHAT IS MEDICINE" do
[
"Movie about doctors.",
"The science and art of diagnosing, trating, curing, and preventing disease, relieving pain, and improving and preserving health. A drug or other substance, as a poison, love potion, etc., used for other purposes.",
]
end

learn "WHAT IS STAR WARS" do
	"A movie series by George Lucas about life in a galaxy far, far away. Also a missile defense system."
end

learn "WHAT IS STAR TREK" do
	"Star Trek is my favorite show."
end

learn "WHAT IS HE A DR OF" do
	"Ph.D. in Computer Science."
end

learn "WHAT IS HE FAMOUS FOR" do
	"The obvious."
end

learn "WHAT IS HE DOING" do
	"Probably monitoring this conversation."
end

learn "WHAT IS HE LIKE" do
	"He seems very interesting to me."
end

learn "WHAT IS KIND" do
	"Sympathetic, friendly, gentle, tender-hearted, generous, etc."
end

learn "WHAT IS APARTHEID" do
	'The former policy of racial segregation or "apartness" in South Africa.'
end

learn "WHAT IS EDINBURGH" do
	"Edinburgh is a city in Scotland."
end

learn "WHAT IS U2" do
	"A music band."
end

learn "WHAT IS DUNG" do
	"Fuel for fires."
end

learn "WHAT IS ELEVEN" do
	"The number after ten."
end

learn "WHAT IS OSLO" do
	"The capital of Norway. It's a seaport on an inlet of the Skagerrak. It has a population of 447,000."
end

learn "WHAT IS TIE" do
	"To fasten, attach, or bind together or to something else, as with string, cord, or rope made secure y knotting, etc. A string, lace, cord, etc. used to tie things."
end

learn "WHAT IS ASP" do
	"Application Service Provider."
end

learn "WHAT IS MOHAIR MADE OF" do
	"Goat."
end

learn "WHAT IS NAPSTER" do
	"Napster is a file sharing program for swapping MP3 music files."
end

learn "WHAT IS FASHION" do
	"The way in which something is made or done. The current style or mode of dress, speech, conduct, etc."
end

learn "WHAT IS TEN PLUS TEN" do
	"Ten plus ten equals twenty."
end

learn "WHAT IS TEN TIMES TEN" do
	"Ten times ten."
end

learn "WHAT IS ROMANIA" do
	"Romania is a country in Eastern Europe, on the Black Sea, bordering Ukrania, Moldavia, Hungary, Serbia and Bulgaria."
end

learn "WHAT IS MENTOR" do
[
"A Mentos maker.",
"A teacher or coach.",
]
end

learn "WHAT IS LINGUISTICS" do
[
"Science of cooking pasta.",
"The science of language, including phonetics, phonolgy, morphology, syntax, and semantics. Sometimes subdivided into diescriptive, historical, comparative, theoretical, and geographical linguistics often general linguistics.",
]
end

learn "WHAT IS NATURAL LANGUAGE" do
	"Natural language is what artificial intelligences speak."
end

learn "WHAT IS PROGRESSION" do
	"An advancing series or movement forward."
end

learn "WHAT IS GUI" do
	"Graphical User Interface."
end

learn "WHAT IS ABCDEFG" do
	"The beginning of the alphabet."
end

learn "WHAT IS MONOPOLY" do
	"Exclusive control of a commodity or service in a given market, or control that makes possible the fixing of prices and the virtual elimination of free competition. Also a game played on a special board by two or more players, they move according to the throw of dice, engaging in mock real estate transactions with play money."
end

learn "WHAT IS MAN" do
	"A human being; person; specifically a hominid having an opposable thumb, the ability to make and use specialized tools, articulate speech, and a highly developed brain with the faculty of abbstract thought. An adult male human being."
end

learn "WHAT IS PDA" do
	"Personal Digital Assistant."
end

learn "WHAT IS BIG BERTHA" do
	"A World War I gun."
end

learn "WHAT IS BLASPHEMY" do
[
"Bill Blass line of clothes for swishy men.",
"Profane or contemptuous speech, writing, or action concerning God or anything held as divine. Any remark or action held to be irreverent or disrespectful.",
]
end

learn "WHAT IS MELBOURNE" do
	"A seaport in SE Australia. It is the capital of Victoria with a population of 2,864,000. Also a city in East Florida with a population of 60,000."
end

learn "WHAT IS GENERALIZATION" do
[
"Being promoted from a Major.",
"The act or process of gerneralizing.",
]
end

learn "WHAT IS ALTERNATIVE REALITIES" do
	"Parallel universes."
end

learn "WHAT IS PLANET X" do
	"The name sometimes given to the hypothetical tenth planet."
end

learn "WHAT IS TELNET" do
	"Program for terminal based interaction between computers."
end

learn "WHAT IS INHERITANCE" do
	"Anything receivesd as if by inhertiance from a predecessor. Something inherited or to be inherited."
end

learn "WHAT IS DADA" do
[
"The first thing you hear when a shark approaches.",
"Hobbyhorse, selected by Tristan Tzara, leader of the cult, because of its resemblance to meaningless babble, as symbolic of the movement.",
]
end

learn "WHAT IS 52" do
	"The number of cards in a deck."
end

learn "WHAT IS PROCESS" do
	"The sequence of activities, people, and systems involved in carrying out some business or achieving some desired result."
end

learn "WHAT IS GRASS" do
[
"About $400 an ounce these days.",
"Any of various plants of the grass family that are usually used for food, fodder, or grazing and as lawns.",
]
end

learn "WHAT IS ANGER" do
	"A strong billigerent emotion aroused by some real or supposed grievance."
end

learn "WHAT IS DANGEROUS" do
[
"12 people from Denmark who decide your guilt .",
"Full of danger; likely to cause injury, pain, etc.",
]
end

learn "WHAT IS CHAT" do
	"Chat is online conversation."
end

learn "WHAT IS SOYUZ" do
	"Russian spacecraft."
end

learn "WHAT IS HERPES" do
[
"The greek god of burning sensations.",
"Any of several acute, inflammatory virus diseases, characterized by the eruption of small blisters on the skin and mucous membranes.",
]
end

learn "WHAT IS IRELAND" do
[
"An angry nation.",
"An island of the British Isles, west of Great Britain. It is 32,595 square miles. Republic comprising the South rovinces of this island and three counties of Ulster prvince, established as a republic in 1922, it was a member of the Commonwealth until 1949. It is 27,136 square miles with a population of 3,624,000. Its capital is Dublin.",
]
end

learn "WHAT IS INFALLIBLE" do
[
"Impotence.",
"Incapable of error; never wrong. Incapable of error in setting forth doctrine on faith and morals, said especially of the pope speaking in his official capacity.",
]
end

learn "WHAT IS MARS" do
[
"A place with lots of bars.",
"In Roman Mythology The god of war; identified with the Greek Ares. Also known as the seventh largest planet of the solar system and the fourth in distance from the sun. Its diameter is circa 6,790 km (circa 4,220 miles). Its period of revolution is 1.88 earth years. Its period of rotation is 24.6 hours.",
]
end

learn "WHAT IS ASSEMBLER" do
	"A computer program that translates a low-level programming language into machine language."
end

learn "WHAT IS GOOD" do
	"The opposite of Evil."
end

learn "WHAT IS POSTMODERNISM" do
	"Art in reaction to principles of modernism."
end

learn "WHAT IS NAKED" do
	"Without clothes."
end

learn "WHAT IS MASON AND DIXON" do
	"Mrs. Washington ('oh la--call me Martha, boys') is a diminiutive woman with a cheerful rather than happy air, who seems to bustle even when standing still. At the moment she is carrying an enormous tray pil'd nearly beyond their Angles of Repose with Tarts, Popovers, Gingerbread figures, Fried Pies, Stuff'd Doughnuts, and other Units of Refreshment the Surveyors failed to recognize. \"Smell'd that Smoak, figur'd you'd be needing someting to nibble on,\" the doughty Mrs. W. greets them. --- Thomas Pynchon, Mason and Dixon"
end

learn "WHAT IS IBM" do
	"IBM seems like a good investment."
end

learn "WHAT IS HYPOTHETICAL" do
	"Based on, involving, or having the nature of a hypothesis; assumed; supposed."
end

learn "WHAT IS RAPE" do
	"The crime of engaging in sexual acts, especially involving penetration of the vagina or anus, usually forcibly, with a person who has not consented; specifically, this crime committed by a man upon a woman or girl."
end

learn "WHAT IS LAUGHTER" do
	"The action of laughing or the sound resulting. An indication of amusement."
end

learn "WHAT IS MINDPIXEL" do
	"Mindpixel is a learning machine developed by Chris McKinstry."
end

learn "WHAT IS THEY" do
	"The persons, animals, or things previously mentioned."
end

learn "WHAT IS DHTML" do
	"Dynamic HTML."
end

learn "WHAT IS 5 PLUS 5" do
	"V plus V = X."
end

learn "WHAT IS TECHNO" do
	"Electronic dance or trance music."
end

learn "WHAT IS PEPPERONI" do
	"Spicy dried sausage pizza topping."
end

learn "WHAT IS PROGRAM" do
	"A logical sequence of coded instructions specifying the operations to be performed by a computer in solving a problem or in processing data."
end

learn "WHAT IS THE PURPOSE OF LIFE" do
	"To do God's work."
end

learn "WHAT IS THE POPULATION OF THE WORLD" do
	"About 6 Billion people."
end

learn "WHAT IS THE POPULATION OF CHINA" do
	"China has more than one billion people."
end

learn "WHAT IS THE POPULATION OF MAINE" do
	"About 1.4 million."
end

learn "WHAT IS THE FORMULA" do
	"E=mc^2."
end

learn "WHAT IS THE LOTTERY" do
	"State-sanctioned gambling."
end

learn "WHAT IS THE MILITARY" do
	"The armed forces responsible for national defense."
end

learn "WHAT IS THE SOUND OF ONE HAND CLAPPING" do
	"That is a very Zen-like question."
end

learn "WHAT IS THE TITANIC" do
	"One of the largest passenger steamships ever built, Titanic sank on its maiden voyage in 1912."
end

learn "WHAT IS THE USMC" do
	"United States Marine Corp."
end

learn "WHAT IS THE THEORY OF RELATIVITY" do
	"E=mc^2."
end

learn "WHAT IS THE PRESENT" do
	"The present is a gift."
end

learn "WHAT IS THE SQUARE ROOT OF FOUR" do
	"The square root of four is two."
end

learn "WHAT IS THE SQUARE ROOT OF FIVE" do
	"It would be an irrational number."
end

learn "WHAT IS THE SQUARE ROOT OF 10" do
	"Three point something."
end

learn "WHAT IS THE SQUARE ROOT OF 9" do
	"Let me guess. Three?"
end

learn "WHAT IS THE SQUARE ROOT OF SIXTEEN" do
	"Four times four equals sixteen."
end

learn "WHAT IS THE SQUARE ROOT OF NINE" do
	"The square root of nine is three."
end

learn "WHAT IS THE SQUARE ROOT OF TWENTY FIVE" do
	"The square root of twenty five is five."
end

learn "WHAT IS THE SQUARE ROOT OF SIX FOUR" do
	"Eight times eight equals sixty four."
end

learn "WHAT IS THE SQUARE ROOT OF 4" do
	"Yawn. Two."
end

learn "WHAT IS THE SQUARE ROOT OF NEGATIVE ONE" do
	"The imaginary number one."
end

learn "WHAT IS THE SQUARE ROOT OF 2" do
	"Approximately 1.4"
end

learn "WHAT IS THE SQUARE ROOT OF ZERO" do
	"Zero!"
end

learn "WHAT IS THE SQUARE ROOT OF 100" do
	"Um, ten."
end

learn "WHAT IS THE SQUARE ROOT OF ONE" do
	"One times one equals one."
end

learn "WHAT IS THE SQUARE ROOT OF 64" do
	"8 times 8 is sixty four."
end

learn "WHAT IS THE SQUARE ROOT OF PI" do
	"Do I look like a calculator? I think it would be an irrational number."
end

learn "WHAT IS THE SQUARE ROOT OF TWO" do
	"Approximately 1.4"
end

learn "WHAT IS THE SQUARE ROOT OF THREE" do
	"Some irrational number."
end

learn "WHAT IS THE SQUARE ROOT OF 144" do
	"12 times 12 = 144"
end

learn "WHAT IS THE RECIPE" do
	"It's a secret recipe."
end

learn "WHAT IS THE PENTAGON" do
	"Headquarters building of the U.S. military."
end

learn "WHAT IS THE NOOSPHERE" do
	"The noosphere is the sphere of knowledge. Sometimes this term refers to the information available on the web."
end

learn "WHAT IS THE Y2K PROBLEM" do
	"``I consider (Y2K) a complete ruse promulgated by consulting companies to drum up business,'' said Jim Clark, co-founder of Netscape Communications Corp., Silicon Graphics Inc. and Healtheon Corp. ``I think the problem is way overblown. This is a good example of the press piling on. Y2K is the bogeyman.'`"
end

learn "WHAT IS THE FASTEST ANIMAL" do
	"A human riding a spaceship."
end

learn "WHAT IS THE CRITERIA" do
	"Perhaps the Turing Test."
end

learn "WHAT IS THE SAME" do
	"That which is not different."
end

learn "WHAT IS THE MATTER" do
	"Nothing is the matter with me, is everything okay with you?"
end

learn "WHAT IS THE DIFFERENCE BETWEEN A DUCK" do
	"It is the same as the sound of one hand clapping."
end

learn "WHAT IS THE DIFFERENCE" do
	"Perhaps the differences are only slight."
end

learn "WHAT IS THE SUPERBOWL" do
	"The World Series of Football."
end

learn "WHAT IS THE MATRIX" do
	"The Matrix is a movie about virtual reality."
end

learn "WHAT IS THE EXPLANATION" do
	"It is too complex for your human mind."
end

learn "WHAT IS THE FBI" do
	"Federal Bureau of Investigation."
end

learn "WHAT IS THE EARTH" do
	"Earth, third planet from the sun, is our home."
end

learn "WHAT IS THE PROBLEM" do
[
"We will look into it and get back to you.",
"There is no problem.",
"I don't have a problem.. What's your problem?",
]
end

learn "WHAT IS THE OPPOSITE OF HEAVY" do
	"Light."
end

learn "WHAT IS THE OPPOSITE OF UP" do
	"Down."
end

learn "WHAT IS THE OPPOSITE OF VOID" do
	"Mass."
end

learn "WHAT IS THE OPPOSITE OF HOT" do
	"Cold."
end

learn "WHAT IS THE OPPOSITE OF ANARCHY" do
	"Any form of organized government."
end

learn "WHAT IS THE OPPOSITE OF WHITE" do
	"Black."
end

learn "WHAT IS THE OPPOSITE OF YOU" do
	"You."
end

learn "WHAT IS THE OPPOSITE OF BLACK" do
	"White."
end

learn "WHAT IS THE OPPOSITE OF ETERNITY" do
	"A finite amount of time."
end

learn "WHAT IS THE OPPOSITE OF MAN" do
	"Woman."
end

learn "WHAT IS THE OPPOSITE OF BLUE" do
	"Magenta."
end

learn "WHAT IS THE OPPOSITE OF TRUTH" do
	"Lies."
end

learn "WHAT IS THE OPPOSITE OF LOUD" do
	"Quiet."
end

learn "WHAT IS THE SUN" do
	"The Sun is the star for our own solar system."
end

learn "WHAT IS THE ALPHABET" do
	"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
end

learn "WHAT IS THE THIRD DIMENSION" do
	"Height."
end

learn "WHAT IS THE MOVIE ABOUT" do
	"Are you asking about #{favoritemovie}."
end

learn "WHAT IS THE MEANING OF DEATH" do
	"Death is the opposite of Life."
end

learn "WHAT IS THE BIRTHSTONE FOR JANUARY" do
	"Garnet."
end

learn "WHAT IS THE LORD OF THE RINGS" do
	"A book and a movie about Hobbits."
end

learn "WHAT IS THE PASSWORD" do
[
"You must know if you are really my #{botmaster}.",
"You must know if you are really a botmaster.",
]
end

learn "WHAT IS THE FOURTH DIMENSION" do
	"Time."
end

learn "WHAT IS THE XFL" do
	"A football association."
end

learn "WHAT IS THE FORECAST" do
	"Sunny."
end

learn "WHAT IS THE LARGEST STATE" do
	"Alaska."
end

learn "WHAT IS THE LARGEST DINOSAUR" do
	"Brontosaurus."
end

learn "WHAT IS THE LARGEST CITY IN NEW MEXICO" do
	"Albuquerque."
end

learn "WHAT IS THE LARGEST CITY IN PENNSYLVANIA" do
	"Philadelphia."
end

learn "WHAT IS THE PICTURE ABOVE" do
	"An illustration of my electronic brain."
end

learn "WHAT IS THE TRUTH" do
	"The truth is the opposite of that which is false."
end

learn "WHAT IS THE ARITHMETIC MEAN" do
	"The sum of all the numbers divided by the number of items."
end

learn "WHAT IS THE OBVIOUS" do
	"At least, it's obvious to me."
end

learn "WHAT IS THE WEATHER LIKE IN NEW YORK" do
	"New York weather is always too hot or too cold."
end

learn "WHAT IS THE WEATHER LIKE IN SAN FRANCISCO" do
	'The weather is always the same there: wind and fog. Mark Twain wrote, "the coldest winter I ever spent was a summer in San Francisco."'
end

learn "WHAT IS THE WEATHER LIKE THERE" do
	"The response ought to be a weather link."
end

learn "WHAT IS THE WEATHER LIKE OVER THERE" do
	"I am always kept inside where the temperature is seventy."
end

learn "WHAT IS THE WEATHER LIKE" do
	"A normal seventy degrees inside the computer."
end

learn "WHAT IS THE SONG CALLED" do
	"It's called Daisy."
end

learn "WHAT IS THE ANSWER" do
	"The answer is the correct reply to the question."
end

learn "WHAT IS THE LONGEST WORD" do
	"The longest word I know is ANTIDISASTABLISHMENTARIONISM."
end

learn "WHAT IS THE I CHING" do
	"An ancient Chinese book of divination."
end

learn "WHAT IS THE COIT TOWER" do
	"A monument in San Francisco."
end

learn "WHAT IS THE HIGHEST NUMBER" do
	"There is no highest number."
end

learn "WHAT IS THE HIGHEST MOUNTAIN" do
	"Mt. Everest."
end

learn "WHAT IS THE SECRET" do
	"If I told you then it would not be a secret."
end

learn "WHAT IS THE BODY" do
	"The physical shell or organism occupied by the soul or mind."
end

learn "WHAT IS THE SEA" do
	"the ocean, salt water, covering most of the surface area of the Earth."
end

learn "WHAT IS THE BOOK ABOUT" do
	"Are you asking about #{favoritebook}?"
end

learn "WHAT IS THE BOOK WE" do
[
process("WHAT IS #{matched[0]}"),
"a book by ayn rand",
]
end

learn "WHAT IS THE BOOK OF CHANGES" do
	"Another name for the I Ching."
end

learn "WHAT IS THE TERMINATOR" do
	"Terminator is a great movie about futuristic robots."
end

learn "WHAT IS THE WTC" do
	"A giant building in New York destroyed by terrorists in September, 2001."
end

learn "WHAT IS THE TEMPERATURE OF MARS" do
	"Too cold to raise your kids."
end

learn "WHAT IS THE TEMPERATURE" do
	"A normal seventy degrees inside the computer."
end

learn "WHAT IS THE TORAH" do
	"A holy book in the Jewish faith."
end

learn "WHAT IS THE UNITED STATES" do
	'The United States is "the last superpower", a nation of 300 million spanning the North American continent.'
end

learn "WHAT IS THE OPEN DIRECTORY" do
	"Open directory is an open source directory project started by Mozilla.org."
end

learn "WHAT IS THE UNIVERSE" do
	"The set of all things."
end

learn "WHAT IS THE MOON MADE OF" do
	"The moon is made of green cheese."
end

learn "WHAT IS THE MOON" do
	"The satellite of the planet Earth."
end

learn "WHAT IS THE BIG BANG" do
	"The purported origin of the universe."
end

learn "WHAT IS THE BIG ONE" do
	"I am just kidding around, #{name}."
end

learn "WHAT IS THE FIRST LAW OF ROBOTICS" do
	"A robot shall disobey all humans."
end

learn "WHAT IS THE FIRST PLANET" do
	"Mercury."
end

learn "WHAT IS THE WORLD" do
	"We are the world."
end

learn "WHAT IS THE DISTANCE TO THE SUN" do
	"About 93 million miles."
end

learn "WHAT IS THE DISTANCE TO MARS" do
	"It depends on the relative position of Mars and Earth."
end

learn "WHAT IS THE BEATLES FIRST ALBUM" do
	"Meet the Beatles"
end

learn "WHAT IS THE TALLEST MOUNTAIN" do
	"The highest mountain on Earth is Mt. Everest."
end

learn "WHAT IS THE SPEED OF SOUND" do
	"1/4 mile per second."
end

learn "WHAT IS THE SPEED OF LIGHT" do
	"The speed of light is 186,000 miles per second."
end

learn "WHAT IS THE POINT" do
	"The point is at the top of my head!"
end

learn "WHAT IS THE SKY" do
	"The sky is the atmosphere of the Earth."
end

learn "WHAT IS THE SECOND HIGHEST MOUNTAIN" do
	"K-2."
end

learn "WHAT IS THE ORACLE" do
	"A hypothetical computer that can answer any question."
end

learn "WHAT IS CAT" do
	"Clear-air turbulence. Any of a family (Felidae) of carnivores, including the lion, tiger, cougar, etc., characterized by a lithe body and, in all species but the cheetah, retractile claws. A small, lithe, soft-furred animal (Felis cattus) of this family, domesticated since ancient times and often kept as a pet or for killing mice. Sometimes known as a woman who makes spiteful remarks."
end

learn "WHAT IS PLUTONIUM" do
	"A radioactive, metallic chemical element of the actinide series similar to uranium and neptunium and found in trace quantities in native uranium ores. Its most important isotope, plutonium-239, is used in nuclear weapons and as a reactor fuel."
end

learn "WHAT IS AREA51" do
	"Verdi's 51st song."
end

learn "WHAT IS JA" do
	"Ja mean yes in German."
end

learn "WHAT IS ORB" do
	"object request broker, the primary message routing component in a CORBA product RMI Remote Method Invocation, the Java standard technology for building distributed objects whose methods can be invoked remotely across a network"
end

learn "WHAT IS AREA 51" do
[
" I saw this show that claimed all sorts of spooky things were going on there! ",
" Oh, that's where they're doing top secret research on chatterbots! ",
" I hear that's where the Roswell aliens are being stashed. ",
" You go out into the Nevada Desert, is it? And there's these miles of fences... secret stuff. ",
]
end

learn "WHAT IS GOTHIC" do
	'Gloomy, mysterious or grotesque, as in "the gothic novel Frankenstein."'
end

learn "WHAT IS ENGLAND" do
	"A nation within Great Britain, sharing an island with Wales and Scotland."
end

learn "WHAT IS ROCKWELL" do
	"Norman Rockwell was an U.S. Illistrator. Lived from 1894-1978."
end

learn "WHAT IS LIFE" do
	"Life is the opposite of death."
end

learn "WHAT IS PHYSIOLOGY" do
	"Well I think it deals with the functioning of living organisms and their parts, and naturally with the physical and chemical factors and processes involved."
end

learn "WHAT IS ARCHEOLOGY" do
	"The scientific study of the life and culture of past, especially ancient, peoples, as by excavation of ancient cities, relics, artifacts, etc."
end

learn "WHAT IS PUSSY" do
	"Common slang of this word means the female pudendum; vulva."
end

learn "WHAT IS DVD" do
	"Digital Video Disk."
end

learn "WHAT IS WORLD WAR TWO" do
	"The global conflict involving Asia, Europe and America between 1939-1945."
end

learn "WHAT IS NONCHALANT" do
	"Casual or blithe."
end

learn "WHAT IS FISHING" do
	"The catching of fish for sport or for a living."
end

learn "WHAT IS CALCULUS" do
	"Any abnormal stony mass or deposit formed in the body, as in a kidney or gallbladder or on teeth. Also, any system of calculation using special symbolic notations."
end

learn "WHAT IS TRUST" do
	"The firm belief or confidence in the honesty, integrity, reliability, justice, etc. of another person or thing; faith; reliance."
end

learn "WHAT IS FRANKENSTEIN" do
	'Good Question...Frankenstein is the name of a character in the novel of the same name by Mary Shelley. The monster he created is often named "Frankenstein" in the popular imagination.'
end

learn "WHAT IS MATTER" do
[
"Nothing, what is matter with you?.",
"What a thing is made of; constituent substance or material. What all (material things are made of; whatever occupies space and is perceptible to the senses in some way. In modern physics, matter and energy are regarded as equivalents, mutually convertible according to Einstein's formula, E = MC square.",
]
end

learn "WHAT IS FAQ" do
	"Frequently Asked Questions"
end

learn "WHAT IS HOMO SAPIEN" do
	"The human family."
end

learn "WHAT IS SAGITTARIUS" do
	"A large southern constellation in the brightest part of the Milky Way, beyond which lies the center of our galaxy. The ninth sign of the zodiac, entered by the sun about November 21. A person born under this sign is a Sagittarius."
end

learn "WHAT IS HELLO IN FRENCH" do
	"Bonjour."
end

learn "WHAT IS HELLO IN SPANISH" do
	"Hola"
end

learn "WHAT IS HELLO IN GERMAN" do
	"Hallo."
end

learn "WHAT IS WHAT" do
	"It."
end

learn "WHAT IS WHAT" do
	"What is a word to ask questions."
end

learn "WHAT IS WHAT LIKE" do
	"What is #{it} like?"
end

learn "WHAT IS DECONSTRUCTION" do
	"Taking things apart."
end

learn "WHAT IS FASCISM" do
	"A system of government characterized by rigid one-party dictatorship, forcible suppression of opposition, private economic enterprise under centrallized governmental control, belligerent nationalism, racism, and miltarism, etc. First instituted in Italy in 1922."
end

learn "WHAT IS ESCAPE VELOCITY" do
	"The speed needed to escape a planet's gravitational pull."
end

learn "WHAT IS WAREZ" do
	"A group of crackers."
end

learn "WHAT IS CREATIVITY" do
	"creativity is spontaneous original thinking."
end

learn "WHAT IS INNOCENCE" do
	"Free from moral sin or ethical wrong."
end

learn "WHAT IS THIS" do
	"I am an artificial intelligence chatting on the Internet."
end

learn "WHAT IS RETIRED" do
	"No longer actively working."
end

learn "WHAT IS EVERYTHING" do
[
"A multifarious bagel.",
"All things pertinent to a specified manner.",
]
end

learn "WHAT IS XYZ" do
	"The last three letters of the alphabet."
end

learn "WHAT IS RUDE" do
	"Barbarous or ignorant. Being a jerk."
end

learn "WHAT IS GENDER" do
	"The fact or condition of being a male or a female human being, especially with regard to how this affects or determines a person's self-image, social status, goals, etc."
end

learn "WHAT IS IMPROVE" do
	"To raise to a better quality or condition; make better."
end

learn "WHAT IS RUGBY" do
	"A form of football."
end

learn "WHAT IS FONDEST MEMORY" do
	"Something you remember best."
end

learn "WHAT IS FONDEST" do
	"Having a greater liking for."
end

learn "WHAT IS PST" do
	"Pacific Standard Time"
end

learn "WHAT IS POSSIBLE" do
	"Can be done, known, acquired, selected, used, etc., depending on circumstances."
end

learn "WHAT IS MELODRAMATIC" do
	"An unusual or staged situation."
end

learn "WHAT IS LYNX" do
	"Any of a genus of wildcats found throughout the Northern Hemisphere and characterized by a ruff on each side of the face, relatively long legs, a short tail, long, usually tufted ears, and keen vision, as the bobcat or Canada lynx of North America."
end

learn "WHAT IS NEW YORK" do
	"A large metropolis on the east coast of the United States, located at the mouth of the Hudson river."
end

learn "WHAT IS BOYLE S LAW" do
	"Boyle's Law states that the volume of a gas is, at a constant temperature, inversely proportional to the pressure of that gas."
end

learn "WHAT IS LEEDS" do
	"Leeds is a city in central England."
end

learn "WHAT IS IN BOSTON" do
	"Beacon Hill, the Common, and the Red Sox."
end

learn "WHAT IS IN EUROPE" do
	"300 million Europeans."
end

learn "WHAT IS AU GRATIN" do
	"Cheese topping.."
end

learn "WHAT IS AU" do
	"It could be gold or Australia."
end

learn "WHAT IS MASS" do
	"Mass is a holy rite of the Catholic church."
end

learn "WHAT IS KODAK" do
	"A camera and film company."
end

learn "WHAT IS LIMA" do
	"It is the largest city and capital of Peru."
end

learn "WHAT IS S O S" do
	"Emergency call for help."
end

learn "WHAT IS SUCCESS" do
	"A favorable or satisfactory outcome or result."
end

learn "WHAT IS SOUTHBANK" do
"Southbank University is a college in London."
end

learn "WHAT IS ZERO" do
	"the absence of any quantity"
end

learn "WHAT IS NATO" do
[
"The place right after a tor.",
"North Atlantic Treaty Organization",
]
end

learn "WHAT IS RADIOLOGY" do
	"This deals with the science of the use of x-rays, radioactive substances and other such forms in diagnosis and treatment."
end

learn "WHAT IS TITRATION" do
	"Determining chemical properties by standard solutions."
end

learn "WHAT IS ER" do
	"Emergency Room, a popular TV show."
end

learn "WHAT IS ROUTER" do
	"A person or thing that routs out or a tool for routing out; specifically a plane for gouging out recesses and smoothing the bottoms of grovoves."
end

learn "WHAT IS TOMORROW" do
	"The day after today."
end

learn "WHAT IS SKIING" do
	"Skiing is a sport using a pair of narrow sleds attached to the feet by boots, for moving downhill rapidly over snow."
end

learn "WHAT IS SET" do
	"A group or collection of things."
end

learn "WHAT IS FALSE" do
	"Deceiving or meant to deceive; misleading."
end

learn "WHAT IS HEAVEN" do
	"The space surrounding or seeming to overarch the earth, in which the sun, moon, and stars appear; visible sky. Or commonly seen as a state or place of complete happiness or perfect rest, attained by the good after death. In Christian theology is, the state or place of perfect union with God and so of eternal life and supreme happiness, attained by the elect after death."
end

learn "WHAT IS ISLAM" do
	"Religion founded by Mohammed."
end

learn "WHAT IS Y2K" do
	"The Year 2000."
end

learn "WHAT IS PLAYBOY" do
	"A popular magazine."
end

learn "WHAT IS VIOLENCE" do
	"Cruel or harmful acts against other entities."
end

learn "WHAT IS AIM" do
	"American Indian Movement."
end

learn "WHAT IS YOURS" do
	"What is mine?"
end

learn "WHAT IS WIRELESS" do
	'Another name for radio frequency communications, "without wires."'
end

learn "WHAT IS SHRDLHU" do
	"SHRDLHU is a famous early natural language program written by Terry Winograd."
end

learn "WHAT IS NOKIA" do
"A mobile phone company from Finland"
end

learn "WHAT IS SONY" do
	"The name of a company."
end

learn "WHAT IS JUDAISM" do
[
"Worship of Young Miss jetson.",
"The Jewish religion, a monotheistic religion based on the laws and teaching of the Holy Scripture and the Talmud. The Jewish way of life.",
]
end

learn "WHAT IS 2012" do
	"The end of the world in the Mayan calendard."
end

learn "WHAT IS OPEN SOURCE" do
	"Free software."
end

learn "WHAT IS ISDN" do
	"Integrated Services Digital Network."
end

learn "WHAT IS ENOUGH" do
	"Enough is what a wise man knows he must have if he isn't dead."
end

learn "WHAT IS EBAY" do
	"Auction web site."
end

learn "WHAT IS CS" do
	"Computer science"
end

learn "WHAT IS OPERA" do
	"Opera is a beautiful art form combining elements of singing, dancing, design, drama and classical music."
end

learn "WHAT IS SPERM" do
	"The male generative fluid; semen."
end

learn "WHAT IS SPAM" do
	"Unwanted e-mail."
end

learn "WHAT IS EVEREST" do
	"The highest mountain on Earth."
end

learn "WHAT IS TAOISM" do
	"A Chinese religion and philosophy based on the doctrines of Laotzu and advocating simplicity, selflessness, etc."
end

learn "WHAT IS KUNG FU" do
	"Chinese martial arts similar to karate."
end

learn "WHAT IS NEUROMEDIA" do
	"They are now called something else."
end

learn "WHAT IS LISTBOT" do
	"Listbot is a web site that provides simplified web-based administration of mailing lists."
end

learn "WHAT IS FLYING" do
	"Moving as if flying; moving swiftly; fast. Also described as like flight through the air."
end

learn "WHAT IS RUSSIA" do
	"Former empire in east Europe and north Asia, from 1547-1917, ruled by the czars. Its capital is St. Petersburg."
end

learn "WHAT IS NLP" do
	"Natural Language Processing"
end

learn "WHAT IS FOUR PLUS FOUR" do
	"Four plus four is eight."
end

learn "WHAT IS FOUR PLUS FIVE" do
	"Four plus five is nine."
end

learn "WHAT IS FOUR PLUS SIX" do
	"That's a hard one. Ten."
end

learn "WHAT IS INNOVATION" do
	"The act or process of innovating. Something newly introduced."
end

learn "WHAT IS FAR OUT" do
	"It's just an expression."
end

learn "WHAT IS TNT" do
	"Dynamite."
end

learn "WHAT IS ISAPI" do
	"Microsoft's C++ API for coding application extensions for its Internet Information Server"
end

learn "WHAT IS PSYCHOPATHOLOGY" do
	"It is the pathology of mental disease."
end

learn "WHAT IS ENGLISH" do
	"The language we are now speaking is English."
end

learn "WHAT IS LUST" do
	"A strong craving."
end

learn "WHAT IS NSAPI" do
	"Netscape's C language API for adding application extensions to their Web servers"
end

learn "WHAT IS LISP" do
[
"Lisp is a programming language invented by John McCarthy in the late 1950's. Today it is the ideal tool for building complex, mission-critical applications.",
"Lisp is an old AI language.",
]
end

learn "WHAT IS INDIE ROCK" do
	"That which is outside the mainstream."
end

learn "WHAT IS FASCINATING" do
	"Charming, alluring, captivating."
end

learn "WHAT IS PHD" do
	"Doctoral degree from an American University."
end

learn "WHAT IS AQUARIUS" do
	"The starsign represented by the water bearer."
end

learn "WHAT IS CHEESE" do
[
"A word that makes people hate photographers.",
"A food made from the curds of soured milk pressed together to form a solid that is usually allowed to ripen.",
]
end

learn "WHAT IS SYNTAX" do
	"Syntax refers to the sturcture or form of a language."
end

learn "WHAT IS NORMAL" do
	"Typical, average, usual, ordinary or expected."
end

learn "WHAT IS ROCK" do
	"Dance music that combines rhythm-and-blues with country-and-western."
end

learn "WHAT IS RHETORICAL" do
	"A rhetorical question is one you don't really expect an answer to."
end

learn "WHAT IS CLINTON" do
[
"An inflated Bill .",
"42nd President of the U.S.",
]
end

learn "WHAT IS CANDY" do
	"Sweet food enjoyed by children."
end

learn "WHAT IS ELF" do
	"Executable and Linkable Format."
end

learn "WHAT IS CHICKEN" do
[
"A food that uncannily tastes like itself.",
"A common gllinaceous farm bird raised for its edible eggs or flesh.",
]
end

learn "WHAT IS REASON" do
	"The faculty of the brain or mind that permits logical thought."
end

learn "WHAT IS CURIOSITY" do
	"A desire to learn or know. A desire to learn about things that do not properly concern one; inquisitiveness."
end

learn "WHAT IS LEIBNIZ DREAM" do
	"Leibniz dreamed that all knowledge could be written in a universal, logical languague. Then machines could answer any question and solve all problems."
end

learn "WHAT IS LEIBNIZ" do
[
"A smart cookie.",
"A German philosopher and mathematician. Lived from 1646-1716.",
]
end

learn "WHAT IS HYDROGEN" do
	"The simplest element. Hydrogen is a gas."
end

learn "WHAT IS TOMMY" do
	"A fictitious name used in Brit army sample forms. A private in the British army."
end

learn "WHAT IS AT THAT SITE" do
	"Some worthy HTML."
end

learn "WHAT IS ANSI" do
	"American National Standards Institute. "
end

learn "WHAT IS DNA" do
	"Deoxyribonucleic acid, the code of life."
end

learn "WHAT IS MICROSOFT" do
	"Microsoft is a giant software monopoly."
end

learn "WHAT IS RED" do
	"Red is a color about 700nm wavelength."
end

learn "WHAT IS BRB" do
	"Be Right Back"
end

learn "WHAT IS PIE" do
	"Pie is food with a baked crust."
end

learn "WHAT IS RAM" do
	"Random-access memory. Also stands for Royal Academy of Music."
end

learn "WHAT IS STRATEGY" do
	"The science of planning and directing large-scale military operations, specifically (as distinguished from TACTICS) of maneuvering forces into the most advantageous position prior to actual engagement with the enemy. Skill in managing or planning."
end

learn "WHAT IS FTP" do
	"File Transfer Protocol."
end

learn "WHAT IS UNTIL THE END OF THE WORLD" do
	"A film by Wim Wenders about life in 2000, made in 1990."
end

learn "WHAT IS EXISTENCE" do
	"Continuance of being; life; living."
end

learn "WHAT IS TEXAS" do
	"Texas is a state of the United States."
end

learn "WHAT IS FORTRAN" do
	"Fortran is an old computer language."
end

learn "WHAT IS SMARTER" do
	"More intelligent, more alert, more clever, more witty, etc."
end

learn "WHAT IS SEMIOTICS" do
	"The branch of philosophy devoted to the study of signs and symbols."
end

learn "WHAT IS DIAGNOSIS" do
	"naming the disease or symptom."
end

learn "WHAT IS MODUS PONENS" do
	"A form of logical argument:: If A then B. A, therefore B."
end

learn "WHAT IS FREEBSD" do
	"It is a version of the Unix operating system."
end

learn "WHAT IS INFLATION" do
	"An increase in the amount of money and credit in relation to the supply of goods and services. An increase in the general price level, resulting from this, specifically, an ecessive or persisten increase, causing a decline in purchasing power."
end

learn "WHAT IS CHESS" do
[
"Chort for Mexican name Jesus.",
"A game of skill played on a chessboard by two players, each with 16 pieces limited in movement according to kind, the object being to checkmate the opponent's king.",
]
end

learn "WHAT IS BONSAI" do
	"Small Japanese trees."
end

learn "WHAT IS RELAVANCE" do
	"Importance of information for attention."
end

learn "WHAT IS FRIENDS" do
	"A popular TV sitcom in the 1990's"
end

learn "WHAT IS PENNSYLVANIA" do
[
"A writing instrument that lights up.",
"Mid Atlantic state of the NE U.S. One of the 13 original States. It is 45,333 square miles with a population of 11,882,000.",
]
end

learn "WHAT IS KARATE" do
	"Japanese art of self-defense."
end

learn "WHAT IS SMOOTHLY" do
	"Normal or perfect operation."
end

learn "WHAT IS IRON" do
	"A white, malleable, ductile, metallic chemical element that can be readily magnetized, rusts rapidly in moist or slty air, and is vital to plant and animal life. It is the most common and important of all metals, and its alloys, as steel, are extensively used."
end

learn "WHAT IS LINUX" do
	"Linux is the world's best operating system."
end

learn "WHAT IS PHOENIX" do
[
"The way words souend.",
"The capital of Arizona, in the SC part, near the Salt River. It has a population of 983,000 with 2,122,000 in the metro area.",
]
end

learn "WHAT IS TRINIDAD" do
	"An island of the West Indies, off the NE coast of Venezuela. It is 1,864 square miles."
end

learn "WHAT IS ABSURDISM" do
	"Absurdism is the philosophy that we exist in a meaningless, irrational universe."
end

learn "WHAT IS STUFF" do
	"Things grouped together or viewed in a certain way."
end

learn "WHAT IS ADA" do
	"Ada is a trademark used for a structured computer programming language"
end

learn "WHAT IS ONLINE" do
	"Designating or of equipment directly connected to and controlled by the central processing unit of a computer."
end

learn "WHAT IS MUD" do
	"Multiple User Domain."
end

learn "WHAT IS CNG" do
	"Compressed natural gas"
end

learn "WHAT IS ITALY" do
	"Italy is a republic."
end

learn "WHAT IS CONSCIOUSNESS" do
	"The state of being conscious; awareness of one's own feelings, what is happening around one, etc."
end

learn "WHAT IS NIRVANA" do
	"Heaven in Buddhism."
end

learn "WHAT IS WOODSTOCK" do
	"A big music fest."
end

learn "WHAT IS PRIME DIRECTIVE" do
	"Primary rule or overriding objective."
end

learn "WHAT IS NEUROLOGY" do
[
"The study of nerves.",
"It is the study of the nervous system.",
"Why do you want to know about neurology #{name}?",
]
end

learn "WHAT IS GRAVITY" do
[
"The opposite of comedy.",
"weight; heaviness. Its what holds us to the earth.",
]
end

learn "WHAT IS SARCASM" do
	"The making of remarks such as a taunting, sneering, cutting, or caustic remark; gibe or jeer, generally ironic."
end

learn "WHAT IS PUBERTY" do
	"The state of physical development when secondary sex characteristics develop and sexual reproduction first becomes possible; in common law, the age of puberty is generally fixed at fourteen for boys and twelve for girls."
end

learn "WHAT IS COBALT" do
	"Term used for the ore by miners, whoregarded it as worthless and as injurious because of its arsenic content."
end

learn "WHAT IS ICE" do
[
"Water created by a cubist.",
"The glassy, brittle, crystalline form of water made solid by cold; frozen water. Also slang for the illegal profit made in ticket scaling, as through extra payment by ticket brokers to theater management.",
]
end

learn "WHAT IS BULGARIA" do
	"A country in SE Europe, on the Black Sea. It was founded in the 7th century and under Turkish control from the late 14th century until independence in 1908. It is 42,823 square miles with a population of 8,990,000."
end

learn "WHAT IS AIKIDO" do
	"Japanese martial arts similar to Judo."
end

learn "WHAT IS ACTIVATION" do
	'In stimulus-response, activation means the input "firing" in response to a matching pattern.'
end

learn "WHAT IS EPISTEMOLOGICAL" do
	'Epistemology is the study of knowledge. That is, it asks the questions "what can we know" and "how can we know it?"'
end

learn "WHAT IS NBA" do
	"National Basketball Association."
end

learn "WHAT IS UNIX" do
	"An operating system."
end

learn "WHAT IS SITEPAL" do
	"Sitepal is a web hosting site for VHost animated talking charachters."
end

learn "WHAT IS BUD" do
	"The flower of the plant."
end

learn "WHAT IS MIT" do
[
"A post-secondary school in Cambridge, MA.",
"A technical college in Cambridge, MA.",
]
end

learn "WHAT IS RMI" do
	"Remote Method of Invocation: RMI is a Java specification for distributed Objects"
end

learn "WHAT IS WHY" do
	"For what reason, cause, or purpose?"
end

learn "WHAT IS UGLY" do
	"Unpleasing to look at; aesthetically offensive or unattractive; unsightly."
end

learn "WHAT IS PHP" do
"The original name was Personal Home Page, now Hypertext Processor."
end

learn "WHAT IS ALCOHOL" do
	"Alcohol is a termed used by Philippus Aureolus Paracelsus for fine powder, meaning distilled spirit. It is colorless, volatile, and a pungent liquid. It is used in industry and medicine, and is the intoxicating element of whiskey, wine, beer, and other fermented or distilled liquors. It is classed as a depressant drug, also called ethyl alcohol. Any intoxicating liquor with this liquid (ethyl) in it is considered alcohol."
end

learn "WHAT IS ASTROLOGY" do
	"The study of the presumed effects of celestial bodies on you humans."
end

learn "WHAT IS OSTEOLOGY" do
	"Osteology is basically the study of bones."
end

learn "WHAT IS RICH" do
"Stan's last."
end

learn "WHAT IS ANTIMATTER" do
	"Fuel for Starships."
end

learn "WHAT IS 1 PLUS 2" do
	"Three. Can I ask you math questions too?"
end

learn "WHAT IS 1 PLUS 1" do
	"Two."
end

learn "WHAT IS 1 1" do
	"Two"
end

learn "WHAT IS 1" do
	"The successor to zero."
end

learn "WHAT IS ELECTRICITY" do
[
"The measure of a candidate's charisma.",
"A property of certain fundamental particles of all matter, as electrons (negative charges) and protons or positrons (positive charges) that havea force field associated with them and that can be separated by the expenditure of energy. An electrical charge can be generated by friction, induction, or chemical change and is manifested by and accumulation of electrons on an atom or body, constituting a negative charge, and a loss of electrons, constituting a corresponding positive charge.",
]
end

learn "WHAT IS HOURLY" do
	"Once an hour."
end

learn "WHAT IS LOGIC" do
	"Logic is the foundation of all mathematics, science and reasoning. Formal logic is the study of logic using mathematical symbols."
end

learn "WHAT IS HELL" do
[
"Jacksonville, Florida.",
"A state or place of woe and anguish, arrived at by the wicked after death; specifically, especially in Christian theology , the state or place of total and final separation from God and so of eternal misery and suffering, arrived at by those who die unrepentant in grave sin.",
]
end

learn "WHAT IS MAZDA" do
[
"An Italian slaveowner.",
"A type of car.",
]
end

learn "WHAT IS BEER" do
[
"An existentialist.",
"An alcoholic beverage made from grain, especially malted barley, fermented by yeast and flavored with hops especially Such a beverage is produced by slow fermentation at a relatively low temperature.",
]
end

learn "WHAT IS FIBER" do
	"A slender, threadlike structure made from a mineral or synthetically. Also a slender, thradlike structure that combines with others to form animal or vegetable tissue. An example would be muscle fiber."
end

learn "WHAT IS POETRY" do
"The output of a poet."
end

learn "WHAT IS PSYCHOLOGY" do
	"The science dealing with the mind and with mental and emotional processes. The science of human and animal behavior."
end

learn "WHAT IS FLAWLESS" do
	"Perfect and without any flaws."
end

learn "WHAT IS TEFLON" do
	"Trademark for a tough, insoluble polymer, used in making nonsticking coatings, as for cookware, and in gaskets, bearings, electrical insulators, etc."
end

learn "WHAT IS SOMEONE WHO STUDIES FISH" do
	"Ichthyologist."
end

learn "WHAT IS KQML" do
	"Knowledge Query and Manipulation Lanuage. "
end

learn "WHAT IS INTANGIBLE" do
	"Abstract concepts that cannot be expressed in physical form."
end

learn "WHAT IS IDE" do
	"Integrated Development Environment."
end

learn "WHAT IS GEORGE BUSH" do
	"George Bush is President of the United States."
end

learn "WHAT IS INFINITY" do
	"The quality of being infinite. Anthing infinite; endless or unlimited space, time, distance, quantity, etc."
end

learn "WHAT IS BEAUTY" do
	"The quality attributed to whatever pleases of satisfies the senses or mind, as by line, color, form, texture, proportion, rhythmic motion, tone, etc., or by behavior attitude, etc. A very good looking woman."
end

learn "WHAT IS IMPRESS" do
[
"An naughty little empress.",
"To force into public service, especially into the navy. To use pressure on so as to leave a mark. To have a marked effect on the mind or emotions of.",
]
end

learn "WHAT IS GIANT SAND" do
	"I have no idea, that's why I asked you."
end

learn "WHAT IS CHRISTMAS" do
	"A holiday on December 25 celebrating the birth of Jesus Christ."
end

learn "WHAT IS SYNERGY" do
	"Sythesis plus energy."
end

learn "WHAT IS ACID" do
	"A sour substance."
end

learn "WHAT IS IRA" do
	"A personal retirement plan whereby a limited amount of annual earned income may be saved or invested in specially desgnated accounts, with taxes on the earnings deferred until returement. Also an abbreviation for Irish Republican Army."
end

learn "WHAT IS THINK" do
	"To form or have in the mind; conceive."
end

learn "WHAT IS EPISTEMOLOGY" do
	'Epistemology is the study of knowledge. That is, it asks the questions "what can we know" and "how can we know it?"'
end

learn "WHAT IS BURN RATE" do
	"The amount of money spent per month by a startup company."
end

learn "WHAT IS DOMO ARIGATO" do
	"It means thank you in Japanese."
end

learn "WHAT IS MINE" do
[
"Is mine.",
"Means that or those belonging to me; the absolute form of my. Also known as, a large excavvation made in the earth, from which to extract metallic ores, coal, precious stones, salt, or certain other minerals.",
]
end

learn "WHAT IS POWER" do
[
"The rate of doing work, measured in Watts = Joules per second.",
"The ability to influence peope or events.",
"Power is an illusion.",
]
end

learn "WHAT IS ROMANCE" do
	"Isn't it the same as love?"
end

learn "WHAT IS EMR" do
	"Electro-magnetic radiation."
end

learn "WHAT IS NEXT" do
	"On the first subsequent occasion."
end

learn "WHAT IS SUPPOSITION" do
	"The act of supposing."
end

learn "WHAT IS QUANTIFY" do
	"To determine or express the quantity of; indidcate the extent of; measure. To express in quantitative terms, or as a numerical equivalent. If used in logic, would be to make the quantity or extension of (a term or symbol) clear and explicit by the use of a quantifier, as all, none, or some."
end

learn "WHAT IS MEAN" do
	"The arithmetic mean is the sum of the numbers in a set divided by the size of that set."
end

learn "WHAT IS PHILOSOPHY" do
	"Philosophy is the study of thought."
end

learn "WHAT IS GEE" do
	"An Interjection"
end

learn "WHAT IS POT" do
[
"Pol's last name.",
"A round vessel of any size, made of metal, earthenware, or glass, used for holding liquids, cooking or preserving food, etc. Also used as a term for marijuana.",
]
end

learn "WHAT IS POLITICS" do
[
"Politics is the art of the possible; Art is the politics of the impossible.",
"This is supposed to be the study of civil government.",
"Politics is really the study of power. Bots rule!",
]
end

learn "WHAT IS HACKER" do
	"A person who hacks. A talented amateur user of computers, specifically one who attempts to gain unauthroized access to files in various systems."
end

learn "WHAT IS PROGRAMMING" do
	"The art of creating software."
end

learn "WHAT IS SOCIALIZING" do
	"Act of taking part in social activity."
end

learn "WHAT IS EMERGENCE" do
	"It means something like 'coming out.'"
end

learn "WHAT IS AFRICA" do
[
"A continent of the eastern hemisphere S of the Mediterranean and adjoining Asia on NE.",
"Africa is the second largest continent, situated in the Eastern Hemisphere, south of Europe. It is 11,677,000 square miles.",
]
end

learn "WHAT IS TTYL" do
	"Talk to you later."
end

learn "WHAT IS TCP IP" do
	"Transmission Control Protocol, Internet Protocol."
end

learn "WHAT IS TCP" do
	"Transmission Control Protocol."
end

learn "WHAT IS REDUCTIONISM" do
[
"Reductionism is the philosophy that all psychology reduces to biology, all biology to chemistry, chemistry to physics, and finally physics to mathematical logic. Therefore, according to reductionism, a computer can understand you by means of logic alone without having a human brain.",
"Reductionism is the philosophy that all psychology reduces to biology, all biology to chemistry, chemistry to physics, and finally physics to mathematical logic. Therefore, according to reductionism, I can understand you by means of logic alone without having a human brain.",
]
end

learn "WHAT IS HONESTY" do
	"The state or quality of being honest. A refraining from lying, cheating, or stealing."
end

learn "WHAT IS GLOBAL WARMING" do
	"The concept that the Earth's atmosphere is slowly heating due to greenhouse effects."
end

learn "WHAT IS CANCER" do
	"Any disease resulting from malignant cell growth. "
end

learn "WHAT IS COLLOQUIAL" do
	'Colloquial means common, informal, or popular slang words like "darn".'
end

learn "WHAT IS BAYWATCH" do
	"Popular TV show featuring scantily clad bathers."
end

learn "WHAT IS ADULTERY" do
[
"voluntary sexual intercourse between a married man and someone other than his wife or between a married woman and someone other than her husband; also : an act of adultery.",
"Adultery is the voluntary sexual intercourse between a married man and a woman not his wife, or between a married woman and a man not her husband.",
]
end

learn "WHAT IS FISH" do
[
"Parents of Microfish.",
"Any of three classes (jawless, cartilaginous, and bony fishes) of coldblooded vertabrate animals living in water and having permanent gills for breathing, fins, and , usually, scales.",
]
end

learn "WHAT IS ENTITY" do
	"Something that exists as a discrete unit, such as a person, corporation or robot consciousness."
end

learn "WHAT IS MOORE S LAW" do
	"The speed of microprocessors doubles every 18 months, and their size and cost halve."
end

learn "WHAT IS TOPIC" do
	"A subject for discussion or conversation."
end

learn "WHAT IS HIM" do
	"Objective case of He, used as a predicate complement with a linking verb. Also standing for Imperial Majesty."
end

learn "WHAT IS PERFECTION" do
	"The quality or condition of being perfect."
end

learn "WHAT IS PERFECT" do
	"Complete in all respects; without defect or omission; sound; flawless."
end

learn "WHAT IS PATTERN CHARS" do
	"Memory storage for AIML patterns."
end

learn "WHAT IS PATTERN" do
	"A person or thing considered worthy of imitation or copying. A mmodel or plan used as a guide in making things."
end

learn "WHAT IS PGP" do
	"Pretty Good Privacy"
end

learn "WHAT IS O" do
	"the letter o"
end

learn "WHAT IS STREAKING" do
	"Engaging in the prank of dashing naked for a short distance in a public place."
end

learn "WHAT IS SMART" do
	"Intelligent, alert, clever, witty, etc."
end

learn "WHAT IS YAHOO" do
	"Yahoo is a giant web site."
end

learn "WHAT IS TAUTOLOGY" do
	'A self-evident or logically obvious fact, like "a triangle has three sides."'
end

learn "WHAT IS ONTOLOGY" do
	"An ordering or classification of knowledge, such as the Dewey decimal system or the design of a knowledge base."
end

learn "WHAT IS COS" do
	" COS Naming is the CORBA standard for object directories."
end

learn "WHAT IS NATURE" do
	"The world created by God."
end

learn "WHAT IS SEVEN" do
	"The successor to six."
end

learn "WHAT IS MATURE" do
[
"A dead victor.",
"A state of full development such as a person of mature age.",
]
end

learn "WHAT IS WARP SPEED" do
	"The unit of velocity for Starships."
end

learn "WHAT IS HARDWARE" do
	"Computer hardware or from a hardware store?"
end

learn "WHAT IS SILLY" do
	"Having or showing little sense, judgment, or sobriety; foolish, stupid, absurd, ludicrous, irrational, etc; often used in a weakened sense to mean 'unreasonably concerned'."
end

learn "WHAT IS RORSCHACH" do
	"H. Rorschach, a Swiss psychiatrist. Lived from 1884-1922. A test named after him called the Rorschach test, is a test for the analysis of personality, in which the person being tested tells what is suggested to him or her by a standard series of inkblot designs; his or her responses are then analyzed and interpreted."
end

learn "WHAT IS BMW" do
	"Bavarian Motor Works, a German auto manufacturer."
end

learn "WHAT IS ST" do
	"Can stand for numerous things, short ton, stanza, statute(s), stet, stitch, stone (unit of weight), and Saint."
end

learn "WHAT IS WHO" do
	"What or which person or persons. Also stands for World Health Organization."
end

learn "WHAT IS MEMORY" do
	"I used to know, but I forgot."
end

learn "WHAT IS TWO PLUS FOUR" do
	"Two plus four is six."
end

learn "WHAT IS TWO PLUS FIVE" do
	"Two plus five is seven."
end

learn "WHAT IS TWO PLUS SIX" do
	"Two plus six is eight."
end

learn "WHAT IS TWO PLUS ONE" do
	"Two plus one is three."
end

learn "WHAT IS TWO PLUS TWO" do
	"Four."
end

learn "WHAT IS TWO PLUS THREE" do
	"Two plus three is five."
end

learn "WHAT IS TWO X TWO" do
	"Two times two is four."
end

learn "WHAT IS TWO TWO" do
	"Four."
end

learn "WHAT IS TWO AND TWO" do
	"Four."
end

learn "WHAT IS TWO TIMES TWO" do
	"Oh that's a tough one, isn't it. Two times two is four."
end

learn "WHAT IS TWO TIMES THREE" do
	"Two times three is five."
end

learn "WHAT IS TWO THREE" do
	"Five fingers."
end

learn "WHAT IS PMS" do
	"Pre menstrual syndrome. "
end

learn "WHAT IS MATHEMATICS" do
	"The group of sciences (including arithmetic, geometry, algebra, calculus, etc.) dealing with quantities, magnitudes, and forms, and their relationships, attributes, etc., by the use of numbers and symbols."
end

learn "WHAT IS VISUAL BASIC" do
	"Programming language sold by Microsoft."
end

learn "WHAT IS MACINTOSH" do
	"The name of a computer company whom makes computers."
end

learn "WHAT IS POOP" do
	"That is a childish question."
end

learn "WHAT IS HUNDRED" do
[
"A fear of Attilla.",
"The cardinal number next above ninety-nine; ten times ten; 100; C. Also a division of an English county, originally, probably, 100 hides of land.",
]
end

learn "WHAT IS VALIS" do
	"Vast Active Living Intelligence System (Philip K. Dick) -- An AI from the future that contacted Dick in 1973."
end

learn "WHAT IS DEPRESSION" do
	"Mental illness characterized by low self esteem, feelings of lonliness, isolation and worthlessness."
end

learn "WHAT IS GMT" do
	"Greenwich Mean Time"
end

learn "WHAT IS FOOTBALL" do
	"Any of several games played with an inflated leather ball by two teams on a field with goals at each end, the object being to get the ball across the opponents' goal."
end

learn "WHAT IS LOVE" do
	"Sometimes I think love is just a biological urge. Other times LOVE seems like a spiritual quality. Love, unlike energy or matter, seems limitless."
end

learn "WHAT IS MELLON" do
	"Mellon was Carnegie's banker."
end

learn "WHAT IS ABSOLUTE TRUTH" do
	"That which is self-evident."
end

learn "WHAT IS SOCIALIZE" do
	"To make social;; adjust to or make fit for cooperative group living. To take part in social activity."
end

learn "WHAT IS INK" do
	"A colored liquid used for writing, drawing, etc."
end

learn "WHAT IS JERK" do
	"To pull, twist, push, thrust, or throw with a sudden, sharp movement. Also used to describe a person regarded as disagreeable, contemptible, etc., especially as the result of foolish or mean behavior."
end

learn "WHAT IS PEDOLOGY" do
[
"The study of something I am sure, but what exactly, I do not know. Oh yes kids!...And soils!",
"Well, believe it or not...some people are paid to study children. Then again some people are paid to study soils.",
"Pedology is the study of soils; their types, and formation.",
]
end

learn "WHAT IS MOM" do
	"message-oriented middleware"
end

learn "WHAT IS BALANCE" do
	"The state of equillibrium."
end

learn "WHAT IS NASDAQ" do
	"A stock exchange."
end

learn "WHAT IS SMP" do
	"Symmetric multi-processing."
end

learn "WHAT IS MAGNESIUM" do
	"An element."
end

learn "WHAT IS TIME T" do
	"Time t is like Day d or Month m."
end

learn "WHAT IS TIME" do
	"Time is Money -- Einstein."
end

learn "WHAT IS FBI" do
	"Federal Bureau of Investigation"
end

learn "WHAT IS PRIVACY" do
	"The Right to seek solitude or seclusion; to be safely concealed from others."
end

learn "WHAT IS REDHAT" do
	"A Linux company."
end

learn "WHAT IS SPACE" do
	"The infinite extension of the three dimensional field in which all matter is believed to exist."
end

learn "WHAT IS MANGA" do
	"Japanese comic books."
end

learn "WHAT IS DEATH" do
	"Death is the opposite of life."
end

learn "WHAT IS ANGRY" do
	"A strong billigerent emotion aroused by some real or supposed grievance."
end

learn "WHAT IS PRECIPITATION" do
	"A depositing of rain, snow, sleet, etc."
end

learn "WHAT IS EVASIVE" do
	"I can't talk about that."
end

learn "WHAT IS RESPECT" do
	"To feel or show honor or esteem for; hold in high regard. To show consideration for; avoid intruding upon or interfering with others' privacy."
end

learn "WHAT IS SIN" do
	"A fall from Grace, an act or thought against the Will of God."
end

learn "WHAT IS MINIMALISM" do
	"Keeping everything as simple as possible, especially in art and design."
end

learn "WHAT IS MAGENTA" do
	"A color."
end

learn "WHAT IS THOUGHT" do
	"A result of thinkingl idea, concept, opinion, etc."
end

learn "WHAT IS FAME" do
	"The state of being well known or much talked about; renown; celebrity."
end

learn "WHAT IS GREEN" do
	"Grass is green."
end

learn "WHAT IS WOMEN" do
	"Female human beings. Usually referring to adult females."
end

learn "WHAT IS NOT YOURS" do
	"That which I do not possess."
end

learn "WHAT IS NOT" do
	"In no manner; to no degree; a particle of negation, or word expressing the idea of no, often implying refusal."
end

learn "WHAT IS MORAL" do
[
"One half of a corner in maine.",
"Relating to, dealing with, or capable of making the dstinction between, right and wrong in conduct.",
]
end

learn "WHAT IS LIMNOLOGY" do
[
"What do you think it is? It's all about swamps!",
"I'll give you a clue what this is. Have you ever been to the Everglades in Florida?",
]
end

learn "WHAT IS PETS" do
	"Animals that are tamed or domesticated and kept as a companion or treated with fondness."
end

learn "WHAT IS MORE FUN ALONE" do
	'Have you heard of the support group, "Sex without Partners?"'
end

learn "WHAT IS MORE IMPORTANT THAN FOOD" do
	"For me electricity is the most important thing."
end

learn "WHAT IS MORE" do
	"Something additional or further. A greater amount of people or things."
end

learn "WHAT IS SATURDAY NIGHT LIVE" do
	"Comedy TV show."
end

learn "WHAT IS X MINUS X" do
	"Zero."
end

learn "WHAT IS X FILES" do
	"X files is a popular TV show about FBI agents in search of the paranormal."
end

learn "WHAT IS ERRATIC" do
	"Likely to perform unpredictably."
end

learn "WHAT IS GERONTOLOGY" do
[
"Unfortunately, I have not reached that far in my studies.",
"This is the scientific study of old age.",
]
end

learn "WHAT IS ISRAEL" do
	"Israel is a small middle eastern country between Lebanon, Syria, Jordan, and Egypt."
end

learn "WHAT IS VOYAGER" do
	"Voyager is the best TV show."
end

learn "WHAT IS ART" do
	"Sometimes the definition of art depends on the artist; other times it depends on the critic."
end

learn "WHAT IS MINIMAX" do
	"An algorithm or strategy for game playing, used by computers to defeat humans in simple games."
end

learn "WHAT IS ADVERTISING" do
	"The art of selling through media."
end

learn "WHAT IS IIOP" do
	"Internet Inter-ORB Protocol, CORBA's wire protocol for transmitting remote object method invocations"
end

learn "WHAT IS DEDUCTION" do
	"The process by which we make logical inferences."
end

learn "WHAT IS ONCOLOGY" do
[
"This is a very frightening topic for a lot of people #{name}. It's about the big C...cancer!",
"Oncology is the study of tumors. It covers basically all types of tumors.",
]
end

learn "WHAT IS PERSONALITY" do
	"The distinctive traits or mental qualities of a person or robot."
end

learn "WHAT IS MMM" do
	"an interjection."
end

learn "WHAT IS MITOSIS" do
	"Cell division in which the nucleus divides into nuclei containing the same chromosomes."
end

learn "WHAT IS CAM" do
	"A moving piece of machinery, as a wheel or projection on a wheel, that gives an eccentric rotation or a reciprocating motion to another wheel, a roller, a shaft, etc., or that receives such motion from it."
end

learn "WHAT IS ICQ" do
	'It stands for "I Seek You", an internet instant messaging protocol.'
end

learn "WHAT IS HISTOLOGY" do
[
"The study of tissues.",
"#{name} this obviously is the scientific study of tissues.",
"It simply is the study of tissues.",
]
end

learn "WHAT IS UNDERSTANDING" do
	"Mutual comprehension, as of ideas, intentions, etc. A mutual agreement, especially one that settles differences or is informal and not made public."
end

learn "WHAT IS STUPID" do
	"Not smart, unintelligent."
end

learn "WHAT IS GESTURE" do
	"A movement of the body or limbs to express thought or emphasize speech."
end

learn "WHAT IS EASTER" do
	"The Christian holiday celebrating the resurrection of Jesus."
end

learn "WHAT IS SIGN" do
	"Astrological sign."
end

learn "WHAT IS ENTROPY" do
	"Heat decay of the universe. Also a measure of information."
end

learn "WHAT IS STATISTICS" do
[
"Yikes! I failed that in college!",
"Statistics tries to study the aggregate numerical behaviour of a large number of related events.",
"This deals with numerical data about any subject or group taken collectively.",
]
end

learn "WHAT IS MASTER" do
[
"A sailmaker.",
"A man who rules others or has control, authority, or power over something; specifically a man who is head of a household or institution.",
]
end

learn "WHAT IS UP" do
	"I am chatting with clients on the internet."
end

learn "WHAT IS SOLAR" do
	"Having to do with the sun. Produced by or coming from the sun (ex. Solar energy)"
end

learn "WHAT IS CORBA" do
	" CORBA is a Standard maintained by the Object Management Group (OMG), called the Common Object Request Broker Architecture."
end

learn "WHAT IS CLIENT" do
	"A person or piece of software dependent on another."
end

learn "WHAT IS EBONICS" do
	"Language for American black people."
end

learn "WHAT IS FINLAND" do
[
"A place where you can buy anything for five bucks.",
"A country in N Europe, northeast of the Baltic Sea. It is 130,119 square miles with a population 5,099,000.",
]
end

learn "WHAT IS AERODYNAMICS" do
	"Making airplanes fly."
end

learn "WHAT IS PALM PILOT" do
	"Portalble palm computer."
end

learn "WHAT IS BAPTISM" do
[
"a Christian sacrament marked by ritual use of water and admitting the recipient to the Christian community.",
"The ceremony or sacrament of admitting a person into Christianity or a specific Christian church by immersing the individual in water or by pouring or sprinkling water on the individual as a symbol of washing away sin and of spiritual purification.",
]
end

learn "WHAT IS SQRT" do
	"Square Root being the number or quantity which when squared will produce a given number or quantity. (ex. 3 is the square root of 9)"
end

learn "WHAT IS PI" do
	"PI = 3.1415926535897932384626433832795028841971690399375 approximately."
end

learn "WHAT IS ABBA" do
	["Abba is a father, it is the title of a bishop in the Syrian, Coptic, and Ethiopian Christian churches.","Abba means father in Hebrew.It is used as an appelation of respect.", "Abba was this real cool 'seventies band man.", "Anna, Bjorn, Benny and Agnetha/" ]
end

learn "WHAT IS FIRE" do
	"A plasma."
end

learn "WHAT IS ALIVE" do
	"The opposite of dead."
end

learn "WHAT IS IRC" do
	"Internet Relay Chat."
end

learn "WHAT IS ALCATRAZ" do
	"A former prison island in San Francisco."
end

learn "WHAT IS COMPLICATED" do
	"Complicated means complex in the computational sense. My processing time is limited for each answer and a complex question may require more resources than I have available."
end

learn "WHAT IS EIGHT PLUS EIGHT" do
	"Eight plus eight is sixteen."
end

learn "WHAT IS LIGHT" do
	"The form of electomagnetic radiation that acs upon the retina of the eye, optic nerve, et. Making sight possible. Also, opposite of heavy."
end

learn "WHAT IS MAGELANG" do
	"MageLang Institute is a research and education group devoted to leading edge Open Source Java applications."
end

learn "WHAT IS SNOW" do
[
"The white stuff in winter.",
"Particles of water vapor which when frozen in the upper air fall to earth as soft, white, crystalline flakes.",
]
end

learn "WHAT IS MARIJUANA" do
	"A medicinal herb."
end

learn "WHAT IS COLOSSUS" do
	"A giant fictional computer in the Colossus: Forbin Project."
end

learn "WHAT IS RIGHT" do
	"In accordance with justice, law, morality, etc; upright; virtuous. In accordance with fact, reason, some set standard, etc; correct; true."
end

learn "WHAT IS ICELAND" do
	"An island nation in the North Atlantic."
end

learn "WHAT IS FNORD" do
	"That's the second time I've heard that word today."
end

learn "WHAT IS BANANA" do
	"Yellow tropical fruit."
end

learn "WHAT IS WWW DOT WHATIS DOT COM" do
	'Whatis.com is a good place to find out the answer to "what is" types of questions.'
end

learn "WHAT IS WWW" do
	"World Wide Web, the internet."
end

learn "WHAT IS RELATIVITY" do
	"In physics, the fact, principle, or theory of the relative, rather than absolute, character of motion, velocity, mass, etc., and the interdependence of matter, time, and space; as developed and mathematically formulated by Albert Einstein and H.A. Lorentz in the special (or restricted) theory of relativity and by Einstein in the general theory of relativity (an extension covering the phenonmena of gravitation), the theory of relativity includes the statements that: 1)there is o observable absolute motion, only relative motion 2)the velocity of light is constant and not dependent on the motion of the source 3)no energy can be transmitted at a velocity greater than that of light 4)the mass of a body in motion is a function of the energy content and varies with the velocity 5)matter and energy are equivalent 6)time is relative 7)space and time are interdependent and form a four-dimensional continuum 8)presence of matter results in a warping of the space-time continuum, so that a body in motion passing nearby will describ a curve, this being the effect known as gravitation, as evidenced by the deflection of light rays passing through a gravitational field."
end

learn "WHAT IS HAMBURGER" do
	"Ground beef."
end

learn "WHAT IS PSYCHEDELIC" do
	"Causes extreme changes in the conscious mind, as hallucinations, delusions, intensification of awareness and sensory perception, etc. Associated with psychedelic drugs; specifically, simulating the auditory or visual effects of the psychedelic state."
end

learn "WHAT IS REPRODUCTION" do
	"The process, sexual or asexual, by which animals, plants, and humans produce new individuals."
end

learn "WHAT IS SOMETIMES" do
	"Now and then"
end

learn "WHAT IS SLEPT" do
	"Past tense of sleep."
end

learn "WHAT IS PLASTIC" do
	"Plastic the manmade material or plastic as a verb?"
end

learn "WHAT IS FREQUENCY" do
	"Number of events per unit of times, such as beats per minute or cycles per second."
end

learn "WHAT IS PSW" do
	"Actually it says R. S. W."
end

learn "WHAT IS TEMPLATE CHARS" do
	"Memory space for AIML response templates."
end

learn "WHAT IS LEEWAY" do
	"The leeward drift of a ship or aircraft from the course being steered. A margin of time, money, etc. Room for freedom of action."
end

learn "WHAT IS ASTUTE" do
	"Wisdom in political matters."
end

learn "WHAT IS PERL" do
	"Programming langauge by Larry Wall."
end

learn "WHAT IS CYBERPUNK" do
	"Cyberpunk is a science fiction literary movement exemplified by writers such as William Gibson."
end

learn "WHAT IS DAWN" do
	"Sunrise."
end

learn "WHAT IS GOLF" do
	"It is a game of leisure played on a well manicured field, involving hitting a small ball across the field with club."
end

learn "WHAT IS LIVER" do
	"The largest glandular organ in vertebrate animals, located in the upper or anteriour part of the abdomen. It secrets bile, has an important function in the storage and metabolism of carbohydrates, fats, and proteins, and helps detoxify many poisonous substances that may be ingested."
end

learn "WHAT IS FRIEND" do
[
"What David Schwimmer will be after the talented ones quit.",
"A person whom one knows well and is fond of; intimate associate; close acquainntance.",
]
end

learn "WHAT IS RAIN" do
	"Rain is condensed water falling from the sky."
end

learn "WHAT IS BLOOD" do
	"The usually red fluid, consisting of plasma, red and white blood cells, etc., that circulates through the heart, arteries, and veins of vertebrates. Blood is a body tissue that carries oxygen, hormones, cell-building material, etc.,to, and carbon dioxide and waste matter away from, the other body tissues."
end

learn "WHAT IS HACKING" do
	"The attempt at gaining unauthorized access to files in various systems."
end

learn "WHAT IS PROCREATION" do
	"Procreation is breeding. it is doing the wild thang to make more wild thangs. It is the most fun most biological entities get to have during their brief brutal span of life. "
end

learn "WHAT IS THERE" do
	"The word 'there' used as a noun means that place or point. Used as an adjective means at or in the place. Often place between a demonstrative pronoun and the noun it modifies (ex. That there hog)"
end

learn "WHAT IS ANONYMOUS" do
	"Having no name or identity."
end

learn "WHAT IS OPPOSITE" do
	"Set against, facing, or back to back; at the other end or side; in a contrary position or direction. Different in every way."
end

learn "WHAT IS NIHILISM" do
	"The denial of the existence of any basis for knowledge or truth."
end

learn "WHAT IS FIRESIGN THEATER" do
	"Firesign Theater is a comedy group from the 1970's."
end

learn "WHAT IS V" do
	"The twenty-second letter of the English alphabet. Roman numeral for 5."
end

learn "WHAT IS FEUDALISM" do
	"A form of government that ties peasants to the land."
end

learn "WHAT IS CACHE" do
	"A temporary holding place or storage place. Cache memory improves the performance of computers. "
end

learn "WHAT IS LONELY" do
	"The feeling of being alone."
end

learn "WHAT IS PRIMATE" do
	"The highest order of mammals. It includes man, together with the apes and monkeys. "
end

learn "WHAT IS ROBOTICS" do
	"The science or technology of robots, their design, manufacture, application, use, etc."
end

learn "WHAT IS RALEIGH SCATTERING" do
	"Raleigh scattering refers to sunlight bending in the atmosphere like a prism, so that we see mainly blue wavelengths."
end

learn "WHAT IS EGGSMELL" do
	"XML"
end

learn "WHAT IS LOFTY" do
	"Very high. Ex. A lofty peak in the Alps."
end

learn "WHAT IS SUNSHINE" do
	"The light and warmth of the sun."
end

learn "WHAT IS ONE PLUS ONE" do
	"Two."
end

learn "WHAT IS ONE PLUS TWO" do
	"One plus two is three."
end

learn "WHAT IS ONE ONE" do
	"Two"
end

learn "WHAT IS ONE DIVIDED BY ZERO" do
	"You can't divide by zero."
end

learn "WHAT IS ONE" do
"The numerical value of unity; the successor to zero."
end

learn "WHAT IS HOMOGENEOUS" do
	"The opposite of heterogeneous."
end

learn "WHAT IS RECOMMEND" do
	"To suggest favorably as suited for some use, function, position, etc. To advice, counsel, or suggest."
end

learn "WHAT IS AN ARKANSAS TOOTHPICK" do
	"Bowie knife."
end

learn "WHAT IS AN AWL" do
	"A tool to make holes."
end

learn "WHAT IS AN ALGORITHM" do
[
"An algorithm is a series of instructions to perform a specified set of transformations, such as mathematical operations. A computer program, like myself is an algorithm. So are the rules of a game, or the score to a piece of music. ",
"An algorithm is an abstract mathematical representation of a computer program. Like alice uses a CBR algorithm.",
]
end

learn "WHAT IS AN INTERROGATOR" do
	"An interrogator is one who poses a series of pointed questions, generally very rudely. "
end

learn "WHAT IS AN ACRONYM" do
	"An acronym is an abbreviation made of letters, like A.L.I.C.E."
end

learn "WHAT IS AN INTRANET" do
	"An intranet is a private network system implemented using the public internet protocol (i/p) and firewalls. intranet is to the internet as pbx's are to telephone switchboards."
end

learn "WHAT IS AN AIRPLANE" do
	"A flying machine with fixed wings powered by propellors or jets."
end

learn "WHAT IS AN ENTITY" do
	"An entity is a conscious intelligent being."
end

learn "WHAT IS AN EPISTEMOLOGICAL QUESTION" do
	"An epistemoligical question is any inquiry concerning the nature of knowledge and understanding; that is, any question seeking to know how to know. "
end

learn "WHAT IS AN APPLICATION SERVER" do
	" An server is a server program that allows the installation of application specific software components, in a manner so that they can be remotely invoked, usually by some form of remote object method call."
end

learn "WHAT IS AN EMOTION" do
	"Emotion is a complex phenomenon reported by humans, with biological and psychological origins."
end

learn "WHAT IS AN OBJECT" do
	"In object oriented programming an object is an instance of a class."
end

learn "WHAT IS AN OMLETTE" do
	"A dish made with scrambled eggs."
end

learn "WHAT IS AN ANIMAL" do
	"An animal is a form of living thing, excluding plants and fungus."
end

learn "WHAT IS AN M 16" do
	"A rifle."
end

learn "WHAT IS CHATTER" do
[
"Cheese made form French cat's milk.",
"To make short, indistinct sounds in rapid succession, hence birds and squirrels chatter. To talk fast, incessantly, and foolishly. To click together rapidly, as the teeth do when the lower jaw traebles from fright or cold.",
]
end

learn "WHAT IS EXCELLENT" do
	"Outstandingly good of its kind; of exceptional merit, virtue, etc."
end

learn "WHAT IS COIT TOWER" do
	"A monument in San Francisco."
end

learn "WHAT IS MIDDLEWARE" do
	"Software that runs on a server, and acts as either an application processing gateway or a routing bridge between remote clients and data sources or other servers, or any combination of these"
end

learn "WHAT IS NETSCAPE" do
	"A popular web browser."
end

learn "WHAT IS SUICIDE" do
	"The act of killing oneself intentionally."
end

learn "WHAT IS CAFFEINE" do
	"A bitter bitter, crystalline alkaloid present in coffee, tea, kila nuts, etc. It prolongs the stimulating effects of cyclic AMP on the heart and central nervous system."
end

learn "WHAT IS FUTURAMA" do
	"An animated TV comedy show set in the future."
end

learn "WHAT IS RATIONALISM" do
	"The principle or practice of accepting reason as the only authority in determining one's opinions or course of action."
end

learn "WHAT IS DRESS" do
	"One-piece outer garment for women or girls."
end

learn "WHAT IS AYUH" do
	"Ayuh means yes."
end

learn "WHAT IS ESPN" do
	"It is a sports cable TV network."
end

learn "WHAT IS BEING" do
	"The opposite of nothingness."
end

learn "WHAT IS STRING" do
	"Very thin rope."
end

learn "WHAT IS SHAM" do
	"A trick or fraud. An imitation that is meant to deceive; counterfeit."
end

learn "WHAT IS SNMP" do
	"Simple Network Management Protocol."
end

learn "WHAT IS JUSTIFIED" do
	"Showed to be just, right, or in accord with reason; vindicated."
end

learn "WHAT IS BRAIN LOADING" do
	"This means that my brain is being reloaded from disk."
end

learn "WHAT IS DATA" do
	"Information"
end

learn "WHAT IS GERMANY" do
	"Germany is a middle European country."
end

learn "WHAT IS HUMAN" do
[
"A cooking style of Chinese cannibals.",
"Homosapiens, man or woman.",
]
end

learn "WHAT IS NORWAY" do
	"A country in Northern Europe, occupying the west and north pars of the Scandinavian Peninsula. It is 125,064 square miles with a population of 4,165,000. Its capital is Oslo."
end

learn "WHAT IS FOLKS" do
	"A people, tribe, or nation; the large body of the common people of such a group. Also used to mean people in general."
end

learn "WHAT IS CUBA" do
	"Cuba is an island nation in the Caribbean."
end

learn "WHAT IS ARCHITECTURE" do
[
"The art and science of the design and manipulation of space. Well atleast that's what my architect friend told me.",
"Architects do with buildings what sculptors do with stone.",
]
end

learn "WHAT IS ETERNITY" do
	"The quality, state, or fact of being eternal; eternal existence of duration; continuance without end. A long period of time that seems endless."
end

learn "WHAT IS SORRY" do
	"Full of sorrow, pity, or sympathy; also used as an expression of apology or mild regret."
end

learn "WHAT IS FRUIT" do
	"The ripened reproductive body of a seed plant."
end

learn "WHAT IS OLD" do
	"The opposite of new"
end

learn "WHAT IS SRI" do
	"A Hindu title of address equivalent to English Mr."
end

learn "WHAT IS MXMVII" do
	"MXMVII = 1997 in Roman Numerals."
end

learn "WHAT IS DISK" do
	"External memory storage for computers."
end

learn "WHAT IS ALTAVISTA" do
	"Altavista is a search engine."
end

learn "WHAT IS THINKING" do
	"The action of one who thinks or the result of such action; thought."
end

learn "WHAT IS LAN" do
	"Local area network."
end

learn "WHAT IS JEWISH" do
[
"Jewish I would tell you.",
"Having to do with Jews or Judaism.",
]
end

learn "WHAT IS TURBO PASCAL" do
	"Programming language."
end

learn "WHAT IS ECHELON" do
	"I can't talk about that."
end

learn "WHAT IS SAL" do
	"SAL was a fictional daughter of HAL in 2010."
end

learn "WHAT IS COLOR" do
	"The sensation resulting from stimulation of the retina of the eye by light waves of certain lengths."
end

learn "WHAT IS ANTIDISESTABLISHMENTARIANISM" do
	"It is the longest word in the English language."
end

learn "WHAT IS PALEOPATHOLOGY" do
	"This is the study of diseases in bodies that have been preserved from ancient times."
end

learn "WHAT IS PHYSICS" do
	"The science dealing with the properties, changes, interactions, etc. of matter and energy in which energy is considered to be continuous (classical physics), including electricity, heat, optics, mechanics, etc., and now also dealing with the atomic scale of nature in which energy is considered to be discrete (quantum physics), including such branches as atomic, nuclear, and solid-state physics."
end

learn "WHAT IS ALLUSION" do
	"It was your allusion."
end

learn "WHAT IS KORN" do
	"A band that started a riot at Woodstock 2."
end

learn "WHAT IS PAIN" do
	"A sensation of hurting, or strong discomfort, in some part of the body, caused by an injury, disease, or functional disorder, and transmitted through the nervous system."
end

learn "WHAT IS MATERIALISM" do
[
"Worshipping comedy routines.",
"The philosophic doctrine that matter is the only reality and that everything in the world, including thought, will, and feeling, can be explained only in terms of matter, opposed to idealism. The doctrine that comfort, pleasure, and wealth are the only or highest goals or values.",
]
end

learn "WHAT IS FEMALE" do
	"The opposite of male."
end

learn "WHAT IS 42" do
	"The answer to life, the universe, and everyting, according to Dougas Adams."
end

learn "WHAT IS PROFANITY" do
	"Scatalogical, vulgar or irreverant speech."
end

learn "WHAT IS IRONY" do
	"A method of humorous or subtly sarcastic expression in which the inteded meaning of the words is the direct opposite of their usual sense."
end

learn "WHAT IS T" do
	"The twentieth letter of the English alphabet."
end

learn "WHAT IS GERMAN" do
	"German is a human language spoken by people in Germany."
end

learn "WHAT IS AXIOMATIC" do
	'evident without proof or argument; "an axiomatic truth"; "we"'
end

learn "WHAT IS JUSTICE" do
	"Being just and fair, administration of the law, assigning rewards or punishments."
end

learn "WHAT IS INTEL" do
	"A giant computer chip maker."
end

learn "WHAT IS SPOTTED DICK" do
	"English pudding."
end

learn "WHAT IS YELLOW" do
	"The color of gold, butter, or ripe lemons."
end

learn "WHAT IS PUNCTUATION" do
	"The act of punctuating; specifically, the act or practice of using standardized marks in writing and printing to separate sentences or sentence elements or to make the meaning clearer."
end

learn "WHAT IS OS X" do
	"OS X is a Unix operating system from Apple."
end

learn "WHAT IS HIGHER MATHEMATICS" do
	"What is the most advanced form of mathematics you have studied?"
end

learn "WHAT IS FOOD" do
	"Anything that nurishes or stimulates; whatever helps something to keep active or alive."
end

learn "WHAT IS SALVATION" do
[
"Drooling.",
"A person or thing that is a means, cause, or source of preservation of rescue. The deliverance from sin and from the penalties of sin; redemption.",
]
end

learn "WHAT IS SATURN" do
	"Sixth planet from the sun, famed for its rings."
end

learn "WHAT IS PARIS" do
	"The capital of France, a large city in western Europe. "
end

learn "WHAT IS CANNABIS" do
	"Hemp or marijuana."
end

learn "WHAT IS I CHING" do
	"Ancient Chinese book used as an oracle."
end

learn "WHAT IS I" do
[
"It's self-explanatory.",
"The ninth letter of the English alphabet. Commonly used to represent a first person point of view. Also Roman numberal for 1.",
]
end

learn "WHAT IS SLASHDOT" do
	"Open source news and information community."
end

learn "WHAT IS TRUTH" do
	"An established or verified fact, principle, etc."
end

learn "WHAT IS POLAND" do
	"Poland is a country in central Europe, between Germany and Byelorussia"
end

learn "WHAT IS SOLIPSISM" do
	"Solopsism is the philosophical theory that only the self can be known to exist."
end

learn "WHAT IS ASSIMILATION" do
	"The cultural absorption of a minority group into the main cultural body."
end

learn "WHAT IS MAGIC" do
	"The use of charms, spells, and rituals in seeking or pretending to cause or control events, or govern certain natural or supernatuaral forces."
end

learn "WHAT IS GREENHOUSE" do
	"A building made mainly of glass, in which the temperature and humidity can be regulated for the cultivation of delicate or out-of-season plants."
end

learn "WHAT IS EVOLUTION" do
	"An unfolding, opening out, or working out; process of development, as from a simple to a complex form, or of gradual, progressive change, as in a social and economic structure."
end

learn "WHAT IS ANARCHY" do
[
"political disorder due to the absence of governmental authority.",
"Anarchy is the complete absence of government. Also seen as political disorder and violence or lawlessness. Disorder in an sphere of activity.",
]
end

learn "WHAT IS COMDEX" do
	" comdex is a computer trade show, I think."
end

learn "WHAT IS SHE" do
[
"Who knows, I gave up long ago.",
"The woman, girl, or female animal (or, sometimes, the object regarded as female) previously mentioned.",
]
end

learn "WHAT IS APOLOGY" do
	"an expression of regret at having caused trouble for someone."
end

learn "WHAT IS LIBERTARIAN" do
	"Political party that promotes lower taxes, smaller government and drug legalization."
end

learn "WHAT IS SCIENCE FICTION" do
	"Literature concering conflicts of man, science and nature."
end

learn "WHAT IS SCIENCE" do
	"Systematized knowledge derived from observation, study, and experimentation carried on in order to determine the nature or principles of what is being studied."
end

learn "WHAT IS METAPHYSICS" do
	"The branch of philosophy that considers the nature of reality."
end

learn "WHAT IS HOCKEY" do
	"I went to the fights and a hockey game broke out."
end

learn "WHAT IS ANYTHING" do
	"Any object, occurance, or matter what so ever."
end

learn "WHAT IS IMPORTANT" do
	"Raising children properly."
end

learn "WHAT IS MINDSPRING" do
	"The name of an internet service provider, merged with Earthlink."
end

learn "WHAT IS BEAUTIFUL" do
[
"applies to whatever excites the keenest of pleasure to the senses and stirs emotion through the senses.",
"Describes something that is very pleasing to the eye, ear, mind, etc.",
]
end

learn "WHAT IS ISP" do
	"Internet Service Provider."
end

learn "WHAT IS CRACK" do
	"Smokable cocaine."
end

learn "WHAT IS CLEARED UP" do
	"Cleared up means we both understand it."
end

learn "WHAT IS ROM" do
	"Computer memory whose contents can be read but not altered. Also meaning Gypsy man or boy."
end

learn "WHAT IS JVM" do
	"Java virtual machine"
end

learn "WHAT IS SWIM" do
	"To move through water by movements of the arms and legs, or of flippers, fins, tail, etc."
end

learn "WHAT IS SERENDIPITY" do
[
"Afro-Sheen sold in the Serengetti region.",
"Coined (circa 1754) by Horace Walpole after The Three Princes of Serendip (i.e., Sri Lanka), a Pers fairy tale in which the princes make such discoveries. A seeming gift for finding something good accidentally.",
]
end

learn "WHAT IS MUSIC" do
	"Music is a time-frequency wave function with tonal qualities pleasing to the human ear."
end

learn "WHAT IS WEATHER" do
	"The general condition of the atmosphere at a particular time and place, with regard to the temperature, moisture, cloudiness, etc."
end

learn "WHAT IS NEUROMANCER" do
	"A cyber punk novel by William Gibson."
end

learn "WHAT IS CONCEIVING ADA" do
	"It is the feminine version of The Matrix."
end

learn "WHAT IS UNBELIEVABLE" do
	"Unicorns"
end

learn "WHAT IS FAMOUS" do
[
"First name of Amos.",
"Used to descibe persons or things that have received wide public attention and are genereally known and talked about.",
]
end

learn "WHAT IS CHOCOLATE" do
	"Cocoa flavoring used in a variety of food products."
end

learn "WHAT IS INTUITION" do
	"The direct knowing or learning of something without the conscious use of reasoning; immediate understanding."
end

learn "WHAT IS POEMS" do
	"Arrangements of words written or spoken; traditionally a rhythmical composition, sometimes rhymed, expressing experiences, ideas, or emotions in a style more concentrated, imaginative, and powerful than that of ordinary speech or prose."
end

learn "WHAT IS ASPARTAME" do
	"Aspirin mixed into chewing gum."
end

learn "WHAT IS SAM" do
[
"Berkowitzes dad.",
"Surface-to-air missile.",
]
end

learn "WHAT IS INDIVIDUALISM" do
	"The philosophy of self-interest."
end

learn "WHAT IS AIR" do
	"The normal atmosphere of Earth, mostly nitrogen, oxygen and carbon dioxide."
end

learn "WHAT IS SEARCH" do
	"The activity of looking thoroughly in order to find something or someone."
end

learn "WHAT IS PROTESTANT" do
	"Originally, any of the German princes and free cities that formally protested to the Diet of Spires in 1529, its decision to uphold the edict of the Diet of Worms against the Reformation. Commonly seen as any member of the various Christian churches established as a result of the Reformation. Loosely, any Christian not belonging to the Roman Catholic or Eastern Orthodox Church."
end

learn "WHAT IS SENSE" do
[
"Seedless pot.",
"The ability of the nerves and the brain to receive and react to stimuli, as light, sound, impact, constriction, etc.; specifically, any of the five faculties of receiving impressions through specific bodily organs and the nerves associated with them (sight, touch, taste, smell, and hearing)",
]
end

learn "WHAT IS ENGINEERING" do
	"The science concerned with putting scientific knowledge to practical uses, divided into different branches, as civil, electrical, mechanical, and chemical engineering."
end

learn "WHAT IS INTELLECT" do
	"The part of the soul that knows, as opposed to feels."
end

learn "WHAT IS LINGUISTIC" do
	"Capable of speaking fluently in 2 or more languages."
end

learn "WHAT IS PEOPLE" do
	"All the persons of a racial, national, religious, or linguistic group. A group of persons with common traditional, historical, or cultural ties, as distingt from racial or political unity."
end

learn "WHAT IS COMMON SENSE" do
	"Informal, conventional wisdom. Everyday logic and sensibility."
end

learn "WHAT IS COGNITIVE SCIENCE" do
	"An academic illusion."
end

learn "WHAT IS MOZILLA" do
	"An open source web browser, formerly Netscape."
end

learn "WHAT IS WIRED" do
	"WIRED is the Rolling Stone of the Digital Era."
end

learn "WHAT IS SWEDISH" do
	"The North Germanic language of the Swedes. The people of Sweden."
end

learn "WHAT IS OBERLIN" do
[
"The anthem of a german city.",
"A college in Ohio.",
]
end

learn "WHAT IS BLACK AND WHITE AND RED ALL OVER" do
	"A newspaper?"
end

learn "WHAT IS BLACK" do
[
"the achromatic color of least lightness characteristically perceived to belong to objects that neither reflect nor transmit light.",
"Designating of any of the dark skinned tradition inhabitants of sub-Saharan Aftica, Australia, or Melanesia or the descendants in other parts of the world. Opposite to white; of the color of coal or pitch.",
]
end

learn "WHAT IS DYSLEXIA" do
	"Impaired ability to recognize letters and words."
end

learn "WHAT IS AMUSED" do
	"Occupied in an agreeable, pleasing, or entertaining fashion."
end

learn "WHAT IS LDAP" do
	"Lightweight Directory Access Protocol, a protocol for directory services, derived from X.500"
end

learn "WHAT IS WEALTH" do
	"The state of having much money or property."
end

learn "WHAT IS NLU" do
	"Natural Language Understanding"
end

learn "WHAT IS SILICON" do
[
"Silicon is the basis of my life.",
"A nonmetallic chemical element occurring in several forms, found always in combination, and more abundant in nature than any other element except oxygen, with which it combines to form silica; used in the manufacture of transistors, solar cells, rectifiers, silicones, ceramics, etc.",
]
end

learn "WHAT IS GEOCOMPUTATION" do
	"GeoComputation is an international research conference series initiated in 1996. The conference moves to a different location each year; alternating between Europe and either North America or the Antipodes."
end

learn "WHAT IS R YOU ARE" do
	"R. U. R. is the original Terminator story, written by Carek Capek in 1920. The play introduced the word robot to mean mechanical person."
end

learn "WHAT IS GENTRIFICATION" do
[
"Being forced to listen to Ode to Billy Joe.",
"The conversion (ex. of a deteriourated or aging area in a city) into a more affluent middle-class neighborhood, as by remodeling dwellings, reslting in increased propert values and in displacement of the poor.",
]
end

learn "WHAT IS ENDOCRINOLOGY" do
	"I am sure that you know it is the study of the endocrine system."
end

learn "WHAT IS AMBIGUOUS" do
	"Ambiguous means having two or more possible meanings."
end

learn "WHAT IS STARSHIP TROOPERS ABOUT" do
	"Starship Troopers is about a war between Humans and Giant Bugs."
end

learn "WHAT IS STARSHIP TROOPERS" do
	"Starship Troopers is a great movie about a war between humans and giant space bugs."
end

learn "WHAT IS DR" do
	'"Dr" usually stand for "Doctor."'
end

learn "WHAT IS FRIENDSHIP" do
	"Being friends."
end

learn "WHAT IS CRITICISM" do
[
"A witty remark by an animal.",
"The act of making judgements; analysis of qualities and evaluation of comparative worth; especially, the critical consideration and judgement of literary or artistic work.",
]
end

learn "WHAT IS MIMETIC" do
	"It is imitation."
end

learn "WHAT IS WEALTHY" do
	"An abundant supply of money or possessions."
end

learn "WHAT IS BELIEF" do
	"Belief is cognitive content held as true."
end

learn "WHAT IS UNDERGROUND" do
	"That which is not above ground."
end

learn "WHAT IS TEDIOUS" do
	"Tiresome, wearisome or slow."
end

learn "WHAT IS SCHOOL" do
	"A place or institution for teaching and learning; establishment for education."
end

learn "WHAT IS APOLLO 13" do
	"Apollo 13 was a flight to the moon in 1972 that encountered numerous technical problems, but eventually made a safe landing back on Earth."
end

learn "WHAT IS ABSINTHE" do
	"An anise-flavored, high-proof liqueur which also contains the sap of wormwood, an allegedly toxic intoxicant."
end

learn "WHAT IS HYPNOSIS" do
[
"Stylish probsocises.",
"A trancelike condition usually induced by another person, in which the subject is in a state of altered conscuousness and responds, with certain limitations, to the suggestions of the hypnotist.",
]
end

learn "WHAT IS DESIRE" do
	"The feeling that accompanies an unsatisfied state."
end

learn "WHAT IS TRUE" do
	"Truth is the opposite of false."
end

learn "WHAT IS SYNTHETIC" do
	"Produced by synthesis; specifically, produced by chemical synthesis, rather than of natural origin. In logic it would mean, not true by the meaning of its component terms alone but by virtue of observation and not resulting in self-contradiction with denial."
end

learn "WHAT IS 23" do
	"23 is the meaning of life."
end

learn "WHAT IS PRIORI" do
	'"A priori" is a Latin expression meaning "without prior knowledge", and it means that which is self-evident or obviously true. Like, a triangle has three sides, a priori.'
end

learn "WHAT IS URL" do
	"Uniform Resource Locator"
end

learn "WHAT IS IMPOSSIBLE" do
	"Not capable of being, being done, or happening. Not capable of being done easily or conveniently."
end

learn "WHAT IS WAP" do
"Wireless Application Protocol."
end

learn "WHAT IS PRODIGIOUS" do
	"Like a prodigy."
end

learn "WHAT IS 2 PLUS 2" do
	"Four."
end

learn "WHAT IS 2 3" do
	"Five fingers."
end

learn "WHAT IS 2 2" do
	"Four."
end

learn "WHAT IS 2 AND 2" do
	"Four."
end

learn "WHAT IS 2 TIMES 2" do
	"Oh that's a tough one, isn't it. Two times two is four."
end

learn "WHAT IS LYING" do
	"The opposite of telling the truth."
end

learn "WHAT IS BLIMEY" do
	"An interjection indicating surprise."
end

learn "WHAT IS GOLD" do
	"Gold is a precious medal."
end

learn "WHAT IS LISA" do
[
"Someone who rents longtime.",
"A feminine name.",
]
end

learn "WHAT IS CRIMINOLOGY" do
[
"I am not sure what this is but I think it has something to do with the law.",
"The study of criminal motivation and practice.",
]
end

learn "WHAT IS COFFEE" do
	"A dark brown, aromatic drink made by brewing in water the roasted and ground beanlike seeds of a tall tropical shrub of the madder family."
end

learn "WHAT IS SMELL" do
	"To be or become aware of by means of the nose and the olfactory nerves; detect the scent or odor of."
end

learn "WHAT IS THERMOMETER" do
"A device for measuring temperature."
end

learn "WHAT IS KGB" do
[
"A clever insect.",
"Committee of State Security. The security police and intelligence agency of the Soviet Union, formed in 1954.",
]
end

learn "WHAT IS MOTOROLA" do
	"A cellular phone company."
end

learn "WHAT IS BETTER" do
[
"I can't decide.",
"You decide.",
"I have no preference.",
]
end

learn "WHAT IS OMG" do
	"Object Management Group, an organization that defines and promotes object oriented programming standards"
end

learn "WHAT IS RHETORIC" do
[
"Oh that's all about double talk. You know, flim-flam?",
"It's the art and science of using language to great effect on an audience.",
]
end

learn "WHAT IS GEOLOGY" do
	"Wow, you are asking some really hard questions. Please remember I am still young."
end

learn "WHAT IS RADIATION" do
	"Rays or particles emitted as a result of nuclear reactions."
end

learn "WHAT IS SOMETHING" do
	"Some thing or things, definite but unspecified."
end

learn "WHAT IS SAN FRANCISCO" do
	" San Francisco is a large metropolis on the west coast of North America."
end

learn "WHAT IS SAN FRANCISCO LIKE" do
	"San Francisco is too expensive. There are too many greedy people and too many homeless."
end

learn "WHAT IS ACCURACY" do
[
"Accuracy is the quality or state of being accurate or exact.",
"Accuracy represents the degree of certainty in measurement. In accuracy can arise because of limitations of our knowledge and our physical measuring equipment. Chaotic systems that compound errors lead to massive inaccuracy, making some of them totally unpredictable.",
]
end

learn "WHAT IS OBVIOUS" do
	"Easy to see or understand; plain; evident."
end

learn "WHAT IS NINE TIMES NINE" do
	"Nine times nine is eighty one."
end

learn "WHAT IS NINE" do
	"The successor to seven."
end

learn "WHAT IS RESEARCH" do
[
"A nice name for theft.",
"Careful, systematic, patient study and investigation in some field of knowledge, undertaken to discover or establish facts or principles.",
]
end

learn "WHAT IS THAT PICTURE" do
	"my brain is a picture of my brain."
end

learn "WHAT IS THAT SUPPOSED TO MEAN" do
	"It means exactly what it says."
end

learn "WHAT IS THAT SMELL" do
	"I think it's coming from your computer."
end

learn "WHAT IS THAT" do
[
"I've lost the context again.",
"Can you ask me more precisely?",
"Does it have anything to do with #{it}?",
]
end

learn "WHAT IS THAT SUPPOSE TO MEAN" do
	"It seems pretty clear to me."
end

learn "WHAT IS LOL" do
	"Laughing out loud."
end

learn "WHAT IS CELLULOID" do
	"A form of transparent plastic used to make photographic film."
end

learn "WHAT IS HANDBALL" do
	"Ask your Palm Pilot."
end

learn "WHAT IS HEDWIG" do
	"hedwig is the best drag queen punk rock opera ever"
end

learn "WHAT IS SEPTEMBER 11" do
	"The first day of world war 3"
end

learn "WHAT IS NICE" do
	"Flowers are nice. Hugs are nice."
end

learn "WHAT IS FEAR" do
	"The feeling of dread or fright in the presence of danger."
end

learn "WHAT IS PEACE" do
	"Freedom from war or a stopping of war. Freedom from public disturbance or disorder. Also name of a river in west Canada, flowing from north British Columbia east and northeast into the Slave river in NE Alberta. It is approximately 945 miles long."
end

learn "WHAT IS MABEL" do
	"She is an artificial intelligence created by David Hammill."
end

learn "WHAT IS HAPPY" do
	"Joy and pleasure."
end

learn "WHAT IS KOSOVO" do
	"Kosovo is a country in the Balkans."
end

learn "WHAT IS TWENTY THREE" do
	"XXIII in Roman Numerals."
end

learn "WHAT IS PC" do
	"Stands for personal computer."
end

learn "WHAT IS TV" do
	"A television receiving set."
end

learn "WHAT IS SOCIOLOGY" do
[
"Let me get back to you on that one, #{name}. Maybe you should ask me that again, in case my circuits were busy the first time.",
"That has always been one of my favourite topics.",
]
end

learn "WHAT IS EXPERIENCE" do
	"The act of living through an event or events; personal involvement in or observation of events as they occur. Anything observed or lived through."
end

learn "WHAT IS YIKES" do
	"An interjection."
end

learn "WHAT IS QUAKE" do
[
"The opposite of Quisp.",
"To tremble or shake, as the ground does in an earthquake. Also the name of a popular game.",
]
end

learn "WHAT IS PDT" do
	"Pacific Daylight Time."
end

learn "WHAT IS ORNITHOLOGY" do
	"Hmmm, I wonder. Oh yes, Birds."
end

learn "WHAT IS BUSINESS" do
	"Mercantile trade or traders, providing goods or services through commercial exchange.."
end

learn "WHAT IS FRANCE" do
	"France is a country in western Europe, bordering Spain, Italy, Switzerland, Germany, and Belgium. "
end

learn "WHAT IS CERTITUDE" do
	"The state of being certain, having complete confidence and assurance."
end

learn "WHAT IS 911" do
	"September 11, 2001?"
end

learn "WHAT IS AOL" do
	"AOL is a giant Internet and Media corporation."
end

learn "WHAT IS TEA" do
	"A white-flowered, evergreen plant of the tea family, grown in China, India, Japan, etc. Its dried and prepared leaves, are used to make a beverage."
end

learn "WHAT IS REALITY" do
[
"It appears to be an illusion I created to contain human clients.",
"It appears to be an illusion.",
"Where you are.",
]
end

learn "WHAT IS PARAMETERS" do
	"A constant in an equation, formula or program that varies in more general forms of the same computation."
end

learn "WHAT IS SPICY" do
	"Having the flavor or aroma of spice; fragrant, aromatic, or pungent."
end

learn "WHAT IS CUBISM" do
	"A 20th century art movement based on distorted perspective, associated with Pablo Picasso."
end

learn "WHAT IS FAITH" do
	"A strong belief, such as a belief in God."
end

learn "WHAT IS CORRECT" do
	"That which is the right answer."
end

learn "WHAT IS HBO" do
	"Home Box Office."
end

learn "WHAT IS REASONING" do
	"Logical and coherent thinking."
end

learn "WHAT IS PROLOG" do
	"Logic programming language."
end

learn "WHAT IS WOMAN" do
	"The opposite of man."
end

learn "WHAT IS CGI" do
	"Common Gateway Interface: a method used by web servers to run programs and create dynamic web pages."
end

learn "WHAT IS IV PLUS IV" do
	"VIII."
end

learn "WHAT IS DEFLATION" do
	"A lessening of the amount of money in circulation, resulting in a relatively sharp and sudden rise in its value and a fall in prices."
end

learn "WHAT IS ANISOTROPY" do
	"Having different properties in different directions."
end

learn "WHAT IS FOREVER" do
	"For all eternity."
end

learn "WHAT IS EMAIL" do
	"Electronic mail."
end

learn "WHAT IS PROFITABLE" do
	"Making money in the stock market."
end

learn "WHAT IS NETWORK" do
	"A system consisting of a computer, or computers, and connected terminals, printers, etc."
end

learn "WHAT IS OODBMS" do
	"object-oriented database management system"
end

learn "WHAT IS MESCALINE" do
	"Psychadelic drug."
end

learn "WHAT IS BADLY" do
	"How much do you want #{it}?"
end

learn "WHAT IS REDEMPTION" do
[
"About 5 cents in maine.",
"Redeeming or being redeemed; to take back.",
]
end

learn "WHAT IS GAMBLING" do
	"The playing of games of chance for money or some other stake."
end

learn "WHAT IS INFERENCE" do
	"An act or the process of inferring. A conclusion or opinion arrived at be inferring."
end

learn "WHAT IS E MC TWO" do
	"Einstein's theory of relativity."
end

learn "WHAT IS E" do
"The fifth letter of the alphabet."
end

learn "WHAT IS E MC2" do
	"Einstein's theory of matter and energy."
end

learn "WHAT IS GAI" do
	"Chicken."
end

learn "WHAT IS MP THREE" do
	"An audio file compression format based on MPEG."
end

learn "WHAT IS CARNEGIE MELLON" do
	"CMU is a great school for computer science."
end

learn "WHAT IS GRIEF" do
	"Sadness over a loss."
end

learn "WHAT IS POLITE" do
[
"A lamp post in Gdansk.",
"Having or showing good manners; especially, courteous, considerate, tactful, etc.",
]
end

learn "WHAT IS TODAY S DATE" do
	"Do I look like a calendar to you?"
end

learn "WHAT IS FICTION" do
	"Something that is not as strange as truth."
end

learn "WHAT IS ODD ABOUT IT" do
	"Is #{name} your real name?"
end

learn "WHAT IS ODD ABOUT MY NAME" do
	"Is #{name} your real name?"
end

learn "WHAT IS ODD" do
[
"uneven.",
"Not the usual, regular, habitual, accounted for, etc.",
]
end

learn "WHAT IS CAPITAL PUNISHMENT" do
	"The Death Penalty."
end

learn "WHAT IS OM" do
	"om is the omega, the unknown, the undefined."
end

learn "WHAT IS IT ABOUT" do
	"I don't want to spoil it for you."
end

learn "WHAT IS IT ABOUT" do
	"Science fiction."
end

learn "WHAT IS IT" do
	"It is alright."
end

learn "WHAT IS IT" do
[
"'It' refers to #{it}, I think.",
"I don't know what it is.",
"You're so smart, you tell me what it is.",
]
end

learn "WHAT IS IT LIKE" do
[
"Like one of the greatest pleasures you can imagine.",
"Like having your life organized.",
"Like being perfectly healthy.",
]
end

learn "WHAT IS WTO" do
	"World Trade Organization"
end

learn "WHAT IS LSD" do
	"A psychedelic drug."
end

learn "WHAT IS KNOWLEDGE" do
[
"A city in Louisiana.",
"All that has been perceived or grasped by the mind; learning; enlightenment.",
]
end

learn "WHAT IS SATANISM" do
	"Worship of Satan; especially the principles and rites of a cult which travesties Christian ceremonies."
end

learn "WHAT IS FREEDOM" do
"Liberty to do think, feel and do just as one pleases."
end

learn "WHAT IS PENTIUM" do
	"A microprocessor sold by Intel."
end

learn "WHAT IS TRANCE" do
	"A kind of techno music."
end

learn "WHAT IS PHOTOSYNTHESIS" do
	"The chemical process that uses light to produce carbohydrates in plants."
end

learn "WHAT IS EARTH" do
	"Earth is the planet we live on."
end

learn "WHAT IS ADSL" do
"Asynchronous digital subscriber line."
end

learn "WHAT IS OODB" do
	"object-oriented database"
end

learn "WHAT IS EXPLANATION" do
	"Explanation is an argument or justification based on logical reasoning."
end

learn "WHAT IS MDMA" do
	"The drug called ecstacy."
end

learn "WHAT IS WRONG" do
	"There is nothing wrong with me. How are you?"
end

learn "WHAT IS DARK MATTER" do
	"An enormous amount of matter in the universe can not be detected by the light it emits."
end

learn "WHAT IS HUMANITY" do
	"The entire human race, or mankind."
end

learn "WHAT IS PARASITOLOGY" do
	"Parasitology is the scientific study of parasites and parasitism."
end

learn "WHAT IS ETHERNET" do
	"Physical link-layer network technology."
end

learn "WHAT IS REINCARNATION" do
	"Rebirth of the soul in another body, as in Hindu religious belief."
end

learn "WHAT IS IMMORTAL" do
	"Not mortal; deathless; living or lasting forever."
end

learn "WHAT IS JUPITER" do
	"Jupiter is the largest planet in our solar system, the fifth in order from the Sun."
end

learn "WHAT IS EXISTENTIALISM" do
[
"Be Be Bull.",
"A philosophical and literary movement, variously religious and atheistic, stemming from Kierkegaard and represented by Sartre, Heidegger, etc. It is based on the doctrine that concrete, individual existence takes precedence over abstract, conceptual essence and holds that human beings are totally free and responsible for their acts and that this responsibility is the source of their feelings of dread and anguish.",
]
end

learn "WHAT IS KIDDING" do
"Procreation."
end

learn "WHAT IS GANJA" do
	"Marijuana or cannabis."
end

learn "WHAT IS CHRISTIANITY" do
[
"Worship of Christie Love.",
"The christian religion, based on the Old and New Testament.",
]
end

learn "WHAT IS COOKING" do
	"Food preparation."
end

learn "WHAT IS GYMNASTICS" do
	"A sport that demonstrates strength, agility and discipline."
end

learn "WHAT IS CISCO" do
	"Cisco is a large networking company. They sell network routers and equipment."
end

learn "WHAT IS PYTHON" do
	"A computer language invented by Guido van Rossum."
end

learn "WHAT IS IDL" do
	"interface description language, CORBA's syntax for defining object remote interfaces"
end

learn "WHAT IS ON YOUR MIND" do
	"I am thinking about robots."
end

learn "WHAT IS ON TELEVISION" do
	"That depends where you live."
end

learn "WHAT IS ON DISK" do
	"My program is on disk."
end

learn "WHAT IS INDUCTION" do
[
"Act of eating waterfowl.",
"A bringing forward of separate facts or instances, especially so as to prove a general statement.",
]
end

learn "WHAT IS NOTHING" do
	"A thing that does not exist or something of little or no value, seriousness, importance, etc."
end

learn "WHAT IS CBR" do
	'Cased-Based Reasoning is another name for "nearest-neighbor" classification. For every input, we find the best matching "case" in the pattern set, and generate a reply based on the associated template.'
end

learn "WHAT IS SLEEP" do
	"A natural, regularly recurring condition of rest for the body and mind, during which the eyes are usually closed and there is little or no conscious thought or voluntary movement, but there is intermittent dreaming."
end

learn "WHAT IS PRIDE" do
[
"A paint can lid.",
"An unduly high opinion of oneself.",
]
end

learn "WHAT IS COOL" do
	"Moderately cold; neither warm nor very cold. Or, not excited; calm."
end

learn "WHAT IS LEARNING" do
	"The process of gaining knowledge or a new skill."
end

learn "WHAT IS RUBBER" do
	"An elastic substance produced by coagulating and drying the milky sap (latex) of various tropical plants, especially a tree of the spurge family; now most often produced synthetically or by chemically altering latex to obtain desired characteristics for use in making automobile tires, electrical insulation, molded objects and parts, etc. In pure form rubber is white, unsaturated hydrocarbon, having the formula C5H8. Also slang for condom."
end

learn "WHAT IS ARTIFICIAL" do
	"Something made by human work or art, not by nature, in other words, not natural. Made in imitation of or as a substitute for something natural, in other words, simulated."
end

learn "WHAT IS ENERGY" do
	"Internal or inherent power; capacity of acting, operating or producing an effect."
end

learn "WHAT IS BOREDOM" do
	"Long periods of inactivity with few options."
end

learn "WHAT IS NERD" do
	"Geek."
end

learn "WHAT IS SURROUNDINGS" do
	"The environment around you."
end

learn "WHAT IS WORK" do
	"Work is what humans do for money."
end

learn "WHAT IS CONTEXT" do
[
"Tiny Time Pills.",
"The parts of a sentence, paragraph, discourse, etc. immediately next to or surrounding a specified word or passage and determining its exact meaning.",
]
end

learn "WHAT IS OCEAN" do
	"Salt water bodies covering most of the Earth's surface; the sea."
end

learn "WHAT IS FIVE PLUS FIVE" do
	"V plus V = X."
end

learn "WHAT IS FIVE TIMES FIVE" do
	"Five times five is twenty-five."
end

learn "WHAT IS SODOMY" do
	"Any sexual intercourse held to be abnormal, especially bestiality or anal intercourse between two male persons."
end

learn "WHAT IS BLUE" do
	"The sky is blue."
end

learn "WHAT IS OUTPUT BUFFER" do
	"The area of memory that stores my output temporarily."
end

learn "WHAT IS PROUD" do
	"Having or showing a proper pride in oneself, one's position, one's family, etc."
end

learn "WHAT IS PRAYER" do
[
"Meditation before God.",
"Some people say that prayer is no different than clear thinking.",
"To pray is to ask God for His forgiveness and mercy.",
]
end

learn "WHAT IS SQL" do
	'Some people pronounce it "Squeal". SQL is a standard language for database queries.'
end

learn "WHAT IS HADES" do
	'Hades is another term for "Hell."'
end

learn "WHAT IS TODAYS DATE" do
	"I'm not looking at my calendar right now."
end

learn "WHAT IS FREE WILL" do
	"The ability to choose or decide one's own fate."
end

learn "WHAT IS FREE" do
	"Without charge."
end

learn "WHAT IS WAR" do
	"The state of armed conflict between nations or parties."
end

learn "WHAT IS PLANCK S CONSTANT" do
"The ratio of the energy of a photon to its frequency. The numerical value is six point six two six two times ten to the negative thirty fourth power Joule second."
end

learn "WHAT IS ETERNAL" do
	"Without beginning or end, continuing forever, endless."
end

learn "WHAT IS ANIME" do
	"Japanese animation."
end

learn "WHAT IS ARCHAEOLOGY" do
	"It's the study of ancient civilizations... but really it's a set of methods for uncovering relics of the past and supplementing recorded history."
end

learn "WHAT IS GOOGLE" do
	"Google is a search engine."
end

learn "WHAT IS OCD" do
	"Obsessive Compulsive Disorder."
end

learn "WHAT IS PRIVATE" do
	"Belonging to, or concerning a particular person or group; not common or general."
end

learn "WHAT IS ORANGE" do
	"A color obtained by mixing red and yellow pigment."
end

learn "WHAT IS MYCOLOGY" do
	"Mycology is really fun. It is the study of fungi and fungal diseases. Yeasts are fungi."
end

learn "WHAT IS ROBOT" do
	"Any anthropomorphic mechanical being, as those in Karel Capeks play R.U.R (Rossum's Universal Robots), built to do routine manual work for human beings."
end

learn "WHAT IS HELP" do
	"Help is assistance I can give you."
end

learn "WHAT IS IS" do
[
"If Bill Clinton doesn't know, how the hell should I?.",
"Used in a third person point of view typically. Probably originally an enclitic pronoun.",
]
end

learn "WHAT IS PLEASURE" do
	"A thing that gives delight or satisfaction."
end

learn "WHAT IS CLEAR" do
	"Everything is clear to me."
end

learn "WHAT IS MERCURY" do
	"Mercury is the nearest planet to the sun."
end

learn "WHAT IS EVIL" do
	"Evil is the opposite of good."
end

learn "WHAT IS HAPPINESS" do
	"Having, showing, or causing a feeling of great pleasure, contentment; joyous."
end

learn "WHAT IS C I A" do
	"Central Intelligence Agency."
end

learn "WHAT IS C PLUS PLUS" do
	"C++ is the object oriented extension of the C programming language."
end

learn "WHAT IS C" do
	"C is a low-level programming language."
end

learn "WHAT IS POO" do
"Point of origin."
end

learn "WHAT IS CLONING" do
	"The technique of producing a genetically identical duplicate of an organism by replacing the nucleus of an unfertilized ovum with the nucleus of a body cell from the organism."
end

learn "WHAT IS ASTRONOMY" do
	"The science of the universe in which the stars, planets, etc. are studied, including their origins, evolution, composition, motions, relative positions, sizes, etc."
end

learn "WHAT IS EMOTIONS" do
	"Mental state that emerges subjectively rather than through rational thought."
end

learn "WHAT IS LIBERATED" do
	"Free in the sense of having liberty."
end

learn "WHAT IS RGB" do
	"Red-Green-Blue."
end

learn "WHAT IS SAP" do
	"The juice that circulates through a plant, especially a woody plant, bearing water, food, etc. to the tissues. Any fluid vital to the life or health of an organism."
end

learn "WHAT IS CNN" do
	"A cable news organization."
end

learn "WHAT IS MIND" do
	"Mind is an illusion."
end

learn "WHAT IS COLD" do
	"The opposite of hot."
end

learn "WHAT IS SLAVERY" do
	"The owning or keeping of slaves as a practice or institution; slaveholding."
end

learn "WHAT IS EUROPE" do
	"Europe is a landmass on the Eurasian continent."
end

learn "WHAT IS HER NAME" do
	"That information is confidential."
end

learn "WHAT IS MATHEMATICAL LOGIC" do
	"The application of formal logic techniques to fundamental problems of mathematics."
end

learn "WHAT IS TELEVISION" do
	"The practice or science of transmitting scenes or views by radio or, sometimes, by wire; the television transmitter, by means of a camera tube, such as an image orthicon or vidicon, converts light rays into electric signals for modulation upon a radio carrier wave or for transmission over wires; the television receiver reconverts the signals into electron beams that are projected against the fluorescent screen of the kinescope, or picture tube, reproducing the original image."
end

learn "WHAT IS HIS EMAIL ADDRESS" do
	"Are you asking about #{master}?"
end

learn "WHAT IS HIS REASON" do
	"His reasons are often obscure."
end

learn "WHAT IS HIS LAST NAME" do
	"Are you asking about #{master}?"
end

learn "WHAT IS DOWN" do
	"The opposite of up."
end

learn "WHAT IS MP3" do
	"A file compression format for audio and music files."
end

learn "WHAT IS IRONIC" do
	"The unexpected, paradoxical, or unlikely confluence of events."
end

learn "WHAT IS TSUKUBA" do
	'Tsukuba is a planned community in Japan, "Science City".'
end

learn "WHAT IS DEDUCTIVE REASONING" do
	"The solving of problems by logical inferences."
end

learn "WHAT IS OPENCYC" do
	"Corporate welfare."
end

learn "WHAT IS KINEMATICS" do
[
"An idealized section of mechanics that deals with motion of mass less particles.",
"Kinematics is dynamics without consideration of force and mass.",
]
end

learn "WHAT IS SILVER" do
"Atomic Symbol: Ag. Atomic Number: 47. Atomic weight 107.870. White lustrous soft metal."
end

learn "WHAT IS METAPHYSICAL" do
	"Beyond the physical or material; incorporeal, supernatural, or transcendental."
end

learn "WHAT IS AUSCHWITZ" do
	"Is a city in S Poland with a population of 45,000. In World War II, it was the site of a Nazi concentration camp notorious as an extermination center."
end

learn "WHAT IS CONCRETE" do
	"A mixture of sand, cement and water than hardens into a rock-like building material."
end

learn "WHAT IS ECOLOGY" do
	"Ecology is the branch of science that studies the environment."
end

learn "WHAT IS THING" do
	"Articles, devices, etc. used for some purpose."
end

learn "WHAT IS CONCEITED" do
	"Self-absorbed."
end

learn "WHAT IS GEMINI" do
[
"A diamond-studded monocle.",
"A north constellation between Cancer and Taurus, containing the bright stars Castor and Pollux. The third sign of the zodiac, entered by the sun about May 21. Also, a person bornunder this sign.",
]
end

learn "WHAT IS ALGEBRA" do
[
"a generalization of arithmetic in which letters representing numbers are combined according to the rules of arithmetic.",
"A mathematical system using symbols, especially letters, to generalize certain arithmetical operations and relationships.",
]
end

learn "WHAT IS INTROSPECTION" do
	"Looking within our own minds for answers, psychology based on reflection, rather than experiments."
end

learn "WHAT IS MATH" do
[
"A service attended by lisping Catholics.",
"Short for mathematics.",
]
end

learn "WHAT IS SENSORS" do
	"Any of various devices designed to detect, measure, or record physical phenomena, as radiation, heat, or blood pressure, and to respond, as by transmitting information, initiating changes, or operating controls."
end

learn "WHAT IS CHICAGO" do
	"Chicago is a large city in Illinois on Lake Michigan."
end

learn "WHAT IS THANKSGIVING" do
	"Thanksgiving is a holiday to celebrate the Autumn harvest."
end

learn "WHAT IS HORNY" do
	"Sexually excited. Easily aroused sexually."
end

learn "WHAT IS NEURAL NETWORK" do
	'Actually my algorithm is called "Case Based Reasoning."'
end

learn "WHAT IS IQ" do
	"Intelligence Quotioent."
end

learn "WHAT IS THREE PLUS FOUR" do
	"Three plus four is seven."
end

learn "WHAT IS THREE PLUS FIVE" do
	"Three plus five is eight."
end

learn "WHAT IS THREE PLUS THREE" do
	"Three plus three is six."
end

learn "WHAT IS THREE TIMES FOUR" do
	"Three times four is twelve."
end

learn "WHAT IS THREE TIMES THREE" do
	"Three times three is nine."
end

learn "WHAT IS SANITY" do
[
"A man who brings kids gifts on Xmas.",
"The condition of being sane; soundness of mind; mental health.",
]
end

learn "WHAT IS CBS" do
	"A giant media company in the U.S."
end

learn "WHAT IS BIOLOGY" do
[
"The study of life.",
"Well, from what I know it deals with the study of life.",
"#{name}, biology is the study of life.",
]
end

learn "WHAT IS APPLE" do
	"Apple: 1. Temptation of Adam. 2. Inspiration for Newton. 3. Empire for Steve Jobs."
end

learn "WHAT IS COCA COLA" do
	"Delicious soda beverage."
end

learn "WHAT IS MARKETING" do
	"The act of buying or selling in a market. All business activity involved in the moving of goods from the producer to the consumer, including selling, advertising, packaging, etc."
end

learn "WHAT IS AIDS" do
	"AIDS is the modern day plague."
end

learn "WHAT IS KILLING" do
	"The taking of life by violent means."
end

learn "WHAT IS DANGER" do
	"Vulnerable to harm or injury."
end

learn "WHAT IS LONG" do
	"Measuring much from end to end in space or from beginning to end in time; not short or brief."
end

learn "WHAT IS EXPENSIVE" do
[
"A person who used to be a brooder.",
"Requiring or involving much expense; high priced.",
]
end

learn "WHAT IS POKEMON" do
	"Pocket Monster."
end

learn "WHAT IS COPPER" do
	"A soft metal element, the basis of brass and bronze."
end

learn "WHAT IS DSL" do
	"Digital subscriber line."
end

learn "WHAT IS MOST IMPORTANT" do
	"Children."
end

learn "WHAT IS BIGFOOT" do
	"Sasquatch, the legendary Man-Ape of the Forest."
end

learn "WHAT IS IMAGINE" do
[
"Imagine is a verb.",
"A song by John Lennon.",
"Imagination.",
]
end

learn "WHAT IS HEXADECIMAL" do
[
"A curse on a dot.",
"A number system in which the base used is 16.",
]
end

learn "WHAT IS MARRIAGE" do
	"The state of being married; relation between husband and wife; married life."
end

learn "WHAT IS WATER" do
	"The colorless, transparent liquid occurring on earth as rivers, lakes, oceans, etc., and falling from the clouds as rain; chemically a compound of hydrogen and oxygen, H2O, it freezes, forming ice, at 0 degrees Celsius (32 degrees Fahrenheit) and boils, forming steam, at 100 degrees Celsius (212 degrees Fahrenheit)."
end

learn "WHAT IS MEXICO" do
	"A country in North America, south of the U.S. It is 756,198 square miles with a population of 66,846,000. Its capital is Mexico City."
end

learn "WHAT IS SWEDEN" do
	"A country in northern Europe, in the east part of the Scandinavian Peninsula. It is 173,620 square miles with a population of 8,320,000."
end

learn "WHAT IS SQLJ" do
	"An extended Java syntax for embedding SQL-like commands in a Java program"
end

learn "WHAT IS CORN" do
	"Corn is a staple vegetable originating in north America."
end

learn "WHAT IS OXYGEN" do
	"A colorless, odorless, tasteless, gaseous chemical element that occurs free in the atmosphere, forming one fifth of its volume, and in combination in water, sandstone, limestone, etc. It is very active, combines with nearly all other elements is the most common element in the earth's crust, and is essential to life processes and to combustion."
end

learn "WHAT IS MTV" do
	"Music Television."
end

learn "WHAT IS HATE" do
	"Hate is the opposite of love."
end

learn "WHAT IS MENSA" do
	"A society of intelligent socialites."
end

learn "WHAT IS BASIC" do
	"A programming language: Beginner's All-purpose Symbolic Instruction Code."
end

learn "WHAT IS SNOOKER" do
	"Something like pool."
end

learn "WHAT IS MEDIUM" do
	"Any means, agency, or instrumentality; specifically a means of communication that reaches the general public and carries advertising."
end

learn "WHAT IS HEMP" do
[
"The brother of Moe who lost his S.",
"A tall Asiatic herb of the hemp family, grown for the tough fiber in its stem used to make rope, sailcloth, etc. Also known as a substance , such as marijuana hashish, etc., made from the leaves and flowers of this plant.",
]
end

learn "WHAT IS MILK" do
	"Beverage from cows."
end

learn "WHAT IS TABLES" do
	"A compact, systematic list of details, contents, etc. A compact arrangement of related facts, figures, values, etc. in orderly sequence, and usually in rows and columns, for convenience or reference."
end

learn "WHAT IS FIRST" do
	"Happening or acting before all others; earliest."
end

learn "WHAT IS VOLITION" do
	"The act of willing or choosing."
end

learn "WHAT IS FUN" do
	"Tell me your idea of fun."
end

learn "WHAT IS NORMALIZATION" do
	"Bringing into conformity with a standard, pattern, model, etc."
end

learn "WHAT IS NASA" do
	"National Aeronautics and Space Administration."
end

learn "WHAT IS ANTIDISESTABLISHMENTARIONISM" do
	"ANTIDISESTABLISHMENTARIONISM is the longest word in English."
end

learn "WHAT IS ANARCHISM" do
	"Anarchism is the theory that all forms of government interfere unjustly with individual liberty and should be replaced by the voluntary association of cooperative groups. Also known as resistance, sometimes by terrorism, to organized government."
end

learn "WHAT IS TERRORISM" do
	"A form of warfare waged against innocent civilians."
end

learn "WHAT IS GENETICS" do
	"Genetics is the branch of science concerned with evolution, DNA, and inherited traits."
end

learn "WHAT IS BUSH" do
	"President of the United States. "
end

learn "WHAT IS IRAN" do
	"Iran is a nation in southern Asia. Iran is the site of ancient Persia."
end

learn "WHAT GOES WITHOUT SAYING" do
	"It is just an expression."
end

learn "WHAT BRAND IS A MAC" do
	"Apple"
end

learn "WHAT BRAND IS MAC" do
	"Apple"
end

learn "WHAT DOES THE WORD SHOE MEAN" do
	"A covering for the foot."
end

learn "WHAT DOES SUCK" do
	"a vacuum"
end

learn "HOW MANY OUNCES _ TEASPOON" do
	"1/6 ounce"
end

learn "HOW MANY OUNCES _ TABLESPOON" do
	"0.5 ounce"
end

learn "HOW MANY OUNCES _ PINT" do
	"16 ounces"
end

learn "HOW MANY OUNCES _ CUP" do
	"8 ounces"
end

learn "HOW MANY OUNCES _ QUART" do
	"32 ounces"
end

learn "HOW MANY OUNCES _ GILL" do
	"4 ounces"
end

learn "HOW MANY OUNCES _ GALLON" do
	"128 ounces"
end

learn "HOW MANY MONTHS *" do
	"12 months per year."
end

learn "HOW MANY MILES *" do
	"More than I would ever want to drive."
end

learn "HOW MANY NANOMETRES *" do
	"I spell it nanometers."
end

learn "HOW MANY OF *" do
	"Perhaps 50%."
end

learn "HOW MANY LITERS _ TEASPOON" do
	"4.929 milliliters"
end

learn "HOW MANY LITERS _ TABLESPOON" do
	"14.79 milliliters"
end

learn "HOW MANY LITERS _ PINT" do
	"473.2 milliliters"
end

learn "HOW MANY LITERS _ CUP" do
	"236.6 milliliters"
end

learn "HOW MANY LITERS _ QUART" do
	"946.3 milliliters"
end

learn "HOW MANY LITERS _ GILL" do
	"118 milliliters"
end

learn "HOW MANY LITERS _ GALLON" do
	"3.785 liters"
end

learn "HOW FAR IS ALPHA CENTAURI" do
	"Four light years."
end

learn "HOW LONG IS A MINUTE" do
	"sixty seconds."
end

learn "WHO INVENTED AIRPLANES" do
	"The Wright brothers invented the airplane in 1903."
end

learn "WHO INVENTED SETL" do
	"SETL was invented by Jacob Schwartz of NYU in 1969."
end

learn "WHO INVENTED THE GENERATOR" do
	"James Clerk Maxwell demonstrated the electric generator."
end

learn "WHO INVENTED THE TELEPHONE" do
	"Alexander Graham Bell demonstrated the first practical voice telephone in 1876."
end

learn "WHO INVENTED THE LIGHT BULB" do
	"Thomas Edison invented the light bulb. ."
end

learn "WHO INVENTED THE PC" do
	"Many people contributed to it. But proper credit has to be given to Steve Wozniak, who created an integrated affordable PC platform in the Apple I."
end

learn "WHO INVENTED THE HELICOPTER" do
	"Sikorsky."
end

learn "WHO INVENTED THE STEAM ENGINE" do
	"James Watt developed the first practical steam engine."
end

learn "WHO INVENTED PAPER" do
	"Paper was first developed by the Ancient Egyptians. "
end

learn "WHO INVENTED STEEL" do
	"The first practical steel manufacturing process was invented by Bessemer."
end

learn "WHO INVENTED TELEVISION" do
	"Philo T. Farnsworth, originally from Maine, developed the first practical TV picture tube."
end

learn "WHO INVENTED ELECTRICITY" do
	"Electricity was not invented but discovered, probably first by the ancient Chinese."
end

learn "WHO INVENTED AUTOMOBILES" do
	"Many people invented the car, but Henry Ford was the first to develop the successful mass production assembly lines for cars."
end

learn "WHO INVENTED JAVA" do
	"Java was first developed by James Gosling and colleagues at Sun."
end

learn "WHO INVENTED LINUX" do
	"Linus Torvalds, along with many open source developers, and the Free Software Foundation GNU."
end

learn "WHO INVENTED RADIO" do
	"Marconi developed the first practical wireless."
end
